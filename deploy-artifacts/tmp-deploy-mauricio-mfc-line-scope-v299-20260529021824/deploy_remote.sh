...
artifact=/tmp/tmp-deploy-mauricio-mfc-line-scope-v299-20260529021824.tgz
workdir=/tmp/tmp-deploy-mauricio-mfc-line-scope-v299-20260529021824
tag=agentezap-app:mauricio-mfc-line-scope-v299-20260529021824
base=agentezap-app:mauricio-mfc-promo-scope-v298-20260529020433
...

{
  ...
  printf 'LABEL agentezap.patch="mauricio-mfc-line-scope-v299"\n'
} > "$workdir/Dockerfile"

test ! -d "$workdir/dist/public"
grep -aR -m 1 'mauricioMfcLineKind' "$workdir/dist" >/dev/null
grep -aR -m 1 'mauricioMfcCatalogEntryMatchesLineKind' "$workdir/dist" >/dev/null
...
echo
printf 'MARKER='
docker exec agentezap-app sh -lc "grep -aR -m 1 'mauricioMfcLineKind' /app/dist >/dev/null && grep -aR -m 1 'mauricioMfcCatalogEntryMatchesLineKind' /app/dist >/dev/null && grep -aR -m 1 'BEBIDAS_DELIVERY2' /app/dist >/dev/null && echo mauricio-mfc-line-scope-ok"
