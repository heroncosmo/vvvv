...
NODE')"
echo "service_codex_home_configured=yes"
docker exec "$APP_CONTAINER" sh -lc 'codex login status >/tmp/policy-audit-codex-status.txt 2>&1 && echo codex_login_status_default=ok || echo codex_login_status_default=fail; rm -f /tmp/policy-audit-codex-status.txt'
docker exec -e CODEX_HOME="$SERVICE_CODEX_HOME" "$APP_CONTAINER" sh -lc 'codex login status >/tmp/policy-audit-codex-status.txt 2>&1 && echo codex_login_status_service_home=ok || echo codex_login_status_service_home=fail; rm -f /tmp/policy-audit-codex-status.txt'
if ! docker exec -e CODEX_HOME="$SERVICE_CODEX_HOME" "$APP_CONTAINER" sh -lc 'codex login status >/tmp/policy-audit-codex-status.txt 2>&1'; then
  docker exec "$APP_CONTAINER" sh -lc 'rm -f /tmp/policy-audit-codex-status.txt'
  echo "POLICY_AUDIT_READONLY_ABORT_CODEX_LOGIN"
  exit 5
fi
docker exec "$APP_CONTAINER" sh -lc 'rm -f /tmp/policy-audit-codex-status.txt'
