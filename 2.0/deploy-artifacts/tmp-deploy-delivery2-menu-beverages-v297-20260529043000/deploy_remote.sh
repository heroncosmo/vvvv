...
artifact=/tmp/tmp-deploy-delivery2-menu-beverages-v297-20260529043000.tgz
workdir=/tmp/tmp-deploy-delivery2-menu-beverages-v297-20260529043000
tag=agentezap-app:delivery2-menu-beverages-v297-20260529043000
base=agentezap-app:vicosa-cardapio-no-handoff-v296-20260529035301
...

{
  ...
  printf 'LABEL agentezap.patch="delivery2-menu-beverages-v297"\n'
} > "$workdir/Dockerfile"

test ! -d "$workdir/dist/public"
grep -aR -m 1 'BEBIDAS_DELIVERY2' "$workdir/dist" >/dev/null
grep -aR -m 1 'menu_auto_send_on_greeting' "$workdir/dist" >/dev/null
...
echo
printf 'MARKER='
docker exec agentezap-app sh -lc "grep -aR -m 1 'BEBIDAS_DELIVERY2' /app/dist >/dev/null && grep -aR -m 1 'menu_auto_send_on_greeting' /app/dist >/dev/null && grep -aR -m 1 'responseOffersBeverages' /app/dist >/dev/null && echo delivery2-menu-beverages-ok"
