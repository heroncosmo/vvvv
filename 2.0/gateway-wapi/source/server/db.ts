...
const isGatewayRuntime = String(process.env.SERVICE_MODE || "").trim() === "wa-gateway";
const configuredPoolMax =
  parsePositiveInteger(process.env.DB_POOL_MAX) ||
  (isGatewayRuntime ? parsePositiveInteger(process.env.WA_GATEWAY_DB_POOL_MAX) : null);
const poolMax =
  configuredPoolMax ||
  (isPoolerConnection
    ? (isGatewayRuntime ? 10 : 3)
    : (isGatewayRuntime ? 12 : 7));
