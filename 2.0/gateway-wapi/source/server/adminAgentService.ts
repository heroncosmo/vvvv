...
import { resolveAdminContextualMediaSelection } from "./adminMediaRouting";
import {
...
async function extractBusinessInfoWithLLM(userMessage: string): Promise<ExtractedBusinessInfo> {
  try {
    ...
    const systemPrompt = `Voce e um parser de informacoes de negocio. O usuario vai descrever seu negocio em linguagem informal de WhatsApp.

Extraia as seguintes informacoes em JSON puro, sem markdown:
{
  "companyName": "nome do negocio/empresa, apenas o nome proprio",
  "businessDescription": "resumo curto do que o negocio faz ou vende",
  "agentType": "delivery|salon|scheduling|generic",
  "mainProduct": "principal produto ou servico"
}

REGRAS CRITICAS para companyName:
- Extraia apenas o nome proprio do negocio.
- Nao use frases descritivas como nome.
- Se nao ficou claro qual e o nome do negocio, retorne null.
- Exemplo: "sou da drielle calcados" -> "Drielle Calcados".
- Se o cliente disse "meu negocio e X" ou "sou do/da X", X provavelmente e o nome.

REGRAS para agentType:
- "delivery" = restaurante, lanchonete, pizzaria, hamburgueria, marmita, acai.
- "salon" = barbearia, salao, cabeleireiro, manicure, estetica, lash, sobrancelha.
- "scheduling" = quando houver agendamento, consulta ou reserva.
...
async function getMasterPrompt(session: ClientSession): Promise<string> {
  ...
  if (existingUser && !session.userId && !forceNew) {
    ...
    try {
      ...
      // Verificar se tem acesso efetivamente ativo (inclui comprovante pendente)
...
async function getReturningClientContext(session: ClientSession, existingUser: any): Promise<string> {
  ...
  try {
    ...
    if (sub) {
      subscriptionStatus = entitlement.hasActiveSubscription
          ? `Ã¢Å“â€¦ Plano ativo`
          : (entitlement.isPendingReceiptAccess
              ? `Ã¢Å¡Â Ã¯Â¸Â Comprovante em analise (ativa so apos aprovacao)`
              : `Ã¢Å¡Â Ã¯Â¸Â Sem plano (limite de 25 msgs)`);
    ...
    try {
      ...
      if (sub) {
        subscriptionStatus = entitlement.hasActiveSubscription
          ...
          : (entitlement.isPendingReceiptAccess
              ? `Ã¢Å¡Â Ã¯Â¸Â Comprovante em analise (ativa so apos aprovacao)`
              : `Ã¢ÂÅ’ Sem plano (limite de 25 msgs)`);
      }
    } catch {}
  }
...
function buildFullPrompt(config: { name?: string; company?: string; role?: string; prompt?: string }): string {
  return repairMojibakeText(`Voc\u00ea \u00e9 ${config.name || "o atendente"}, ${config.role || "atendente"} da ${config.company || "empresa"}.
        ...
        if (ctxState && typeof ctxState === "object") {
          ...
          if (ctxState.pendingAction && !session.pendingAction) {
            ...
            const companyFromPending = String(
              restored?.payload?.nomeEmpresa ||
              restored?.payload?.companyName ||
              restored?.payload?.company ||
              restored?.payload?.businessName ||
              restored?.payload?.nomeNegocio ||
              "",
            ).trim();
            const updatedAtMs = conversation?.updatedAt ? new Date(conversation.updatedAt as any).getTime() : 0;
            const recentWindowMs = 14 * 24 * 60 * 60 * 1000;
            const allowRecentCreateRecovery =
              restored?.type === "criar_agente" &&
              Boolean(companyFromPending) &&
              Boolean(updatedAtMs && Date.now() - updatedAtMs <= recentWindowMs);
            ...
            if (restored && ((restored.expiresAt && restored.expiresAt > Date.now()) || allowRecentCreateRecovery)) {
              ...
              console.log(
                '[STATE] Restaurado pendingAction do banco para ' +
                  ...
                  ' (tipo=' +
                  restored.type +
                  (allowRecentCreateRecovery && (!restored.expiresAt || restored.expiresAt <= Date.now())
                    ? ', recuperado por contexto recente'
                    : '') +
                  ')',
              );
            } else {
    ...
    try {
      ...
      if (graphResult.shouldCreateAgent) {
        ...
        if (!resolvedGraphCompany) {
          response.text =
            "Fechou. Antes de eu criar seu teste, me fala so o nome da sua empresa para eu deixar o agente certo no seu negocio.";
  ...
  if (processedMediaActions.length === 0 && bypassOnboardingGraph) {
    try {
      const contextualSelection = await resolveAdminContextualMediaSelection({
        ...
        mediaLibrary: await getAdminMediaList(undefined),
      });
