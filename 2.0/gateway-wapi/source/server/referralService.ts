...
import { analyzeImageForAdmin } from "./mistralClient";
import {
  buildReferralCode,
...
const SUPPORT_MATERIAL_AI_MODEL = "pixtral-12b-2409";
...
function sanitizeAiGeneratedTitle(raw: string, fallbackTitle: string) {
  const normalized = normalizeSupportMaterialLabel(raw);
  if (!normalized) return fallbackTitle;
  const lowered = normalized.toLowerCase();
  if (["imagem", "foto", "arte", "material", "arquivo", "documento"].includes(lowered)) {
...
async function buildReferralSupportMaterialMetadata(input: ReferralSupportMaterialUploadInput & { mediaType: SupportMaterialMediaType }) {
  ...
  try {
    const aiResult = await analyzeImageForAdmin(input.fileUrl);
    if (!aiResult) {
      return fallback;
    }

    const title = sanitizeAiGeneratedTitle(aiResult.summary, fallback.title);
    const description = clampText(aiResult.description || fallback.description, 220) || fallback.description;

    return {
      title,
      description,
      caption: clampText(description, 140),
      aiGenerated: true,
      aiModel: SUPPORT_MATERIAL_AI_MODEL,
      metadataJson: {
        source: "mistral_vision",
        visionSummary: aiResult.summary,
        visionDescription: aiResult.description,
        mediaType: input.mediaType,
      },
    };
  } catch (error) {
    console.warn("[REFERRAL] Failed to generate support material metadata with vision:", error);
    return fallback;
  }
}
...
async function callJsonLlm<T>(messagesInput: Array<{ role: "system" | "user" | "assistant"; content: string }>, maxTokens = 300): Promise<T | null> {
  try {
    ...
    console.warn("[REFERRAL] LLM JSON parsing failed:", error);
    return null;
  }
}
