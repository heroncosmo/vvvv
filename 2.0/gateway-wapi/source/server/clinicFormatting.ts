...
export function normalizeClinicDateValue(value?: string | null): string | null {
...
export function normalizeClinicTimeValue(value?: string | null): string | null {
  if (!value) return null;
...
export function formatClinicContextualDate(dateStr: string, referenceDate?: string): string {
  const normalizedDate = normalizeClinicDateValue(dateStr);
  if (!normalizedDate) {
    return dateStr;
  }

  const referenceIso = normalizeClinicDateValue(referenceDate) || getBrazilToday();
...
export function buildDeterministicSlotSuggestionMessage(options: {
  ...
  const contextualDate = formatClinicContextualDate(options.date);
