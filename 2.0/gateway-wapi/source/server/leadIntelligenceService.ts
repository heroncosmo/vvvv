...
function buildLeadFallbackMessage(recipient: LeadCampaignRecipient, manualBaseText: string) {
  const cleanedManualBaseText = trimText(manualBaseText, 700);
  const hasTemplatePlaceholders = /\{[a-z_]+\}/i.test(cleanedManualBaseText);

  if (cleanedManualBaseText && hasTemplatePlaceholders) {
    return trimText(renderLeadCampaignTemplate(cleanedManualBaseText, recipient), 1200);
  }

  const contextualIntro = renderLeadCampaignTemplate(DEFAULT_CAMPAIGN_TEMPLATE, recipient);
  const approachText = trimText(recipient.recommendedApproach || "", 220);
  const summaryText = trimText(recipient.summary || "", 220);
  const manualDirection = cleanedManualBaseText
    ? `Seguindo a linha da campanha, ${cleanedManualBaseText.replace(/\s+/g, " ").trim()}`
    : "";

  return trimText(
    [
      contextualIntro,
      approachText ? `A abordagem que faz mais sentido aqui seria ${approachText}` : "",
      summaryText ? `Pelo que identifiquei do seu contexto, ${summaryText}` : "",
      manualDirection,
    ]
      .filter(Boolean)
      .join(" "),
...
async function requestCompactLeadQualification(params: {
  ...
  const completion = await chatComplete({
    messages: [
      {
        role: "system",
        content: "Voce classifica leads internos e responde somente com JSON compacto valido.",
      },
      {
        role: "user",
        content: buildCompactLeadQualificationPrompt(params),
      },
    ],
    maxTokens: 220,
    temperature: 0.1,
    ...
    messages: [
      {
        role: "system",
        content:
          "Responda com um unico objeto JSON valido, minificado, completo e fechado. Nenhum texto antes ou depois.",
      },
      {
        role: "user",
        content: [
          buildCompactLeadQualificationPrompt(params),
          "",
          "IMPORTANTE: a resposta deve comecar com { e terminar com } em uma unica linha.",
        ].join("\n"),
      },
    ],
    maxTokens: 320,
...
async function requestCompactLeadCatalogProfile(params: {
  ...
  const completion = await chatComplete({
    messages: [
      {
        role: "system",
        content: "Voce cataloga leads internos e responde somente com JSON compacto valido.",
      },
      {
        role: "user",
        content: buildCompactLeadCatalogPrompt(params),
      },
    ],
    maxTokens: 260,
    temperature: 0.1,
  ...
  const completion = await chatComplete({
    messages: [
      ...
      },
      {
        role: "user",
        content: [
          buildCompactLeadCatalogPrompt(params),
          "",
          "IMPORTANTE: a resposta deve comecar com { e terminar com } em uma unica linha.",
        ].join("\n"),
      },
...
export async function analyzeConversationLead(params: {
  ...
  const qualificationPrompt = buildLeadQualificationPrompt({
    conversation: context.conversation,
    messages: context.messages,
    latestAgentReply,
  });

  const qualificationCompletion = await chatComplete({
    messages: [
      {
        role: "system",
        content:
          "Voce atua como classificador interno de leads do AgenteZap. Nao escreva texto fora do JSON solicitado.",
      },
      {
        role: "user",
  ...
  const qualificationRawText = String(qualificationCompletion.choices?.[0]?.message?.content || "").trim();
  if (!qualificationRawText) {
    throw new Error("A IA nao retornou classificacao do lead AgenteZap");
  }
  ...
  const catalogPrompt = buildLeadCatalogPrompt({
    conversation: context.conversation,
    messages: context.messages,
    latestAgentReply,
  });

  const catalogCompletion = await chatComplete({
    messages: [
      {
        role: "system",
        content:
          "Voce organiza um banco interno de leads e retorna somente JSON valido para uso administrativo.",
      },
      {
        role: "user",
  ...
  const catalogRawText = String(catalogCompletion.choices?.[0]?.message?.content || "").trim();
  if (!catalogRawText) {
    throw new Error("A IA nao retornou perfil de catalogo do lead");
  }
  ...
  return mapLeadInsightRow((upsertResult as any)?.rows?.[0] || {});
}
...
export async function generateLeadCampaignTemplate(
  ...
  if (leads.length === 0) {
    return {
      messageTemplate: DEFAULT_CAMPAIGN_TEMPLATE,
      rationale: "Nenhum lead selecionado; usando modelo padrao.",
  ...
  const fallbackPreviews = leads.map((lead) => {
    ...
    return {
      ...
      message: buildLeadFallbackMessage(recipient, baseManualText),
      rationale: trimText(
        `Fallback local com foco em ${lead.recommendedApproach || "abordagem sugerida"} e no contexto identificado.`,
        220,
      ),
  ...
  try {
    const completion = await chatComplete({
      messages: [
        {
          role: "system",
          content:
            "Voce gera mensagens internas de prospeccao do AgenteZap. Retorne apenas JSON valido.",
        },
        {
          role: "user",
          content: prompt,
      ...
      maxTokens: Math.min(4200, Math.max(900, 220 * leads.length)),
      temperature: 0.35,
      skipMistralQueue: true,
    });
    ...
    const messageTemplate =
      trimText(parsed?.messageTemplate, 1200) ||
      trimText(baseManualText, 1200) ||
      DEFAULT_CAMPAIGN_TEMPLATE;
    const rationale =
      trimText(parsed?.rationale, 500) ||
      "Mensagem gerada com base em leitura da IA, perfil detectado e abordagem sugerida.";
    const items = Array.isArray(parsed?.items) ? parsed.items : [];
    const previews = fallbackPreviews.map((fallbackPreview) => {
      const matched = items.find((item: any) => String(item?.leadId || "") === fallbackPreview.leadId);
      const message = trimText(matched?.message, 1200) || fallbackPreview.message;
      const itemRationale = trimText(matched?.rationale, 220) || fallbackPreview.rationale;
    ...
    return { messageTemplate, rationale, previews };
  } catch (error) {
    console.error("[LEAD INTELLIGENCE] Falha ao gerar mensagem de campanha:", error);
    return {
      messageTemplate: trimText(baseManualText, 1200) || DEFAULT_CAMPAIGN_TEMPLATE,
      rationale: "Fallback local aplicado por falha na IA.",
