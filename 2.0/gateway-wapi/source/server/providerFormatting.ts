import { getBrazilTimeDate } from "./greetingTime";
...
export function normalizeProviderDateValue(value?: string | null): string | null {
...
export function normalizeProviderTimeValue(value?: string | null): string | null {
  if (!value) return null;
...
export function formatProviderContextualDate(dateStr: string, referenceDate?: string): string {
  const normalizedDate = normalizeProviderDateValue(dateStr);
  if (!normalizedDate) {
    return dateStr;
  }

  const referenceIso = normalizeProviderDateValue(referenceDate) || getBrazilToday();
...
export function buildDeterministicSlotSuggestionMessage(options: {
  ...
  const contextualDate = formatProviderContextualDate(options.date);
