...
async function requestStrictCourseSchedulingInsightJson(params: {
  ...
  const completion = await chatComplete({
    messages: [
      ...
      },
      {
        role: "user",
        content: [
          buildCourseSchedulingPrompt(params),
          "",
          "Retorne agora apenas um JSON valido e minificado, sem markdown, sem texto adicional e sem aspas soltas dentro dos arrays.",
        ].join("\n"),
      },
    ],
    maxTokens: 320,
  ...
  if (!rawText) {
    throw new Error("A IA nao retornou JSON estrito de agendamento de curso");
  }
...
async function analyzeConversationCourseScheduling(params: {
    ...
    messages: [
      {
        role: "system",
        content:
          "Voce atua como auditor interno de agendamentos fechados em conversas de curso. Nunca escreva nada fora do JSON solicitado.",
      },
      {
        role: "user",
        content: buildCourseSchedulingPrompt({
  ...
  const rawText = String(completion.choices?.[0]?.message?.content || "").trim();
  if (!rawText) {
    throw new Error("A IA nao retornou classificacao de agendamento de curso");
  }
