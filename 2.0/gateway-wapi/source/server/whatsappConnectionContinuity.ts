...
export async function findReusableDisconnectedConnectionForCreation(
  ...
  const reusableEmptyConnection = connections.find((connection) => {
    if (connection.isConnected) {
      return false;
    }
    if (normalizeManagedPhoneNumber(connection.phoneNumber)) {
      return false;
    }
    const providerStatus = String(connection.providerStatus || "").trim().toLowerCase();
    if (!["", "inactive", "disconnected", "not_connected", "qr_required", "open_timeout"].includes(providerStatus)) {
      return false;
    }
    return true;
  });
  if (reusableEmptyConnection) {
    const historyCount = await countConnectionConversations(reusableEmptyConnection.id);
    if (historyCount === 0) {
      return reusableEmptyConnection;
    }
  }
