...
import { chatComplete, detectMediaSendingIntent, type ChatMessage } from "./llm";
...
function listCatalogAnchorTerms(product: CatalogProductCandidate): string[] {
  ...
  for (const term of rawTerms) {
    ...
    if (!cleanTerm || cleanTerm.length < 4 || stopTerms.has(cleanTerm)) {
      continue;
    }
...
function buildAttachedImagesResponseFallback(productName: string, imageCount: number): string {
  return `Separei ${imageCount} foto(s) de ${productName} e estou enviando agora logo abaixo para voce ver.`;
}
