...
export async function ensureKanbanBoardsForOwner(ownerId: string): Promise<void> {
  await pool.query(
    `
      ...
      SELECT $1, 'Kanban Principal', 'Quadro principal do CRM', 'default', true, true
      WHERE NOT EXISTS (
        SELECT 1
        FROM kanban_boards
        WHERE owner_id = $1
  ...
  if (defaultBoardId) {
    ...
    await pool.query(
      `
        ...
        SET kanban_board_id = COALESCE(c.kanban_board_id, ks.board_id, $2)
        ...
        WHERE c.connection_id = wc.id
          ...
          AND (
            ...
            OR (c.kanban_stage_id IS NOT NULL AND ks.board_id IS NOT NULL AND c.kanban_board_id <> ks.board_id)
          )
      `,
      [ownerId, defaultBoardId],
    );
