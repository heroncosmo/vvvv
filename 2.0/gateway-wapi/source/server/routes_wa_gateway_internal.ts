...
import { flushGatewayEventOutbox, getGatewayEventOutboxStats } from "./gatewayEventOutboxService";
...
export function registerWhatsAppGatewayInternalRoutes(app: Express) {
  ...
  app.get("/internal/event-outbox/status", async (req: Request, res: Response) => {
    if (!requireGatewayAuthorization(req, res)) {
      return;
    }
    ...
    res.json({
      success: true,
      ...getGatewayEventOutboxStats(),
    });
  });

  app.post("/internal/event-outbox/flush", async (req: Request, res: Response) => {
    if (!requireGatewayAuthorization(req, res)) {
      return;
    }
    ...
    res.json({
      success: true,
      ...(await flushGatewayEventOutbox({
        limit: Number.isFinite(limit) ? Math.max(1, Math.min(1000, Math.floor(limit))) : 100,
      })),
      ...getGatewayEventOutboxStats(),
    });
  });
    ...
    await withGatewayConnection(req, res, async (connection) => {
      ...
      res.json(
        await sendTextViaInstance({
          ...
          existingMessageDbId: body.existingMessageDbId || undefined,
        }),
      );
    });
  });
