...
export class UserFollowUpService {
  ...
  private async sendFollowUpAsAudio(
    ...
    const voice = audioConfig.voiceType === "male" ? "pt-BR-AntonioNeural" : "pt-BR-FranciscaNeural";
  ...
  private async applyBacklogGovernor(conversationsToProcess: any[], now: Date): Promise<any[]> {
    ...
    for (const { userId, connectionId, count } of deferredByScope.values()) {
      console.log(
  ...
  private async processFollowUps() {
    ...
    try {
      await this.recoverStaleWaitingConnectionReasons();
      ...
      const sorted = sortUserFollowUpBacklogEntries(readyPendingConversations);
      ...
      if (uniqueConversations.length !== readyPendingConversations.length) {
        console.log(`[USER-FOLLOW-UP] Deduplication: ${readyPendingConversations.length} -> ${uniqueConversations.length} unique conversations`);
      }
      ...
      for (const conv of governedConversations) {
  ...
  private async executeFollowUp(conversation: any) {
    ...
    try {
      ...
      try {
        ...
        if (shouldHoldFollowUpForOutgoingConfirmation(recentMsg)) {
...
{"action":"${decisionWindow.allowedActions.join("|")}","reason":"motivo curto","message":"texto (só se send)","scheduleDate":"YYYY-MM-DDTHH:MM (só se schedule)"}`;
  ...
  async pauseFollowUpUntilCompanyReply(conversationId: string, reason?: string) {
    ...
    console.log(`⏸️ [USER-FOLLOW-UP] Pausado para ${conversationId} até a empresa responder novamente.`);
  }
