...
import {
  flushGatewayEventOutbox,
  startGatewayEventOutbox,
  stopGatewayEventOutbox,
} from "./gatewayEventOutboxService";
...
export async function startWhatsAppGateway() {
  ...
  const server = app.listen(port, "0.0.0.0", async () => {
    ...
    if (!disableBackgroundJobs) {
      ...
      void (async () => {
        ...
        try {
          console.log("[WA GATEWAY] Starting background restore of customer sessions...");
  ...
  const gracefulShutdown = async (signal: string) => {
    ...
    try {
      ...
      stopWhatsAppSessionSnapshotCron();
      stopConnectionHealthCheck();
      await flushGatewayEventOutbox({ limit: 100 });
