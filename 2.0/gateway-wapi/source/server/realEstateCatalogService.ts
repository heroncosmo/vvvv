...
type RealEstateCatalogResolverDecision = {
  decision: "MATCH" | "ASK_CLARIFY" | "NO_MATCH";
  code: string | null;
  confidence: number;
  reason: string;
};
...
function parseCatalogResolverDecision(rawResponse: string): RealEstateCatalogResolverDecision | null {
  ...
  let jsonText = sanitizedLines.join("\n").trim() || trimmed;
  ...
  try {
    ...
    return {
      decision:
        parsed?.decision === "MATCH" || parsed?.decision === "ASK_CLARIFY" || parsed?.decision === "NO_MATCH"
          ? parsed.decision
          : "ASK_CLARIFY",
    ...
    console.warn("[RealEstateCatalog] Falha ao parsear decisao do resolver LLM:", error);
    return null;
  }
}
...
async function resolveGrupoOlxListingWithLLM(params: {
  ...
  const { customerMessage, conversationHistory = [], candidates } = params;
  ...
  const candidatesText = topCandidates
    .map((candidate, index) => {
      ...
      return [
        `${index + 1}. CODE=${listing.listingCode || listing.externalListingId}`,
        `TITLE=${listing.title}`,
        `ADDRESS=${listing.address || "nao informado"}`,
        `NEIGHBORHOOD=${listing.neighborhood || "nao informado"}`,
        `CITY=${listing.city || "nao informado"}`,
        `PRICE=${listing.price || "nao informado"}`,
  ...
  const systemPrompt = [
    "Voce e um resolvedor de referencia imobiliaria dentro de um orquestrador LLM.",
    "Sua funcao e decidir se a mensagem atual do cliente aponta para um anuncio especifico do catalogo.",
    "Use a mensagem atual, o historico recente e os candidatos do catalogo.",
    "Aceite abreviacoes e pequenos erros humanos de digitacao quando o restante do contexto apontar para o mesmo anuncio.",
    "Nunca invente imovel. Se houver duvida real, responda ASK_CLARIFY.",
    'Responda somente JSON puro com: {"decision":"MATCH|ASK_CLARIFY|NO_MATCH","code":"codigo ou null","confidence":0-100,"reason":"motivo curto"}',
  ].join(" ");

  const userPrompt = [
    `MENSAGEM_ATUAL: ${customerMessage}`,
    `HISTORICO_RECENTE:\n${historyText}`,
    `CANDIDATOS_DO_CATALOGO:\n${candidatesText}`,
    "Escolha MATCH apenas se houver confianca alta de que o cliente quis exatamente aquele anuncio.",
    "Se a pessoa falou rua e numero, compare isso com o endereco do catalogo.",
  ].join("\n\n");

  try {
    const rawResponse = await generateWithLLM(systemPrompt, userPrompt, {
      temperature: 0,
      maxTokens: 180,
    });
    const decision = parseCatalogResolverDecision(rawResponse);
    if (!decision) return null;

    if (decision.decision !== "MATCH" || decision.confidence < 70 || !decision.code) {
      return [];
    }

    const matchedCandidate = topCandidates.find((candidate) =>
      [candidate.listing.listingCode, candidate.listing.externalListingId]
        .filter(Boolean)
        .some((value) => String(value).toUpperCase() === decision.code!.toUpperCase()),
    );

    return matchedCandidate ? [mapListingForAI(matchedCandidate)] : [];
  } catch (error) {
    console.warn("[RealEstateCatalog] Resolver LLM falhou, usando ranking local:", error);
    return null;
  }
}
