...
export interface IStorage {
  ...
  getPublicCatalogPlans(): Promise<Plan[]>;
  ...
  markPendingAIResponseFailed(conversationId: string, reason: string, lastError?: string): Promise<void>;
  resetPendingAIResponseForRetry(conversationId: string, delaySec?: number): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getConnectionByUserId(userId: string, connectionId?: string): Promise<WhatsappConnection | undefined> {
    ...
    return memoryCache.getOrCompute(cacheKey, async () => {
      ...
      const visibleConnections = allConnections.filter(
  ...
  async getPrimaryConnectionPerUser(): Promise<WhatsappConnection[]> {
    ...
    for (const conn of allConnections) {
      ...
      if (isInternalOnlySimulatorConnection(conn)) continue;
  ...
  async getRecentConversationRecoveryState(
    connectionId: string,
    since: Date,
  ): Promise<Array<Pick<Conversation, "id" | "contactNumber" | "jidSuffix" | "lastMessageText" | "lastMessageTime" | "lastMessageFromMe">>> {
    return await db
      .select({
        id: conversations.id,
  ...
  async updateMessage(id: string, data: Partial<InsertMessage>): Promise<Message> {
    ...
    if (
      ...
      nextMediaType === "audio" &&
      nextMediaUrl &&
      isAudioPlaceholderText(nextText)
    ) {
      try {
...
Responda de forma concisa. Para catalogo/pedido com varios itens, use uma lista curta com ate 10 itens para preservar todos os codigos visiveis.`;
  ...
  async getDisabledConversationIds(conversationIds: string[]): Promise<string[]> {
    ...
    const disabledRows = await db
      .select({
        conversationId: agentDisabledConversations.conversationId,
      })
      .from(agentDisabledConversations)
      .where(inArray(agentDisabledConversations.conversationId, conversationIds));

    return disabledRows.map((row) => row.conversationId);
  }
  ...
  async getPublicCatalogPlans(): Promise<Plan[]> {
    const activePlans = await this.getActivePlans();
    return filterPublicCatalogPlans(activePlans);
  }
...
Responda de forma concisa (máximo 3 frases) descrevendo o que você vê.`;
  ...
  async incrementAudioMessageCounter(userId: string): Promise<{ count: number; limit: number; canSend: boolean }> {
    ...
    const hasUnlimitedAudio = Array.isArray(subscription?.plan?.caracteristicas)
      && subscription.plan.caracteristicas.includes("audio_ilimitado");
    ...
    if (!counter) {
      ...
      const [newCounter] = await db.insert(audioMessageCounter).values({
        ...
        date: today,
        count: 1,
        dailyLimit: hasUnlimitedAudio ? -1 : 30,
      }).returning();
      return { count: 1, limit: newCounter.dailyLimit, canSend: true };
    }
    
    if (hasUnlimitedAudio && counter.dailyLimit !== -1) {
      const [updatedToUnlimited] = await db.update(audioMessageCounter)
        .set({ count: counter.count + 1, dailyLimit: -1, updatedAt: new Date() })
        .where(eq(audioMessageCounter.id, counter.id))
        .returning();

      return { count: updatedToUnlimited.count, limit: -1, canSend: true };
    }

    if (counter.dailyLimit === -1) {
      const [updatedUnlimited] = await db.update(audioMessageCounter)
        .set({ count: counter.count + 1, updatedAt: new Date() })
        .where(eq(audioMessageCounter.id, counter.id))
        .returning();

      return { count: updatedUnlimited.count, limit: -1, canSend: true };
    }
  ...
  async canSendAudio(userId: string): Promise<{ canSend: boolean; remaining: number; limit: number; used: number; isUnlimited: boolean }> {
    ...
    if (config && !config.isEnabled) {
      return { canSend: false, remaining: hasUnlimitedAudio ? -1 : 0, limit: hasUnlimitedAudio ? -1 : 30, used: 0, isUnlimited: hasUnlimitedAudio };
    }
    ...
    if (!counter) {
      return {
        canSend: true,
        remaining: hasUnlimitedAudio ? -1 : 30,
        limit: hasUnlimitedAudio ? -1 : 30,
        used: 0,
        isUnlimited: hasUnlimitedAudio,
      };
    }
    ...
    return { canSend: remaining > 0, remaining, limit: counter.dailyLimit, used: counter.count, isUnlimited: false };
  }
  ...
  async getConversationLastMessageTimes(conversationId: string): Promise<{
    ...
    try {
        // "queued" ja significa que o callback de envio real retornou com sucesso.
        // Se ignorarmos esse status, os crons de recovery entendem que a conversa
        // segue sem resposta e reprocessam o mesmo timer em loop.
        const result = await db.execute(sql`
          SELECT
