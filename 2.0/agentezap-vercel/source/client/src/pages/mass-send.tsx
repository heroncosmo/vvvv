...
export default function MassSendPage() {
  ...
  const hasGreetingPlaceholder = (template: string): boolean => (
    /\[(?:saudacao|sauda(?:c|\u00e7)(?:a|\u00e3)o|cumprimento)\]/i.test(template)
  );

  const applyMessageTemplate = (template: string, name: string): string => {
    return template
      .replace(/\[nome\]/gi, name || 'Cliente')
      .replace(/\[(?:saudacao|sauda(?:c|\u00e7)(?:a|\u00e3)o|cumprimento)\]/gi, getCurrentGreeting());
  };
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Use <code className="bg-muted px-1.5 py-0.5 rounded text-xs">[nome]</code> para o nome e <code className="bg-muted px-1.5 py-0.5 rounded text-xs">[saudacao]</code> para Bom dia, Boa tarde ou Boa noite
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Textarea
...
Use [nome] para inserir o nome e [saudacao] para ajustar a saudacao pelo horario.`}
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    ...
                    {hasGreetingPlaceholder(messageTemplate) && (
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Button
                    variant="secondary"
                    size="sm"
                    onClick={() => setMessageTemplate(prev => prev + '[saudacao]')}
                  >
                    [saudacao]
                  </Button>
                </div>
