...
import { MessageCircle, Settings, Smartphone, Bot, CreditCard, LayoutDashboard, AlertCircle, Send, Kanban, Users, Tags, Filter, Plug, CalendarClock, BedDouble, Wrench, Megaphone, Upload, BookUser, BookOpen, Bell, Rocket, Sparkles, Zap, Receipt, Ban, Building2, FormInput, Package, ShoppingBag, UtensilsCrossed, ClipboardList, Mic, Workflow, Ticket, HelpCircle, Gift, QrCode, X, Palette, Shield, Copy, Mail, Wallet, ChevronRight, Plus, Search, Download, Star, Globe, Database, BarChart3 } from "lucide-react";
...
export default function Dashboard() {
  ...
  }, [isConversasRoute, isConexaoRoute, isMeuAgenteRoute, isDashboardMode, location]);
  ...
  const { data: agentMediaItems } = useQuery<SidebarAgentMediaItem[]>({
  ...
  const selectedSidebarAddonIds = useMemo(() => {
    const metadata = (subscription as any)?.metadata || {};
    const rawIds = Array.isArray(metadata.selectedAddonIds) ? metadata.selectedAddonIds : [];
    return new Set(
      rawIds
...
const toolsNavigation: ToolNavItem[] = [
  ...
  const hasSidebarAddon = (addonId: string) => selectedSidebarAddonIds.has(addonId);
  const hasActiveAgentMedia = Array.isArray(agentMediaItems)
    && agentMediaItems.some((media) => media?.isActive !== false);
  ...
  const activeSidebarToolsNavigation = filteredToolsNavigation.filter((item) => {
    ...
    switch (item.testId) {
      ...
      case "button-nav-whatsapp-qr":
        return hasSidebarAddon(PLAN_ADDON_IDS.whatsappQr);
      ...
      case "button-nav-contact-lists":
        return hasSidebarAddon(PLAN_ADDON_IDS.massSend);
      ...
      case "button-nav-grupo-zap":
        return hasSidebarAddon(PLAN_ADDON_IDS.conversationsCrm);
      case "button-nav-products":
        return productsConfig?.is_active === true || hasSidebarAddon(PLAN_ADDON_IDS.products);
      case "button-nav-media-library":
        return hasActiveAgentMedia;
      case "button-nav-meta-form":
        return hasSidebarAddon(PLAN_ADDON_IDS.metaForm);
      default:
        return false;
    }
  });
  ...
  const mobilePrimaryNavigation: ToolNavItem[] = [
    ...
    ...(canAccessAgentView
      ? [{
          ...
          icon: CreditCard,
          tooltip: "Planos",
          isActive: isPlansRoute || isSubscribeRoute,
  ...
  const sidebarPlanName = isEffectivelyPaid
    ...
    : assignedPlanData?.nome || "Plano Ilimitado";
  ...
  const filteredAppStoreTools = appStoreTools.filter((app) =>
    appStoreMatches(app.name, app.category)
  );
  const openTeamTools = () => {
  ...
  return (
    <>
      ...
      <Sidebar collapsible="none">
              ...
              <SidebarMenu>
                <SidebarMenuItem>
                  <SidebarMenuButton
                    ...
                    data-testid="button-nav-ai-assistant"
                  >
        ...
        <SidebarSeparator />
        <SidebarFooter className="bg-[#f1f1f1]">
            ...
            {canAccessReferrals && (
              <SidebarMenuItem>
                <button
                  ...
                  className="group flex w-full items-start gap-3 rounded-xl border border-[#e3e3e3] bg-white p-3 text-left shadow-sm transition hover:bg-slate-50"
                  data-testid="button-sidebar-referral-popup"
                  aria-label="Ganhe R$ 50,00 indicando o AgenteZap"
                >
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600 group-hover:bg-emerald-100">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block text-[10px] font-medium leading-tight text-slate-500">
                      Indique o AgenteZap e receba créditos.
                    </span>
                  </span>
                </button>
              </SidebarMenuItem>
            ...
            <SidebarMenuItem>
              <div className="rounded-xl bg-black p-3.5 text-white">
                ...
                <p className="mt-1 text-[11px] font-medium leading-tight text-slate-300">
                  {isEffectivelyPaid ? `${sidebarPlanName} no plano atual. Veja o próximo plano disponível.` : "Apenas R$ 49,99/mês. Use sem restrições."}
                </p>
                <button
                  type="button"
                ...
                >
                  {isEffectivelyPaid ? "Ver planos" : "Assinar Agora"}
                </button>
              </div>
            </SidebarMenuItem>
      ...
      <SidebarInset className="flex h-screen flex-col overflow-hidden">
        ...
        <div className="mobile-bottom-nav md:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-border/60 bg-background/95 backdrop-blur-sm pb-[env(safe-area-inset-bottom)]">
          <div className={`grid ${mobileNavColumns} text-[10px]`}>
            ...
            {canAccessDashboardView && (
            <button
              className={`flex flex-col items-center py-2.5 gap-0.5 ${isMeuAgenteRoute || (isDashboardMode && selectedView === "agent") ? "text-primary font-medium" : "text-muted-foreground"}`}
              onClick={() => goToSection("agent")}
            >
            ...
            {canAccessAgentView && (
            <button
              className={`flex flex-col items-center py-2.5 gap-0.5 ${isPlansRoute || isSubscribeRoute ? "text-primary font-medium" : "text-muted-foreground"}`}
              onClick={() => setLocation("/plans")}
            >
        ...
        <Sheet open={toolsPickerOpen} onOpenChange={setToolsPickerOpen}>
          <SheetContent side="left" className="flex h-full w-[86vw] max-w-sm flex-col gap-0 border-r border-[#e3e3e3] bg-[#f1f1f1] p-0">
            <SheetHeader className="border-b border-[#e3e3e3] px-4 py-5 text-left">
                  ...
                  {canAccessConnectionView && (
                    <div className="space-y-2">
                      ...
                      >
                        <span className={cn(
                          "w-9 h-9 rounded-lg flex items-center justify-center",
                          isConexaoRoute || (isDashboardMode && selectedView === "connection") ? "bg-primary/10" : "bg-muted"
                        )}>
                          <Globe className="w-4 h-4" />
                        </span>
