...
function matchesToolSearch(query: string, fields: Array<string | undefined | null>) {
  const tokens = normalizeToolSearchText(query).split(/\s+/).filter(Boolean);
  if (tokens.length === 0) return true;

  const haystack = normalizeToolSearchText(fields.filter(Boolean).join(" "));
...
const toolHubSections: ToolHubSection[] = [
  {
    ...
    description: "As áreas mais usadas do sistema para configurar, responder e destravar fluxo.",
    entries: [
      {
        id: "ai",
      ...
      },
      {
        ...
        description: "Configure respostas em áudio, espelhamento e áudio no follow-up.",
      ...
      },
      {
        ...
        description: "Organize áudios, imagens e vídeos que o agente vai usar.",
      ...
      },
      {
        id: "flow2-leona",
        label: "Fluxo 2.0",
        description: "Editor visual com blocos, canvas, PIX, IA, delays e condicoes.",
        href: "/meu-agente-ia?tab=flow2",
        icon: Workflow,
        keywords: ["fluxo 2.0", "chatbot visual", "pix", "canvas"],
        featured: true,
      },
      {
        id: "team",
        label: "Membros e Setores",
        description: "Gerencie equipe, acessos e organizacao de setores em uma ferramenta unica.",
        href: "/membros",
        icon: Users,
        keywords: ["membros", "setores", "equipe", "permissões"],
        memberHidden: true,
        subLinks: [
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        id: "delivery",
        label: "Delivery",
        description: "Central do delivery com cardápio, pedidos e relatórios no mesmo lugar.",
        href: "/delivery-cardapio",
        icon: UtensilsCrossed,
        keywords: ["delivery", "cardápio", "pedidos", "relatórios delivery"],
        featured: true,
        subLinks: [
...
function HubEntryCard({
  ...
  return (
    <Card className="rounded-xl border-[#e3e3e3] bg-white p-4 shadow-sm transition hover:border-[#8c9196] hover:bg-slate-50">
      <button type="button" className="flex w-full items-start gap-3 text-left" onClick={() => onOpen(entry.href)}>
        <span
          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-[#e3e3e3] bg-[#f1f1f1]"
          style={{ backgroundColor: brandSurface }}
        >
          <entry.icon className="h-5 w-5 text-[#008060]" />
        </span>
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-3 inline-flex items-center text-sm font-medium text-[#008060]">
            Abrir
...
function HubEntryRow({
  ...
  return (
    <div className="space-y-3 rounded-xl border border-[#e3e3e3] bg-white p-4">
...
export default function ToolsMenuPage() {
  ...
  const featuredEntries = visibleEntries.filter((entry) => entry.featured).slice(0, 3);
  ...
  return (
    <div className="flex-1 overflow-auto bg-[#f6f6f7]">
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <section className="overflow-hidden rounded-2xl border border-[#e3e3e3] bg-white shadow-sm">
          <div className="border-b border-[#e3e3e3] px-6 py-5">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h1 className="text-xl font-bold tracking-tight text-slate-900">Selecionados para você</h1>
                <p className="mt-1 text-sm text-slate-500">Encontre ferramentas, apps e atalhos do AgenteZap.</p>
              </div>
              <Badge variant="secondary" className="w-fit rounded-lg border border-[#e3e3e3] bg-[#f6f6f7]">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl space-y-2">
                ...
                <p className="text-sm leading-6 text-muted-foreground">
                  O caminho principal continua sendo a central acima. Se quiser, aqui no final você
