...
function formatArticleHtml(content: string) {
  return content
    ...
    .replace(
      /(https?:\/\/[^\s<]+)/g,
      '<a href="$1" target="_blank" rel="noreferrer" class="font-medium text-primary underline underline-offset-4">$1</a>',
    );
}
...
function ArticleView({
  ...
  return (
    <div className={`max-w-3xl mx-auto px-4 ${isCourseVideoArticle ? "py-3 sm:py-6" : "py-6"}`}>
      {/* Breadcrumb + voltar */}
      <div className={`items-center gap-2 text-sm text-muted-foreground ${isCourseVideoArticle ? "mb-3 hidden sm:flex" : "mb-6 flex"}`}>
      ...
      <div className={isCourseVideoArticle ? "mb-3 sm:mb-8" : "mb-8"}>
        <div className={`items-center gap-2 mb-2 ${isCourseVideoArticle ? "hidden sm:flex" : "flex"}`}>
          <Badge variant="secondary" className={diff.color}>
            {diff.label}
          </Badge>
        </div>
        <h1 className={`${isCourseVideoArticle ? "mb-1 text-xl sm:mb-2 sm:text-2xl" : "mb-2 text-2xl"} font-bold text-foreground`}>{article.title}</h1>
        <p className={`${isCourseVideoArticle ? "hidden sm:block" : ""} text-muted-foreground`}>{article.description}</p>
      </div>
                  ...
                  >
                    ...
                    <div className="px-4 py-3 space-y-2">
                      <p
                        ...
                        dangerouslySetInnerHTML={{
                          __html: formatArticleHtml(vs.explain),
                        }}
                      />
                      {vs.result && (
          ...
          <>
            ...
            <div className="mt-8 mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  {
                    title: "Curso AgenteZap",
                    desc: `${COURSE_VIDEOS.length} aulas e extras dentro da plataforma`,
                    icon: BookUser,
                    articleId: "course-video-all",
                    catId: "course-video",
                  },
                  {
