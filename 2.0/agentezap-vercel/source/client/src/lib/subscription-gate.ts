...
export interface UsageGateData {
  ...
  priorityMode?: "priority" | "economy" | "plus" | string;
...
export interface AccessStatusGateData {
  accessStatus: "active" | "trial" | "blocked" | "expired" | "free" | "free_economy" | "temporary_unavailable";
  shouldBlock: boolean;
  blockReason: string | null;
  hasActiveSubscription?: boolean;
...
interface SubscriptionGatedActionOptions {
  gatePremiumModuleControls?: boolean;
}
...
export function isSubscriptionGatedActionTarget(
  target: HTMLElement | null,
  options: SubscriptionGatedActionOptions = {},
): boolean {
  if (!target) {
    return false;
  }
  ...
  if (interactive.dataset.gatedAction === "false") {
    return false;
  }
  ...
  const hasPassiveIntent = Boolean(actionText && PASSIVE_KEYWORDS.some((keyword) => actionText.includes(keyword)));
  const hasActionIntent = Boolean(actionText && ACTION_KEYWORDS.some((keyword) => actionText.includes(keyword)));
  ...
  if (interactive instanceof HTMLInputElement) {
    ...
    if (inputType === "submit") {
      ...
      return options.gatePremiumModuleControls ? true : hasActionIntent;
    }
    ...
    if (inputType === "checkbox" || inputType === "radio") {
      ...
      return options.gatePremiumModuleControls || hasActionIntent;
    }
  ...
  if (interactive instanceof HTMLButtonElement) {
    ...
    if (buttonType === "button" && options.gatePremiumModuleControls) {
      if (hasPassiveIntent) {
        return false;
      }
      ...
      const ariaPressed = interactive.getAttribute("aria-pressed");
