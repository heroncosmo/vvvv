...
function getQrGeneratedAtFromConnection(connection?: { qrCodeGeneratedAt?: unknown; sessionData?: unknown; updatedAt?: unknown } | null): number | null {
  ...
  return parseQrGeneratedAt(connection?.updatedAt);
}
...
export function ConnectionPanel() {
  ...
  const lastOperationalConnectionsRef = useRef<
    Map<string, { connection: ConnectionWithAgents; expiresAt: number }>
  >(new Map());
  const visibleConnections = useMemo(() => {
    const now = Date.now();
    const lastOperational = lastOperationalConnectionsRef.current;

    for (const [connectionId, snapshot] of lastOperational.entries()) {
      if (snapshot.expiresAt <= now) {
        lastOperational.delete(connectionId);
      }
    }

    return allConnections.filter((conn) => !isInternalSimulatorConnection(conn)).map((conn) => {
      if (isAppVisibleOperationalConnection(conn)) {
        lastOperational.set(conn.id, {
          connection: conn,
          expiresAt: now + LAST_OPERATIONAL_CONNECTION_TTL_MS,
        });
        return conn;
      }
      ...
      const providerStatus = String(conn.providerStatus || "").trim().toLowerCase();
      const hasStrongDisconnectSignal =
        Boolean(conn.qrCode) ||
        [
          "auth_failed",
          "close",
      ...
      if (previous && !hasStrongDisconnectSignal && !isOfficialProviderConnection(conn)) {
        return {
          ...conn,
          isConnected: true,
          isRecovering: true,
  ...
  }, [connectMutation, startQrCodePolling]);
  ...
  useEffect(() => {
    ...
    const refreshExpiredQrCodes = () => {
      ...
      for (const [connectionId, qr] of Object.entries(connectionQrCodes)) {
        ...
        connectConnectionMutation.mutate(connectionId);
        break;
      }
    };
