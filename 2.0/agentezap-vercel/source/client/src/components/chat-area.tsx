...
const LAST_OPERATIONAL_CHAT_CONNECTION_TTL_MS = 2 * 60_000;
...
function normalizeMessageDedupeText(message: Partial<Message>) {
  return repairMojibakeText(String(message.text || message.mediaCaption || ""))
    .replace(/\s+/g, " ")
    .trim();
}

function normalizeMessageDedupeMediaType(message: Partial<Message>) {
  return String(message.mediaType || "").trim().toLowerCase();
}

function messagesAreCloseEnoughForRealtimeDedupe(a: Partial<Message>, b: Partial<Message>) {
  return Math.abs(getMessageTimeMs(a) - getMessageTimeMs(b)) <= RECENT_OPTIMISTIC_MESSAGE_TTL_MS;
}

function messagesRepresentSameManualOutgoing(a: Partial<Message>, b: Partial<Message>) {
  if (!a || !b) return false;
  if (String(a.id || "") && String(a.id || "") === String(b.id || "")) return true;
  if (messagesShareServerId(a, b)) return true;
  if (a.fromMe !== true || b.fromMe !== true) return false;
  if (a.isFromAgent === true || b.isFromAgent === true) return false;
  if (!messagesAreCloseEnoughForRealtimeDedupe(a, b)) return false;

  const aMediaType = normalizeMessageDedupeMediaType(a);
  const bMediaType = normalizeMessageDedupeMediaType(b);
  if (aMediaType !== bMediaType) return false;

  const aText = normalizeMessageDedupeText(a);
  const bText = normalizeMessageDedupeText(b);
  if (aText && bText && aText === bText) return true;

  return Boolean(aMediaType && (a.mediaUrl || b.mediaUrl));
}
...
function replaceOptimisticMessage(conversationId: string | null, optimisticId: string | undefined, nextMessage?: Message | null) {
  ...
  queryClient.setQueryData<Message[]>(["/api/messages", conversationId], (old = []) => {
    let replaced = false;
    const nextMessages = old
      .filter((message) => (
        message.id === optimisticId ||
        isTemporaryMessageId(message.id) ||
        !messagesRepresentSameManualOutgoing(message, nextMessage)
      ))
      .map((message) => {
        ...
        return {
          ...nextMessage,
          mediaUrl: nextMessage.mediaUrl || message.mediaUrl,
        };
      });
    ...
    return nextMessages.sort((a, b) => getMessageTimeMs(a) - getMessageTimeMs(b));
  });
  return true;
}
...
function optimisticMessageMatchesIncoming(optimistic: any, incoming: any) {
  ...
  return messagesRepresentSameManualOutgoing(optimistic, incoming);
}
...
function readBlobAsDataUrl(blob: Blob) {
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onerror = () => reject(reader.error || new Error("Falha ao ler audio gravado."));
...
function connectionHasStrongDisconnectSignal(connection?: {
  providerStatus?: string | null;
  qrCode?: string | null;
  qr?: boolean | null;
  isConnected?: boolean | null;
  ...
  if (Boolean(connection.qrCode) || connection.qr === true) {
    return true;
  }
  if (
    [
...
function isAudioPlaceholderText(text?: string | null): boolean {
  ...
  if (normalized.startsWith("*")) {
    const speakerMarkerIndex = normalized.indexOf("*: ");
    if (speakerMarkerIndex > 0 && speakerMarkerIndex <= 96) {
      normalized = normalized.slice(speakerMarkerIndex + 3).trim();
    }
  }
  ...
  while (changed && normalized.length > 1) {
    ...
    for (const [left, right] of wrapperPairs) {
      if (normalized.startsWith(left) && normalized.endsWith(right)) {
...
export function ChatArea({
  ...
  const lastOperationalChatConnectionsRef = useRef<
    Map<string, { connection: WhatsappConnection; expiresAt: number }>
  >(new Map());
  const visibleConnections = useMemo(() => {
    const now = Date.now();
    const lastOperational = lastOperationalChatConnectionsRef.current;

    for (const [cachedConnectionId, snapshot] of lastOperational.entries()) {
      if (snapshot.expiresAt <= now) {
        lastOperational.delete(cachedConnectionId);
      }
    }

    const seenIds = new Set<string>();
    const currentRows = allConnections.map((connection) => {
      seenIds.add(connection.id);
      if (connectionLooksConnected(connection)) {
        lastOperational.set(connection.id, {
          connection,
          expiresAt: now + LAST_OPERATIONAL_CHAT_CONNECTION_TTL_MS,
        });
        return connection;
      }

      const previous = lastOperational.get(connection.id);
      if (previous && !connectionHasStrongDisconnectSignal(connection)) {
        return {
          ...connection,
          isConnected: true,
          providerStatus: connection.providerStatus || previous.connection.providerStatus || "connected",
          phoneNumber: connection.phoneNumber || previous.connection.phoneNumber,
          connectionName: connection.connectionName || previous.connection.connectionName,
        };
      }
    ...
    for (const [cachedConnectionId, snapshot] of lastOperational.entries()) {
      if (!seenIds.has(cachedConnectionId)) {
  ...
  const currentConnectionFromList = activeConnectionId
    ? visibleConnections.find((connection) => connection.id === activeConnectionId)
    : null;
  ...
  const conversationStatusIsIndeterminate = Boolean(
    activeConnectionId &&
      !currentConnectionFromList &&
      allConnections.length === 0 &&
      (isConnectionsLoading || isConnectionsFetching) &&
      !connectionHasStrongDisconnectSignal({
  ...
  const isConversationConnectionConnected = currentConnectionFromList
    ...
    : (
        conversationStatusIsIndeterminate ||
  ...
  const connectedAlternativeConnections = useMemo(
    ...
    [activeConnectionId, visibleConnections],
  );
  ...
  const sendAudioMutation = useMutation({
    mutationFn: async ({ audioData, duration, mimeType }: { audioData: string; duration: number; mimeType: string }) => {
      const response = await apiRequest("POST", `/api/conversations/${conversationId}/send-audio`, {
        audioData,
        duration,
        mimeType,
      });
  ...
  const handleSendAudio = useCallback(async (audioBlob: Blob, duration: number, mimeType: string) => {
    ...
    try {
      ...
      sendAudioMutation.mutate({
        audioData,
        duration,
        mimeType: mimeType || audioBlob.type || "audio/ogg",
      });
    } catch (error: any) {
      toast({
        title: "Erro ao preparar audio",
        description: error?.message || "Nao foi possivel enviar o audio gravado.",
        variant: "destructive",
      });
    }
  }, [connectionUnavailableMessage, conversationLockedMessage, isConversationClosed, isConversationConnectionDisconnected, sendAudioMutation, toast]);
                ...
                if (data.messageData) {
                  ...
                  queryClient.setQueryData(
                    ...
                    (old: any) => {
                      ...
                      const incomingId = String(data.messageData.id || "").trim();
                      const incomingMessageId = String(data.messageData.messageId || "").trim();
                      const exists = messages.some((m: any) =>
                        (incomingId && String(m.id || "") === incomingId) ||
                        (incomingMessageId && String(m.messageId || "") === incomingMessageId)
                      );
                      ...
                      const optimisticIndex = messages.findIndex((message: any) =>
                        optimisticMessageMatchesIncoming(message, newMsg)
                      );
                      const equivalentOutgoingIndex = optimisticIndex >= 0
                        ? optimisticIndex
                        : messages.findIndex((message: any) =>
                            !isTemporaryMessageId(message?.id) &&
                            messagesRepresentSameManualOutgoing(message, newMsg)
                          );
                      const replacementIndex = optimisticIndex >= 0 ? optimisticIndex : equivalentOutgoingIndex;
  ...
  return (
    ...
    >
      ...
      {/* Chat Header */}
      <div className="shrink-0 border-b border-card-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/85">
        <div className={cn(
      ...
      <div
        ...
        className={cn(
          "az-chat-surface min-h-0 flex-1 overflow-auto p-3 md:p-4 space-y-3 md:space-y-4"
        )}
      ...
      >
        ...
        {isLoading ? (
          ...
          allMessages
            ...
            >
              <div
                className={`max-w-[85%] md:max-w-md rounded-xl border px-3 py-2 shadow-sm md:px-4 ${
                  message.fromMe
                    ? "ml-auto border-[#b7e3a3] bg-[#dcf8c6] text-slate-900 dark:border-emerald-700/50 dark:bg-emerald-950 dark:text-emerald-50"
                    : "mr-auto border-card-border bg-white text-slate-900 dark:bg-card dark:text-card-foreground"
                }`}
              >
                {message.isFromAgent && (
                ...
                {message.mediaType === "image" && message.mediaUrl ? (
                  ...
                  <div className={`space-y-2 p-3 rounded-lg ${
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => redownloadMediaMutation.mutate(message.id || message.messageId)}
                        disabled={redownloadingMessageId === (message.id || message.messageId)}
                        className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${
                          message.fromMe 
                            ? "bg-emerald-100 hover:bg-emerald-200 dark:bg-emerald-800/60 dark:hover:bg-emerald-800" 
                            : "bg-gray-200 hover:bg-gray-300"
                        } ${redownloadingMessageId === (message.id || message.messageId) ? "animate-pulse" : ""}`}
                      >
                        {redownloadingMessageId === (message.id || message.messageId) ? (
                          <Loader2 className={`h-5 w-5 animate-spin ${message.fromMe ? "text-slate-900 dark:text-emerald-50" : "text-gray-600"}`} />
                        ) : (
                      ...
                      <div className="flex-1">
                        <p className={`text-sm font-medium ${
                          message.fromMe ? "text-slate-900 dark:text-emerald-50" : "text-gray-900"
                        }`}>
                          Imagem {message.fromMe ? "enviada" : "recebida"}
                        </p>
                        <p className={`text-xs ${
                          message.fromMe ? "text-slate-600 dark:text-emerald-100/60" : "text-gray-500"
                        }`}>
                    ...
                    {(message.text || message.mediaCaption) && (
                      <div className={`border-l-2 pl-3 ${
                        message.fromMe ? "border-emerald-200 dark:border-emerald-700/60" : "border-gray-300"
                      }`}>
                        <p className={`text-xs font-medium mb-1 ${
                          message.fromMe ? "text-slate-600 dark:text-emerald-100/70" : "text-gray-500"
                        }`}>
                          {message.mediaCaption ? "📝 Legenda:" : "👁️ Descrição da IA:"}
                        </p>
                        <p className={`text-sm whitespace-pre-wrap break-words italic ${
                          message.fromMe ? "text-slate-800 dark:text-emerald-50/90" : "text-gray-700"
                        }`}>
                ...
                <div className="flex items-center justify-between mt-1 gap-2">
                  <div className="flex items-center gap-1.5">
                    <p
                      className={`text-xs ${
                        message.fromMe ? "text-slate-500 dark:text-emerald-100/70" : "text-muted-foreground"
                      }`}
                    >
                      {format(new Date(message.timestamp), "HH:mm", { locale: ptBR })}
                    </p>
                    {message.fromMe && String(message.status || "").toLowerCase() === "sending" && (
                      <span className="text-[10px] text-slate-500 dark:text-emerald-100/65">Enviando...</span>
                    )}
                    {message.fromMe && String(message.status || "").toLowerCase() === "failed" && (
                      <span className="text-[10px] font-medium text-red-600 dark:text-red-300">Falhou</span>
                    )}
                  </div>
                  {/* Botão encaminhar */}
                  <button
                    ...
                    className={`opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-black/10 ${
                      message.fromMe ? "text-slate-500 hover:text-slate-800 dark:text-emerald-100/70 dark:hover:text-emerald-50" : "text-muted-foreground hover:text-foreground"
                    }`}
      ...
      <div
        className={cn(
          "z-20 border-t border-card-border bg-card p-3 md:p-4",
          isMobile && "border-border/80 bg-card/95 supports-[backdrop-filter]:bg-card/85"
        )}
      >
        {isConversationClosed && (
        ...
        {isRecordingAudio ? (
          ...
          <div className="mx-auto flex max-w-4xl items-end gap-2 rounded-xl border border-input bg-card p-2 shadow-sm focus-within:border-primary/60 focus-within:ring-2 focus-within:ring-primary/10">
            ...
            <Textarea
              ...
              disabled={isComposerDisabled}
              className="min-h-11 max-h-32 flex-1 resize-none border-0 bg-transparent px-2 text-base shadow-none ring-offset-0 focus-visible:ring-0 focus-visible:ring-offset-0"
            ...
            <div className="flex items-center gap-1">
              ...
              {messageText.trim() && (
                <Button
                  size="icon"
                  onClick={handleSend}
                  disabled={!messageText.trim() || isComposerDisabled}
