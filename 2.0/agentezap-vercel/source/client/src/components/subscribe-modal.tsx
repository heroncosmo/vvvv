...
import type { UpgradeProrationQuote } from "@shared/subscriptionProration";
...
function isImplementationSubscriptionPlan(plan: any): boolean {
  return plan?.tipo === "implementacao" || plan?.tipo === "implementacao_mensal";
}

function getPlansCheckoutAmount(subscription: any): number | null {
  const metadata = subscription?.metadata || subscription?.paymentMetadata || {};
  const createdFrom = String(metadata.createdFrom || metadata.created_from || "").trim();
  const hasPlansCheckoutMarker =
    createdFrom === "plans_checkout" ||
    metadata.checkoutAmount !== undefined ||
    metadata.checkout_amount !== undefined ||
    metadata.checkoutAmountAfterCoupon !== undefined ||
    metadata.checkout_amount_after_coupon !== undefined;

  if (!hasPlansCheckoutMarker) {
    return null;
  }

  const amount = parseMoneyValue(
    metadata.checkoutAmountAfterCoupon ??
      metadata.checkout_amount_after_coupon ??
      metadata.checkoutAmount ??
      metadata.checkout_amount,
  );

  return amount > 0 ? amount : null;
}

function getPlansCheckoutRecurringAmount(subscription: any): number | null {
  const metadata = subscription?.metadata || subscription?.paymentMetadata || {};
  const amount = parseMoneyValue(
    metadata.checkoutRecurringAmount ??
      metadata.checkout_recurring_amount,
  );

  return amount > 0 ? amount : null;
}

function getCheckoutProration(subscription: any): UpgradeProrationQuote | null {
  const metadata = subscription?.metadata || subscription?.paymentMetadata || {};
  const proration = metadata.checkoutProration || metadata.checkout_proration;
  return proration && typeof proration === "object" && !Array.isArray(proration)
    ? proration as UpgradeProrationQuote
    : null;
}

function resolveSubscriptionDisplayAmounts(subscription: any, plan: any) {
  const planPrice = parseMoneyValue(plan?.valor ?? plan?.price);
  const rawSetupFee = parseMoneyValue(plan?.valorPrimeiraCobranca);
  const couponPrice = parseMoneyValue(subscription?.couponPrice);
  const checkoutAmount = getPlansCheckoutAmount(subscription);
  const checkoutRecurringAmount = getPlansCheckoutRecurringAmount(subscription);
  const isImplementationPlan = isImplementationSubscriptionPlan(plan);
  const recurringAmount = !isImplementationPlan && checkoutRecurringAmount
    ? checkoutRecurringAmount
    : couponPrice || planPrice;
  const firstChargeAmount = !isImplementationPlan && checkoutAmount
    ? checkoutAmount
    : rawSetupFee;
  const hasDifferentFirstCharge = firstChargeAmount > 0 && Math.abs(firstChargeAmount - recurringAmount) >= 0.01;
  const totalInitial = hasDifferentFirstCharge ? firstChargeAmount : recurringAmount;

  return {
    monthlyPrice: recurringAmount,
    setupFee: firstChargeAmount,
...
export function SubscribeModal({ open, onOpenChange, subscriptionId, onSuccess, onRequestClose }: SubscribeModalProps) {
  ...
  useEffect(() => {
    const fetchInstallments = async () => {
      ...
      // Calcular o valor a cobrar usando o mesmo valor efetivo do checkout.
      const { totalInitial: amountToCharge } = resolveSubscriptionDisplayAmounts(subscription, plan);
  ...
  const handleReceiptUpload = async () => {
    ...
    try {
      ...
      const { totalInitial: fallbackAmount } = resolveSubscriptionDisplayAmounts(subscription, plan);
  ...
  const {
    ...
    hasDifferentFirstCharge,
    totalInitial,
  } = resolveSubscriptionDisplayAmounts(subscription, plan);
  ...
  const checkoutProration = getCheckoutProration(subscription);
  const showUpgradeProration = Boolean(
    checkoutProration?.applied &&
      Number(checkoutProration.creditAmount || 0) > 0 &&
      Number(checkoutProration.targetAmount || 0) > Number(checkoutProration.payableAmount || 0),
  );
  const showIntroductoryFirstCharge = hasDifferentFirstCharge && !isImplementationPlan && !showUpgradeProration;
            ...
            <div className="w-full bg-gradient-to-br from-[#1a1a1a] to-[#2d2d2d] p-5 text-white md:w-[42%] md:p-10">
              <div className="mb-3 md:mb-6">
                <h2 className="text-xl font-bold mb-1">
                  {showUpgradeProration ? "Upgrade do plano" : showSetupBreakdown ? "Implementação + Plano" : "Volte aos negócios"}
                </h2>
              ...
              <div className="hidden space-y-4 md:block">
                ...
                {showUpgradeProration && checkoutProration && (
                  <div className="flex items-start gap-3">
                    <Check className="w-4 h-4 text-yellow-400 mt-1 flex-shrink-0" />
                    <div>
                      <p className="font-medium text-sm">
                        Crédito do plano atual: R$ {Number(checkoutProration.creditAmount || 0).toFixed(2).replace(".", ",")}
                      </p>
                      <p className="text-xs text-gray-400">
                        Hoje você paga só a diferença. Depois fica R$ {monthlyPrice.toFixed(2).replace(".", ",")}/{periodLabel}.
                      </p>
                    </div>
                  </div>
                )}
