...
function LeonaFlow2Inner() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      const savedFlow = data?.flow || data;
      if (savedFlow) {
        setFlowName(savedFlow.name || "Fluxo 2.0");
        setIsActive(savedFlow.isActive === true);
        setIsArchived(savedFlow.isArchived === true);
        setSelectedNodeId(savedFlow.selectedNodeId || "start");
        if (savedFlow.definition) {
          setDefinition(normalizeDefinition(savedFlow.definition));
        }
      }
    ...
    <div className="flex h-full min-h-[820px] flex-col overflow-hidden bg-[#f7f8fb]" data-subscription-gate-ignore="true">
      <div className="flex flex-col gap-4 border-b border-slate-200 bg-white px-4 py-4 lg:flex-row lg:items-center lg:justify-between lg:px-5">
        <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="min-w-0 sm:pl-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {isArchived ? (
                <Badge className="bg-slate-100 text-slate-700">Arquivado</Badge>
              ) : isActive ? (
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3 lg:flex lg:flex-wrap lg:items-center lg:gap-3">
          <Button
            type="button"
            variant={isActive && !isArchived ? "outline" : "default"}
            className={cn("w-full lg:w-auto", isActive && !isArchived ? "border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100 hover:text-amber-800" : "bg-emerald-600 hover:bg-emerald-500")}
            disabled={saveMutation.isPending}
            onClick={() => {
              const nextActive = !(isActive && !isArchived);
              setIsActive(nextActive);
              setIsArchived(false);
              saveMutation.mutate({ isActive: nextActive, isArchived: false });
            }}
          >
            {isActive && !isArchived ? <X className="mr-2 h-4 w-4" /> : <Play className="mr-2 h-4 w-4" />}
            {isActive && !isArchived ? "Desativar" : "Ativar"}
          </Button>
        ...
        <main className="relative min-h-[680px] flex-1 bg-white">
          ...
          >
            ...
            <Panel position="top-left" className="ml-2 mt-3 flex max-w-[calc(100vw-2rem)] gap-2 overflow-x-auto rounded-lg pr-3 sm:ml-4 sm:mt-4">
              ...
              <Button type="button" variant="outline" className="shrink-0 bg-white shadow-sm" onClick={() => reactFlow.fitView({ padding: 0.65, duration: 300, maxZoom: 0.85 })} aria-label="Centralizar fluxo inteiro">
                <MousePointer2 className="h-4 w-4 sm:mr-2" />
                <span className="hidden sm:inline">Centralizar</span>
              </Button>
              <Button type="button" variant="outline" className="shrink-0 bg-white shadow-sm" onClick={() => testMutation.mutate()} disabled={testMutation.isPending} aria-label="Testar fluxo">
                {testMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin sm:mr-2" /> : <Play className="h-4 w-4 sm:mr-2" />}
