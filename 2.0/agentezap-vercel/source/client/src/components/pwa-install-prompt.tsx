...
function getDismissedUntil(): number {
  ...
  const rawValue = window.localStorage.getItem(DISMISSED_UNTIL_KEY);
...
export function PwaInstallPrompt() {
  ...
  if (!isAuthenticated || isLoading || !promptEvent || isInstalled || isDismissed) {
    return null;
  }
  ...
  const handleDismiss = () => {
    ...
    if (typeof window !== "undefined") {
      window.localStorage.setItem(DISMISSED_UNTIL_KEY, String(nextDismissedUntil));
    }
