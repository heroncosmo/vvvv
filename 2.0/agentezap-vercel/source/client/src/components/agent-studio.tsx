...
function createSimulatorSessionId() {
...
export function AgentStudio({ initialPrompt, onSave, onNavigateToConnect, isNew = false }: AgentStudioProps) {
  ...
  // Estado do prompt
  const [currentPrompt, setCurrentPrompt] = useState(repairMojibakeText(initialPrompt));
  ...
  const simulatorSessionIdRef = useRef(createSimulatorSessionId());
  ...
  useEffect(() => {
    const normalizedPrompt = repairMojibakeText(initialPrompt);
  ...
  const handleUndo = useCallback(() => {
    if (canUndo) {
      ...
      setCurrentPrompt(repairMojibakeText(previousEntry.prompt));
  ...
  const handleRedo = useCallback(() => {
    if (canRedo) {
      ...
      setCurrentPrompt(repairMojibakeText(nextEntry.prompt));
      setHistoryIndex(newIndex);
      setHasChanges(true);
  ...
  const restoreFromHistory = useCallback((index: number) => {
    ...
    if (entry) {
      setCurrentPrompt(repairMojibakeText(entry.prompt));
  ...
  const handleEditPrompt = async () => {
    ...
    try {
      ...
      if (data.success && data.newPrompt && data.newPrompt !== currentPrompt) {
        ...
        const normalizedPrompt = repairMojibakeText(data.newPrompt);
        addToHistory(normalizedPrompt, currentInstruction, data.summary || "Edição aplicada");
  ...
  const handleSimulate = async () => {
    ...
    try {
      ...
      const agentMessages: SimulatorMessage[] = [];

      if (splitResponses.length > 0) {
        for (const part of splitResponses) {
          const message = String(part || "").trim();
          if (!message) continue;
          agentMessages.push({
      ...
      } else if (!hasTextActions && responseText.trim()) {
        agentMessages.push({
      ...
      for (const action of mediaActions) {
        if (action?.type === "send_text" && String(action.text || "").trim()) {
          agentMessages.push({
            id: `sim-agent-${Date.now()}-${Math.random()}`,
            role: "agent",
            message: String(action.text).trim(),
            time: agentTime,
          });
        }
      }
      ...
      if (agentMessages.length > 0) {
        setSimulatorMessages(prev => [...prev, ...agentMessages]);
      }
  ...
  // ============ SALVAR ============
  const handleSave = () => {
    onSave(repairMojibakeText(currentPrompt));
    setHasChanges(false);
    toast({
      title: "Salvo!",
          ...
          {activeView === "chat" ? (
            ...
            <div className="flex-1 flex flex-col p-4">
              <Textarea
                value={currentPrompt}
                onChange={(e) => {
          setCurrentPrompt(repairMojibakeText(e.target.value));
                  setHasChanges(true);
                }}
        ...
        <div className={cn(
          ...
          <div className="bg-[#075E54] dark:bg-zinc-800 text-white px-4 py-3 flex items-center gap-3 flex-shrink-0">
            ...
            <div className="flex-1">
              <p className="font-semibold text-sm">Testar Agente</p>
            ...
            <Button
              ...
              onClick={() => {
                ...
                setSimulatorInput("");
                setIsSimulating(false);
                simulatorSessionIdRef.current = createSimulatorSessionId();
              }}
