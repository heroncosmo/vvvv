...
export function getConversationFilterNumber(
  conversation: Pick<Conversation, "contactNumber" | "remoteJid"> | null | undefined,
): string {
  const directNumber = String(conversation?.contactNumber || "").trim();
  if (directNumber) {
    return directNumber;
  }

  return String(conversation?.remoteJid || "")
    .split("@")[0]
    ?.split(":")[0]
    ?.trim() || "";
}
