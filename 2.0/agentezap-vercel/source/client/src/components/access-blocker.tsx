...
export function AccessBlocker({ children }: { children: React.ReactNode }) {
  ...
  if (accessStatus?.shouldBlock) {
    ...
    const isResellerNotice = (accessStatus.blockReason === 'reseller_client_expired' || accessStatus.blockReason === 'reseller_blocked') && accessStatus.resellerInfo;
...
export function SubscriptionExpiringBanner() {
  ...
  const isPublicRoute = [
    ...
    '/revenda',
    '/white-label',
  ].some(route => location.startsWith(route) || location === '/');
