...
export async function getSimulatorChannelGuardResult(
  ...
  if (connection?.isConnected && agentConfig && agentConfig.isActive === false) {
    return {
      ...
      blockReason:
        "O agente global esta desligado em Meu Agente IA. Ative-o para que o WhatsApp real responda.",
  ...
  if (connection?.isConnected && businessAgentConfig && businessAgentConfig.isActive === false) {
    return {
      ...
      blockReason:
        "A IA operacional do agente esta desligada. Reative em Meu Agente IA antes de confiar no simulador.",
      connectionId: connection.id,
    };
  }
