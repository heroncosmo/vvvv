...
import { registerVpsCronRoutes } from "./routes_vps_crons";
import { repairMojibakeDeep, repairMojibakeText } from "@shared/mojibake";
import { describeAppRuntimeProfile, isWebOnlyAppRuntime } from "./runtimeProfile";
import {
  delegateToVercelHttpHandler,
  registerVercelHttpFallbackRoutes,
  registerVercelHttpParityRoutes,
...
export interface CreateHttpApiAppOptions {
  mountHealthRoutes?: boolean;
}
...
export async function createHttpApiApp(
  options: CreateHttpApiAppOptions = {},
): Promise<{ app: Express; server: Server }> {
  const { mountHealthRoutes: shouldMountHealthRoutes = true } = options;
  const app = express();
  ...
  if (shouldMountHealthRoutes) {
    mountHealthRoutes(app);
  }

  registerVpsCronRoutes(app);
  registerVercelHttpParityRoutes(app);

  const server = isWebOnlyAppRuntime()
    ? await registerWebOnlyRoutes(app)
    : await registerRoutes(app);

  registerVercelHttpFallbackRoutes(app);
