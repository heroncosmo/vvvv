export function stripDanglingTrailingListLines(message: string): string {
  ...
  while (lines.length > 0) {
    ...
    if (/^\d{1,3}[.)]?$/.test(lastLine) || /^[-*\u2022]+$/.test(lastLine)) {
      lines.pop();
      continue;
    }
...
export function normalizeInlineNumberedListBreaks(message: string): string {
...
function stripDanglingTrailingHeadingLine(message: string): string {
  const lines = message.split("\n");

  while (lines.length > 1) {
    const lastLine = String(lines[lines.length - 1] || "").trim();
    if (!/^[^\n]{1,80}:\s*$/.test(lastLine)) {
      break;
    }
...
export function sanitizeAgentResponseTail(message: string): string {
  ...
  sanitized = stripDanglingTrailingHeadingLine(sanitized);
