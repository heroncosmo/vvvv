import { db } from "./db";
import { adminConversations, followupLogs, adminMessages, admins } from "@shared/schema";
...
export class FollowUpService {
  ...
  private async analyzeWithAI(conversation: typeof adminConversations.$inferSelect): Promise<{
    ...
    const admin = await db.query.admins.findFirst({
      where: eq(admins.id, conversation.adminId),
      columns: { email: true },
    });
    const ownerEmail = String(admin?.email || "").trim().toLowerCase();
    const scope: AgenteZapLiveCliScope =
    ...
    const prompt = `
      ...
      A decisao deve sair apenas pelo contrato estruturado do Codex descrito abaixo.
      Nao retorne JSON textual solto, texto publico, frase de follow-up nem confirmacao local.
    `;

    const codexDecisionPrompt = [
      prompt,
      "Contrato Codex vivo para esta etapa:",
      "- Use o schema padrao agentezap_live_cli_plan_v1.",
      "- Nao escreva mensagem publica em customerFacingMessages; esta etapa decide apenas o side effect de follow-up.",
      "- Para SEND, retorne action schedule_followup com arguments.reason iniciando por ACTION=send; e inclua o contexto recomendado depois do ponto e virgula.",
      "- Para WAIT, retorne action no_send com arguments.reason iniciando por ACTION=wait;.",
      "- Para ABORT, retorne action no_send com arguments.reason iniciando por ACTION=abort;.",
      "- Se faltar contexto, use ACTION=wait; nao invente envio nem texto publico.",
    ].join("\n");
    ...
    const buildTechnicalFallback = (reason: string) => {
      ...
      if (!lastMessage || lastMessage.fromMe || !lastCustomerMessage) {
        return {
          ...
          reason,
          context: "Falha operacional na decisao Codex do follow-up; aguarde nova rodada ou revisao manual.",
        };
      }
    ...
    const parseCodexDecision = (result: Awaited<ReturnType<typeof runAgenteZapLiveCliRuntime>>) => {
      const actions = result.plan?.actions || [];
      const scheduleAction = actions.find((action) => action.type === "schedule_followup");
      const noSendAction = actions.find((action) => action.type === "no_send");
      const reasonText = String(
        scheduleAction?.arguments?.reason ||
        noSendAction?.arguments?.reason ||
        result.plan?.evidence?.uncertainty?.join("; ") ||
        "",
      ).trim();
      const normalizedReason = reasonText.toLowerCase();
      const context = reasonText.replace(/^ACTION=(send|wait|abort);?/i, "").trim() || undefined;

      if (scheduleAction && normalizedReason.startsWith("action=send")) {
        return {
          action: "send" as const,
          reason: reasonText || "Codex decidiu enviar follow-up",
          context,
        };
      }

      if (noSendAction && normalizedReason.startsWith("action=abort")) {
        return {
          action: "abort" as const,
          reason: reasonText || "Codex decidiu abortar follow-up",
          context,
        };
      }

      if (noSendAction && normalizedReason.startsWith("action=wait")) {
        return {
          action: "wait" as const,
          reason: reasonText || "Codex decidiu aguardar follow-up",
          context,
        };
      }

      return buildTechnicalFallback("Fallback tecnico: Codex nao retornou action de follow-up valida");
    };
    ...
    try {
      const result = await runAgenteZapLiveCliRuntime({
        scope,
        ownerEmail,
        conversationId: `admin-followup:${conversation.id}`,
        contactPhone: conversation.contactNumber,
        currentMessage: codexDecisionPrompt,
        messages: lastMessages,
        contextArtifacts: {
          followUpDecisionContract: {
            allowedOutcomes: ["send", "wait", "abort"],
            sendAction: "schedule_followup",
            noSendAction: "no_send",
            reasonPrefixes: ["ACTION=send;", "ACTION=wait;", "ACTION=abort;"],
            publicMessageAuthor: "adminAgentService.generateFollowUpResponse via Codex after this decision",
          },
          conversation: {
            id: conversation.id,
            ...
            followupStage: conversation.followupStage,
      ...
      return parseCodexDecision(result);
    } catch (e) {
      console.error("Erro na análise de IA:", e);
      return buildTechnicalFallback("Fallback tecnico: erro na analise Codex de follow-up");
    }
  }
  ...
  /**
   * Agenda follow-up com delay customizado (solicitado via contrato estruturado)
