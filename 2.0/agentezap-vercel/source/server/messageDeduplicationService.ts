...
const CONFIG = {
  ...
  AUTOMATED_REPEAT_LOOKBACK_HOURS: 12,       // 12 horas - bloqueia automações que repetem exatamente o mesmo texto
...
class MessageDeduplicationService {
  ...
  private isAutomatedMessage(messageType: MessageType, source: MessageSource): boolean {
    return (
  ...
  private buildAutomatedRepeatFingerprint(content: string): string {
    const fingerprint = buildOutgoingMessageFingerprint(content);
    if (!fingerprint) return fingerprint;

    const hasPaymentOptions =
      fingerprint.includes("opcoes disponiveis") &&
      fingerprint.includes("pix") &&
      fingerprint.includes("link de pagamento") &&
      fingerprint.includes("pagamento em dinheiro") &&
      fingerprint.includes("cartao");
    if (hasPaymentOptions) {
      return "semantic:payment-options";
    }

    const hasCartFinalizationPrompt =
      fingerprint.includes("total geral") &&
      fingerprint.includes("posso seguir") &&
      fingerprint.includes("finalizacao");
  ...
  private generateAutomationRepeatHash(content: string): string {
  ...
  async canSendMessage(params: {
    ...
    const { userId, contactNumber, content, conversationId, messageType = 'unknown', source = 'unknown', latestCustomerMessageAt } = params;
    ...
    const automationRepeatHash = isAutomated
      ? this.generateAutomationRepeatHash(content)
      : contentHash;
    ...
    if (isAutomated) {
      ...
      const cachedAutomatedRepeat = this.outgoingCache.get(automatedRepeatCacheKey);
      const latestCustomerMessageAtMs = parseTurnTimestampMs(latestCustomerMessageAt);
      if (
        cachedAutomatedRepeat &&
        (!latestCustomerMessageAtMs || latestCustomerMessageAtMs <= cachedAutomatedRepeat.createdAt)
      ) {
        console.log(`ðŸ›¡ï¸ [ANTI-REENVIO] âŒ BLOQUEADO (automacao repetida em cache): ${contactNumber} - "${content.substring(0, 30)}..."`);
        console.log(`   ðŸ“ Source: ${source} | Type: ${messageType}`);
        this.stats.outgoingBlocked++;
        return false;
      }
    }
    ...
    try {
      ...
      if (isAutomated) {
        const lookbackDate = new Date(
          Date.now() - CONFIG.AUTOMATED_REPEAT_LOOKBACK_HOURS * 60 * 60 * 1000,
        ).toISOString();

        const { data: repeatedAutomation, error: repeatedAutomationError } = await this.supabase
          .from('message_deduplication')
          .select('id, created_at')
          .eq('user_id', userId)
          .eq('contact_number', contactNumber)
          .eq('content_hash', automationRepeatHash)
          .in('message_type', ['ai_response', 'followup'])
          .gte('created_at', lookbackDate)
          .order('created_at', { ascending: false })
          .limit(1);

        if (repeatedAutomation && repeatedAutomation.length > 0) {
          const repeatedCreatedAtMs = parseTurnTimestampMs(repeatedAutomation[0]?.created_at);
          const latestCustomerMessageAtMs = parseTurnTimestampMs(latestCustomerMessageAt);
          if (latestCustomerMessageAtMs && repeatedCreatedAtMs && latestCustomerMessageAtMs > repeatedCreatedAtMs) {
            console.log(`[ANTI-REENVIO] Automacao repetida no banco liberada por novo turno do cliente: ${contactNumber}`);
          } else {
          console.log(`ðŸ›¡ï¸ [ANTI-REENVIO] âŒ BLOQUEADO (automacao repetida no banco): ${contactNumber} - "${content.substring(0, 30)}..."`);
          console.log(`   ðŸ“ Source: ${source} | Type: ${messageType}`);
          this.stats.outgoingBlocked++;
        ...
        if (repeatedAutomationError) {
          console.error('ðŸ›¡ï¸ [ANTI-REENVIO] Erro ao verificar automacao repetida:', repeatedAutomationError);
          this.stats.dbErrors++;
        }
      }
    ...
    await this.registerOutgoingMessage({
      ...
      messageType,
      source,
      contentHashOverride: isAutomated ? automationRepeatHash : undefined,
    });
  ...
  async registerOutgoingMessage(params: {
    ...
    if (isAutomated) {
      this.persistOutgoingMessage({
        ...
        console.error('ðŸ›¡ï¸ [ANTI-REENVIO] Erro ao persistir chave de automacao:', err);
        this.stats.dbErrors++;
      });
    }
  }
...
export function clearOutgoingDeduplicationCacheForConversation(params: {
  conversationId: string;
  userId?: string;
  contactNumber?: string;
}): number {
