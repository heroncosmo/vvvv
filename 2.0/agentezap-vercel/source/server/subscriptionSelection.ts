...
export interface SubscriptionSelectionCandidate {
  ...
  receiptCreatedAt?: string | Date | null;
...
function resolveCoverageEnd(candidate: SubscriptionSelectionCandidate): Date | null {
  return resolveManualReceiptCoverageDate(candidate.nextPaymentDate, candidate.dataFim);
}
...
export function resolveApprovedReceiptActivationWindow(
  ...
  const receiptAnchor = toValidDate(candidate.receiptCreatedAt) || toValidDate(candidate.approvedReceiptAt) || now;
  ...
  const dataFim = resolveManualReceiptCoverageDate(candidate.nextPaymentDate, candidate.dataFim) || fallbackWindow.dataFim;
  const nextPaymentDate =
    resolveManualReceiptCoverageDate(candidate.nextPaymentDate, candidate.dataFim) || fallbackWindow.nextPaymentDate;
