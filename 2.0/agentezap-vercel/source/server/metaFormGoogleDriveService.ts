...
function buildGoogleOAuthAppConfig(params: {
  ...
  moduleApiKey?: string | null;
  moduleClientId?: string | null;
  moduleClientSecret?: string | null;
  legacyApiKey?: string | null;
  legacyClientId?: string | null;
  legacyClientSecret?: string | null;
}): GoogleOAuthAppConfig | null {
  const moduleClientId = normalizeText(params.moduleClientId);
  const moduleClientSecret = normalizeText(params.moduleClientSecret);
  if (moduleClientId && moduleClientSecret) {
    return {
      apiKey: normalizeText(params.moduleApiKey),
      clientId: moduleClientId,
      clientSecret: moduleClientSecret,
      source: "env",
    };
  }
...
function buildRedirectUri(origin: string): string {
  const explicitRedirect = normalizeText(
    process.env.META_FORM_GOOGLE_REDIRECT_URI || process.env.GOOGLE_META_FORM_REDIRECT_URI,
  );
  if (explicitRedirect) {
    return explicitRedirect;
  }
  const normalizedOrigin = normalizeText(
...
async function getGoogleOAuthAppConfig(): Promise<GoogleOAuthAppConfig | null> {
  const envConfig = buildGoogleOAuthAppConfig({
    moduleApiKey: process.env.META_FORM_GOOGLE_API_KEY || process.env.GOOGLE_META_FORM_API_KEY,
    moduleClientId: process.env.META_FORM_GOOGLE_CLIENT_ID || process.env.GOOGLE_META_FORM_CLIENT_ID,
    moduleClientSecret: process.env.META_FORM_GOOGLE_CLIENT_SECRET || process.env.GOOGLE_META_FORM_CLIENT_SECRET,
