 ...
 *
 * USO:
 *   import { clearGraphState } from "./adminAgentGraphPOC";
 *   clearGraphState(phone);
 *
 * INTEGRAÇÃO:
...
export type GraphDeliveryValidationResult = Record<string, unknown>;
export type GraphAntiPatternAlert = Record<string, unknown>;
...
export interface GraphPipelineResult {
  ...
  sanitizeResult: GraphSanitizeResult;

  /** Alertas de anti-padrões */
  alerts: GraphAntiPatternAlert[];

  /** Validation result (se houve delivery) */
  deliveryValidation?: GraphDeliveryValidationResult;
...
export async function processAdminMessageGraph(
  ...
  const state = getOrCreateGraphState(cleanPhone, contactName);
  void messageText;
  void mediaType;
  void mediaUrl;
  return buildNoSendResult(state, startTime);
}
// ============================================================================
// HELPERS
// ============================================================================

function buildNoSendResult(state: AdminGraphState, startTime: number): GraphPipelineResult {
  ...
  const emptyDecision: PolicyDecision = {
    action: "noop",
    reason: "admin_graph_poc_disabled_until_codex_structured_contract",
    shouldAudit: false,
  };
