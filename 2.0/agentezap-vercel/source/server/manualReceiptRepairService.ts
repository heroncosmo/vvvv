...
function resolveActivationWindow(candidate: ApprovedManualReceiptCandidate, now: Date) {
  const existingCoverageEnd = resolveManualReceiptCoverageDate(candidate.nextPaymentDate, candidate.dataFim);
  const activationAnchor = existingCoverageEnd || toValidDate(candidate.receiptCreatedAt) || toValidDate(candidate.receiptApprovedAt) || now;
  ...
  return {
    ...
    dataFim: existingCoverageEnd || fallback.dataFim,
    nextPaymentDate: existingCoverageEnd || fallback.nextPaymentDate,
  };
}
...
export async function repairApprovedManualReceipts(
  ...
  const rows = await db
    .select({
      ...
      receiptCreatedAt: paymentReceipts.createdAt,
  ...
  for (const row of rows as ApprovedManualReceiptCandidate[]) {
    try {
      ...
      if (existingHistory) {
        ...
        const paymentDate = toValidDate(row.receiptCreatedAt) || toValidDate(row.receiptApprovedAt) || now;
