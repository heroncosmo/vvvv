...
test("normalizeAdminSetupPlan aplica defaults coerentes e corrige tipos invÃ¡lidos", () => {
...
test("mapAdminSetupStatusToCustomerReply nao escreve fala publica local", () => {
  ...
  assert.equal(openReply, "");
  assert.equal(executingReply, "");
  assert.equal(createdReply, "");
});
