...
test("esconde conexao removida pelo usuario sem confundir com simulador", () => {
  const connection = {
    phoneNumber: "5517991956944",
    provider: "baileys",
    connectionMethod: "qr",
    connectionType: "removed",
    providerStatus: "removed",
    isConnected: false,
    connectionName: "Conexao 3",
    providerConfig: { removedFromApp: true, removalMode: "preserve_history" },
  };

  assert.equal(isInternalOnlySimulatorConnection(connection), false);
  assert.equal(isUserRemovedConnection(connection), true);
  assert.equal(isAppVisibleWhatsappConnection(connection), false);
});

test("esconde conexao removida mesmo quando a consulta retorna campos snake_case", () => {
  const connection = {
    phone_number: "556599792300",
    provider: "baileys",
    connection_method: "qr",
    connection_type: "removed",
    provider_status: "removed",
    is_connected: true,
    connection_name: "Conexao antiga",
    provider_config: { removedByUser: true, removalMode: "preserve_history" },
  } as any;
