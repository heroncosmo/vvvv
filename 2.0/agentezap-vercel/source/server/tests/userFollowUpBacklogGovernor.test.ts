...
test("prioriza conversas mais recentes antes das antigas no backlog do mesmo usuário", () => {
  ...
  assert.deepEqual(ordered.map((entry) => entry.conversationId), ["recente", "meio", "antiga"]);
});

test("o governor processa primeiro as conversas mais recentes quando o usuário tem backlog grande", () => {
  const decisions = buildUserFollowUpBacklogDecisions(
    [
      {
  ...
  assert.deepEqual(
    ...
    [
      "recente-2:process_now",
      "recente-1:process_now",
      "antiga-3:delay",
      "antiga-2:delay",
      "antiga-1:delay",
    ],
...
test("mantém a prioridade temporal mesmo quando o conversationId em ordem lexicográfica sugeriria o contrário", () => {
  ...
  assert.deepEqual(
    decisions.map((decision) => `${decision.conversationId}:${decision.action}`),
    [
      "zzz-conversa-recente:process_now",
      "mmm-conversa-intermediaria:process_now",
      "aaa-conversa-antiga:delay",
    ],
  );
});
