  ...
  app.post("/api/chatbot/generate-flow", isAuthenticated, async (req: any, res: Response) => {
    try {
      ...
      return res.status(503).json({
        error: "Geracao de fluxo por IA indisponivel",
        code: "codex_contract_required"
      });
...
9. Responda APENAS com o JSON válido conforme especificado`;
      ...
      // Codex-only: fluxo/config gerados por IA precisam vir do contrato Codex vivo.
      let aiResponse: string | null = null;
      let usedFallback = false;
      let attemptNumber = 0;

      // ============================================================
      // ESTRATEGIA LEGADA DESATIVADA - NUNCA USAR PROVIDER DIRETO OU TEMPLATE LOCAL
      // ============================================================
      const tryLLMCall = async (
        ...
        void usrPrompt;
        void timeoutMs;
        console.warn(`[FLOW_GENERATOR] Legacy direct LLM disabled for attempt ${attempt}; Codex contract required`);
        return null;
      };
...
Personalize os textos para o negócio solicitado. Responda APENAS o JSON.`;
      ...
      if (!aiResponse || aiResponse.trim() === '' || aiResponse === '{}') {
        ...
        return res.status(503).json({
          error: "Geracao de fluxo indisponivel",
          code: "codex_contract_required"
        });
      }
...
RESPONDA APENAS COM O JSON, NADA MAIS.`;
          ...
          try {
            void configPrompt;
            const configContent = '';
            throw new Error('flow_config_generation_requires_codex_contract');
  ...
  app.post("/api/chatbot/edit-flow-chat", isAuthenticated, async (req: any, res: Response) => {
    try {
      ...
      if (!currentNodes || currentNodes.length === 0) {
        return res.status(400).json({ error: "Fluxo atual obrigatorio" });
      }

      return res.status(503).json({
        error: "Edicao de fluxo por IA indisponivel",
        code: "codex_contract_required"
      });
...
- Retorne APENAS JSON válido`;
      ...
      void editUserPrompt;
      throw new Error('flow_edit_requires_codex_contract');

      const aiResponse: string | null = String('');
