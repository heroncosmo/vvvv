...
export function shouldScheduleFollowUpForTrackedSharedAutomaticSource(source: string): boolean {
  const normalizedSource = String(source || "").trim().toLowerCase();
  return !(
    normalizedSource === "customer_system_followup" ||
    normalizedSource === "user_follow_up" ||
    normalizedSource === "userfollowupservice" ||
    normalizedSource === "followup" ||
    normalizedSource === "admin_followup_text"
  );
}
...
export async function persistTrackedSharedAutomaticOutgoingMessage(
  ...
  const savedMessage = await params.createMessage({
    ...
    mediaUrl: params.mediaUrl || params.trackedMessage.mediaUrl || null,
  ...
  if (shouldScheduleFollowUpForTrackedSharedAutomaticSource(params.trackedMessage.source)) {
    try {
