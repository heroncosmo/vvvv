 ...
 * ║  2. Delay REALISTA de 10-20 segundos entre mensagens                        ║
...
import { AUTOMATION_PAUSE_BLOCKED_MESSAGE_ID } from './conversationAutoPauseGuard';
...
interface QueuedMessage {
  ...
  channelSpacingRequired?: boolean;
  channelSpacingDelayMs?: number;
}
...
const INTERACTIVE_AI_CONTENTION_MIN_DELAY_MS = 10000;
const INTERACTIVE_AI_CONTENTION_MAX_DELAY_MS = 18000;

function isInteractiveAiReply(message: QueuedMessage): boolean {
  return message.priority === 'high'
    && message.options?.isFromAgent === true
    && Boolean(message.options?.conversationId);
}

function randomInteractiveContentionDelayMs(): number {
  return INTERACTIVE_AI_CONTENTION_MIN_DELAY_MS +
    Math.random() * (INTERACTIVE_AI_CONTENTION_MAX_DELAY_MS - INTERACTIVE_AI_CONTENTION_MIN_DELAY_MS);
}

export function shouldInterruptPendingReplyYield(
  queue: ReadonlyArray<{ priority: QueuePriority }>,
  yieldedMessage: { priority: QueuePriority },
): boolean {
  return queue.some((queued) => queued !== yieldedMessage && queued.priority !== 'low');
}
...
class MessageQueueService {
  ...
  private directExecutionChains = new Map<string, Promise<void>>();
  ...
  async enqueue(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (
        isInteractiveAiReply(queuedMessage) &&
        (
          state.isProcessing ||
          state.directExecutionPending > 0 ||
          state.queue.some((queued) => isInteractiveAiReply(queued))
        )
      ) {
        queuedMessage.channelSpacingRequired = true;
        queuedMessage.channelSpacingDelayMs = randomInteractiveContentionDelayMs();
      }
  ...
  private insertByPriority(queue: QueuedMessage[], message: QueuedMessage): void {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      const shouldPreemptSamePriority =
        queuedPriority === msgPriority &&
        message.options?.skipDelay === true &&
        queue[i].options?.skipDelay !== true;
      if (queuedPriority > msgPriority || shouldPreemptSamePriority) {
        insertIndex = i;
        break;
      }
    }
   ...
   * v5.1 SIMPLIFICADO: Delay 10-20s + pausa 1 min após 10 msgs
   */
  private async processQueue(userId: string): Promise<void> {
    ...
    while (state.queue.length > 0) {
      if (state.directExecutionPending > 0) {
        console.log(
          `🛡️ [ANTI-BLOCK v5.0] Cedendo fila comum para envio direto prioritario: ${userId.substring(0, 8)}...`,
        );
        break;
      }
      ...
      try {
        const skipArtificialDelay = message.options?.skipDelay === true;
        // Verificar se pode enviar (pausa de lote?)
        const canSendCheck = skipArtificialDelay
          ? { canSend: true, waitMs: 0, reason: "manual_fast_lane" }
          : antiBanProtectionService.canSendMessage(userId);
        if (!canSendCheck.canSend) {
          ...
          // Aguardar
          await this.sleepUntilManualFastLane(state, canSendCheck.waitMs);
          continue;
        }
        
        // Calcular delay (usa o serviço anti-ban)
        if (!skipArtificialDelay) {
          await antiBanProtectionService.prepareAdaptiveDelayPolicy(userId);
        }
        let delay: number;
        if (skipArtificialDelay) {
          delay = 0;
        } else if (isInteractiveAiReply(message)) {
          delay = message.channelSpacingRequired
            ? message.channelSpacingDelayMs || randomInteractiveContentionDelayMs()
            : 0;
        } else {
          delay = antiBanProtectionService.calculateDelay(userId, contactNumber);
        }
        ...
        if (remainingDelay > 0) {
          const reason = isInteractiveAiReply(message) && message.channelSpacingRequired
            ? "disputa simultanea no mesmo canal"
            : "anti-ban";
          console.log(`🛡️ [ANTI-BLOCK v5.0] ⏳ Aguardando ${(remainingDelay/1000).toFixed(1)}s antes de enviar (${reason})...`);
          if (await this.sleepUntilManualFastLane(state, remainingDelay)) {
            this.insertByPriority(state.queue, message);
            continue;
          }
        }

        const targetConversationId = message.options?.conversationId;

        if (message.priority === 'low') {
          const pendingCount = targetConversationId
            ? (await isConversationPendingPriority(targetConversationId) ? 1 : 0)
            : await countPendingPriorityConversations(userId);
          if (pendingCount > 0) {
            const waitMs = resolvePendingPriorityWaitMs(pendingCount);
            const pendingDescription = targetConversationId
              ? "conversa alvo aguardando resposta"
              : `${pendingCount} conversa(s) aguardando resposta`;
            console.log(
              `[ANTI-BLOCK v5.0] Cedendo canal para ${pendingDescription} por ${Math.ceil(waitMs / 1000)}s`,
            );
            state.queue.unshift(message);
            ...
            while (Date.now() < waitUntil) {
              if (state.directExecutionPending > 0) {
                console.log(
                  `🛡️ [ANTI-BLOCK v5.0] Envio direto prioritario entrou na fila; liberando canal apos envio atual`,
                );
                break;
              }
              if (shouldInterruptPendingReplyYield(state.queue, message)) {
                console.log(
                  `[ANTI-BLOCK v5.0] Resposta prioritaria entrou durante espera de baixa prioridade; retomando fila agora`,
                );
                break;
              }
              await this.sleep(Math.min(1000, waitUntil - Date.now()));
            }
            continue;
          }
        }

        // Enviar mensagem com exclusão mútua global do canal
        const result = await channelDispatchLock.runExclusive(
          userId,
          `legacy-queue ${message.priority}`,
          async () => this.sendMessage(userId, message),
        );
        ...
        const batchResult = message.options?.messageType === 'manual'
          ? null
          : antiBanProtectionService.registerMessageSent(userId, contactNumber);
        if (message.options?.messageType === 'manual') {
          antiBanProtectionService.registerOwnerManualMessage(userId, contactNumber, 'manual');
        } else if (batchResult?.shouldPause) {
          console.log(
            `🛡️ [ANTI-BLOCK v5.0] 📦 Iniciando pausa de ${batchResult.pauseDuration/1000}s ` +
            `(nível progressivo do anti-ban)`,
          );
        }
  ...
  private async sendMessage(userId: string, message: QueuedMessage): Promise<MessageSendResult> {
    ...
    const sendOptions = isInteractiveAiReply(message)
      ? { ...message.options, fastLane: true }
      : message.options;

    const messageId = await this.sendCallback(userId, message.jid, message.text, sendOptions);

    if (messageId === AUTOMATION_PAUSE_BLOCKED_MESSAGE_ID) {
      return {
        success: false,
        messageId,
        error: 'Conversa pausada por resposta manual do dono',
      };
    }
  ...
  private async sleepUntilManualFastLane(state: WhatsAppQueueState, waitMs: number): Promise<boolean> {
    const deadline = Date.now() + Math.max(0, waitMs);
    while (Date.now() < deadline) {
      if (state.queue.some((queued) => queued.options?.skipDelay === true)) {
        return true;
      }
  ...
  getStats(): Record<string, any> {
    ...
    for (const [userId, state] of entries) {
      ...
      const oldestQueuedAt = state.queue.reduce<number | null>((oldest, message) => {
        if (oldest === null || message.addedAt < oldest) return message.addedAt;
        return oldest;
      }, null);
      const priorityBreakdown = state.queue.reduce<Record<string, number>>((acc, message) => {
      ...
      stats.queues[userId.substring(0, 8)] = {
        ...
        directExecutionPending: state.directExecutionPending,
  ...
  getUserStats(userId: string): {
    ...
    return {
      ...
      isPaused: antiBanStats.isPaused,
      pauseRemainingMs: antiBanStats.pauseRemainingMs,
      minuteCount: antiBanStats.minuteCount,
      hourCount: antiBanStats.hourCount,
      dayCount: antiBanStats.dayCount,
      batchPauseLevel: antiBanStats.batchPauseLevel,
      currentPauseDurationMs: antiBanStats.currentPauseDurationMs,
    };
  }
  ...
  async waitForTurn(
    userId: string,
    description: string = 'mídia',
    options?: { prioritizeOverQueue?: boolean; skipDelay?: boolean },
  ): Promise<void> {
    return this.waitForTurnInternal(
      userId,
      description,
      Boolean(options?.prioritizeOverQueue),
      Boolean(options?.skipDelay),
    );
  }

  private async waitForTurnInternal(
    userId: string,
    description: string,
    prioritizeOverQueue: boolean,
    ...
    if (prioritizeOverQueue) {
      state.directExecutionPending += 1;
    }

    try {
      // Verificar se está em pausa de lote
      const antiBanCheck = skipDelay
        ? { canSend: true, waitMs: 0, reason: "manual_fast_lane" }
        : antiBanProtectionService.canSendMessage(userId);
      if (!antiBanCheck.canSend) {
      ...
      if (prioritizeOverQueue) {
        while (state.isProcessing) {
          console.log(`🛡️ [ANTI-BLOCK v5.0] ⏳ Aguardando envio em andamento terminar antes de enviar ${description}...`);
      ...
      const delay = skipDelay ? 0 : antiBanProtectionService.calculateDelay(userId, contactNumber);
      const timeSinceLastSent = Date.now() - state.lastSentAt;
      const remainingDelay = Math.max(0, delay - timeSinceLastSent);
      ...
      if (skipDelay) {
        console.log(`[ANTI-BLOCK v5.0] Envio manual liberado sem delay artificial: ${description}`);
      } else {
        antiBanProtectionService.registerMessageSent(userId, contactNumber);
      }
      ...
      if (prioritizeOverQueue) {
        const latestState = this.queues.get(userId);
        if (latestState) {
          latestState.directExecutionPending = Math.max(0, latestState.directExecutionPending - 1);
        }
      }
    }
  }
  ...
  async executeWithDelay<T>(
    ...
    const previous = this.directExecutionChains.get(userId) || Promise.resolve();
    ...
    try {
      await this.waitForTurnInternal(
        userId,
        description,
        Boolean(options?.yieldQueue),
        Boolean(options?.skipDelay),
      );

      try {
        const result = await sendFn();
        if (options?.ownerManualContactNumber) {
          antiBanProtectionService.registerOwnerManualMessage(
            userId,
            options.ownerManualContactNumber,
            description,
          );
        }
      ...
      release();
      const state = this.queues.get(userId);
      if (state && state.queue.length > 0 && !state.isProcessing) {
        void this.processQueue(userId);
      }
      const latest = this.directExecutionChains.get(userId);
      if (latest === chain) {
