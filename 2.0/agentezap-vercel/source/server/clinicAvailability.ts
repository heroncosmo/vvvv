/**
 * CLINIC AVAILABILITY MODULE
 *
 * Modulo unificado para calculo de disponibilidade da clinica com:
 ...
 */
export function computeMinNoticeMinutes(config: ClinicConfig): number {
...
export async function listAppointmentsForDate(
  ...
  try {
    ...
    console.error('❌ [ClinicAvailability] Erro ao buscar agendamentos:', err);
    return [];
  }
}
...
export async function getAvailableStartTimes(options: AvailableSlotsOptions): Promise<string[]> {
  ...
  if (!config) {
    console.warn('⚠️ [ClinicAvailability] Config não encontrada para userId:', userId);
    return [];
  }
  ...
  const googleBusyWindowsResult = await listClinicGoogleBusyWindows(
    userId,
    date,
  ...
  if (googleBusyWindowsResult.error) {
    console.warn("⚠️ [ClinicAvailability] Google Calendar indisponível para filtro de slots:", googleBusyWindowsResult.error);
  }
  ...
  return filterSlotsAgainstGoogleBusyWindows(
    ...
    availableSlots,
    serviceDurationMinutes,
    googleBusyWindowsResult.busyWindows,
  );
}
...
export async function checkOverlapBeforeInsert(
  ...
  try {
    ...
    console.error('❌ [ClinicAvailability] Erro na checagem de overlap:', err);
