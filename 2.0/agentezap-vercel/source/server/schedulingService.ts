  ...
  try {
    const response = await chatComplete({
      messages: [
        ...
        { role: "user", content: "Extraia nome e endereço do cliente." },
      ],
      maxTokens: 150,
      temperature: 0,
    });
...
async function extractSlotSelectionViaLLM(
  ...
  try {
    const response = await chatComplete({
      messages: [
        { role: "system", content: systemPrompt },
        { role: "user", content: "Extraia a seleção de horário do cliente." },
      ],
      maxTokens: 100,
      temperature: 0,
    });
...
async function callSchedulingPlanner(
  ...
  for (const strictSchedulingRecovery of [false, true]) {
    ...
    const messages: ChatMessage[] = [
      { role: "system", content: systemPrompt },
      { role: "user", content: "Analise a conversa e devolva o JSON do próximo passo do agendamento." },
    ];
...
async function confirmSchedulingExecutionGate(
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { allowSchedulingExecution?: boolean };
