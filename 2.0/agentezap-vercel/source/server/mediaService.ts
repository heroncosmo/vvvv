 ...
 * Agent Media Service
 * 
 * Manages agent media library context and WhatsApp media delivery.
 * Codex decides media actions through the structured turn contract.
 * 
 * All sends pass through the centralized anti-ban queue.
 */

import { db } from "./db";
import { agentMediaLibrary, messages, type AgentMedia, type InsertAgentMedia, type MistralResponse } from "@shared/schema";
...
import {
  ...
  registerSharedAutomaticOutgoingMessageForPersistence,
} from "./whatsapp";
...
/**
 * Builds the media context block supplied to Codex.
 *
 * This describes tenant-configured media as available capabilities. It must not
 * choose media by itself; media is sent only from explicit Codex contract actions.
 */
export function generateMediaPromptBlock(mediaList: AgentMedia[]): string {
  ...
  // Keep only active media in the context package.
...
`;
  ...
  // List each media entry with tenant-provided usage notes.
  for (let i = 0; i < activeMedias.length; i++) {
    const media = activeMedias[i];
    const whenToUse = media.whenToUse || 'quando solicitado';
    const mediaType = media.mediaType === 'audio' ? 'AUDIO' :
                      media.mediaType === 'video' ? 'VIDEO' :
                      media.mediaType === 'image' ? 'IMAGEM' :
                      media.mediaType === 'flow' ? 'FLUXO' : 'DOCUMENTO/PDF';
    
    // For flows, include a compact item summary.
    const flowSummary = media.mediaType === 'flow' && media.flowItems && Array.isArray(media.flowItems) && media.flowItems.length > 0
      ? `(${media.flowItems.length} itens: ${(media.flowItems as any[]).map((it: any) => it.type === 'text' ? 'texto' : `${it.mediaType || 'midia'}`).join(' -> ')})`
      : '';
    
    // Extract only lightweight terms for context display, not for local routing.
    const keywordsRaw = whenToUse.toLowerCase()
      .replace(/enviar apenas quando:|nao enviar:|não enviar:|quando:/gi, '')
...
- ${mediaType}: ${media.name}${flowSummary ? ' ' + flowSummary : ''}
  ...
  ${media.suppressTextResponse ? 'MODO DE RESPOSTA: se esta midia for acionada, responda sem texto publico e use action send_media estruturada' : ''}
  ACTION_MEDIA_NAME: ${media.name}
`;
  }
...
2. Para enviar, devolva action send_media com arguments.mediaName exatamente igual ao ACTION_MEDIA_NAME.
...
5. Se voce mencionou que vai enviar, OBRIGATORIO devolver action send_media no contrato estruturado.

Formato: action send_media com arguments.mediaName = NOME_DA_MIDIA
`;
...
// =============================================================================
// LEGACY MEDIA DETECTION COMPATIBILITY
// =============================================================================
// Media selection is disabled here. Codex must request media through the
// structured send_media contract, and this module only validates/applies sends.
// =============================================================================
...
async function classifyMediaWithLLM(_input: unknown): Promise<{
  shouldSend: boolean;
...
/**
 * Compatibility stub. Automatic media selection is intentionally disabled.
 * Codex must emit an explicit send_media action before any media is sent.
 */
export async function forceMediaDetection(
  ...
  void aiResponseText;
  void userId;
  console.log("[FORCE MEDIA] Automatic media selection disabled; explicit Codex send_media action required");
  return {
...
// Sync compatibility stub for old callers.
...
export function forceMediaDetectionSync(
  ...
  console.warn("[FORCE MEDIA] forceMediaDetectionSync is deprecated - use forceMediaDetection (async)");
  // Return an empty result so old callers fail closed.
  ...
  try {
    ...
    try {
      const transcription = await transcribeAudioCostOptimized(audioBuffer, {
        ...
        userId,
        language: 'pt',
        mistralMaxAttempts: maxAttempts,
        mistralInitialDelayMs: initialDelayMs,
        mistralMaxDelayMs: maxDelayMs,
        throwOnFailure: true,
      });
      ...
      console.warn('[MediaService] Cost-optimized audio transcription failed:', error);
    }

    console.error('[MediaService] Audio transcription failed on all configured providers. Keeping audio placeholder until provider is fixed.', {
      transcriptionError: transcriptionError instanceof Error ? transcriptionError.message : String(transcriptionError),
    });
    return null;
  } catch (error) {
...
interface ExecuteMediaActionsParams {
  ...
  onOutgoingPersisted?: (sentAt: Date) => Promise<void> | void;
  contactName?: string;
}
...
export async function executeMediaActions(
  ...
  const persistOutgoingAndBroadcast = async (payload: {
    ...
    registerSharedAutomaticOutgoingMessageForPersistence({
      ...
      mediaCaption: payload.mediaCaption || undefined,
      isFromAgent: payload.isFromAgent ?? true,
      source: payload.mediaType ? "customer_media_service" : "customer_media_service_text",
    });
    ...
    try {
      ...
      await params.onOutgoingPersisted?.(sentAt);
    } catch (error) {
    ...
    for (const action of actions as any[]) {
      if (action?.type === 'send_text') {
        ...
        await executeMediaActions({
          ...
          socket,
          wapiConfig,
          onOutgoingPersisted: params.onOutgoingPersisted,
          contactName,
        });
      }
        ...
        try {
          ...
          if (media.mediaType === 'audio') {
            ...
            if (media.transcription) {
              ...
              try {
                const audioBuffer = await downloadMediaAsBuffer(media.storageUrl);
                transcriptionText = await transcribeAudioCostOptimized(audioBuffer, {
          ...
          const messageId = acceptedMessageId || `media-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
          ...
          await persistOutgoingAndBroadcast({
            ...
            mediaCaption: media.caption || media.name,
          });
