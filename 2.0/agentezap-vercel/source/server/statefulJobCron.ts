...
const STATEFUL_JOB_CRON_GROUPS: Record<StatefulJobCronGroupId, StatefulJobCronGroupDefinition> = {
  ...
  "ai-responses": {
    id: "ai-responses",
    path: "/api/cron/stateful-jobs/ai-responses",
    description: "Recuperacao frequente das respostas de IA pendentes do WhatsApp.",
    jobs: ["pending-ai-timers"],
    defaultSchedule: "* * * * *",
    scheduleEnvVar: "STATEFUL_JOB_CRON_AI_RESPONSES_SCHEDULE",
    planHint: "pro-recommended",
    enabledByDefault: true,
  },
  "user-followup": {
  ...
  "policy-audit": {
    id: "policy-audit",
    path: "/api/cron/stateful-jobs/policy-audit",
    description: "Auditoria diaria Codex CLI de politica da plataforma e suspensao auditavel quando habilitada.",
    jobs: ["policy-audit"],
    defaultSchedule: "0 7 * * *",
    scheduleEnvVar: "STATEFUL_JOB_CRON_POLICY_AUDIT_SCHEDULE",
    planHint: "daily-compatible",
    enabledByDefault: true,
...
export function hasRequiredVercelCronSecret(
  group: StatefulJobCronGroupDefinition,
  env: NodeJS.ProcessEnv = process.env,
): boolean {
  return !group.requiresCronSecret || Boolean(resolveVercelCronSecret(env));
}
