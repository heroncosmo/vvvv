...
export async function getMemberScope(req: any): Promise<MemberScope | null> {
  ...
  const connectionResult = await pool.query(
    `
      ...
      JOIN whatsapp_connections wc ON wc.id = cm.connection_id
      WHERE cm.member_id = $1
        AND wc.user_id = $2
        AND COALESCE(cm.can_view, true) = true
    `,
    [memberId, ownerId],
  );
  const connectionIds = connectionResult.rows.map((row: any) => String(row.connection_id)).filter(Boolean);
...
function getConversationConnectionId(conversation: MemberConversationScopeCandidate): string | null {
  return conversation.connectionId ?? conversation.connection_id ?? null;
}

function isMemberConnectionScopedUnroutedConversation(
  conversation: MemberConversationScopeCandidate,
  scope: MemberScope,
): boolean {
  const sectorId = conversation.sectorId ?? conversation.sector_id ?? null;
  const connectionId = getConversationConnectionId(conversation);
  return !sectorId && !!connectionId && scope.connectionIds.includes(connectionId);
}
...
export function canMemberAccessConversation(
  ...
  if (!sectorSettings) {
    return isMemberConnectionScopedUnroutedConversation(conversation, scope);
  }
...
export function authorizeMemberReplyToConversation(
  ...
  const sectorSettings = resolveConversationSectorSettingsForMemberScope(conversation, scope);
  if (!sectorSettings) {
    if (isMemberConnectionScopedUnroutedConversation(conversation, scope) && !assignedToMemberId) {
      return {
        allowed: true,
