...
import {
  ...
  type MauricioMfcCatalogItemKind,
...
export interface CatalogProductDeliveryActionOptions {
  userId?: string | null;
  userEmail?: string | null;
  contextText?: string | null;
  mauricioMfcIncludeReady50x50Promo?: boolean | null;
  mauricioMfcLineKind?: MauricioMfcCatalogItemKind | null;
}
...
function normalizeCatalogDeliveryText(value: string | null | undefined): string {
  return String(value || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function buildCatalogVariationCaption(
  ...
  index: number,
  options?: CatalogProductDeliveryActionOptions,
): string | undefined {
  const lines: string[] = [];
  ...
  if (treatAsVariation) {
    ...
    const normalizedArtReferenceContext = normalizeCatalogDeliveryText([
      product.name,
      product.category,
      image.variation_name,
      image.caption,
    ].filter(Boolean).join(" "));
    const shouldSuppressMfcArtReferenceBasePrice =
      isMauricioMfcCatalogTenant(options || {}) &&
      !String(image.variation_price || "").trim() &&
      normalizedArtReferenceContext.includes("catalogo") &&
      (normalizedArtReferenceContext.includes("arte") || normalizedArtReferenceContext.includes("foto")) &&
      !/\b(painel|paineis|cilindro|cilindros|redondo|lateral|costura|costurado|acabamento)\b/i.test(normalizedArtReferenceContext);
    const priceLabel =
      mauricioMfcPriceLine ||
      formatVariationPrice(image.variation_price) ||
      (shouldSuppressMfcArtReferenceBasePrice ? null : formatVariationPrice(product.price));
    if (priceLabel) {
      lines.push(mauricioMfcPriceLine ? priceLabel : `Preço ${priceLabel}`);
    }
...
export function buildCatalogProductDeliveryActions(
  ...
  const mauricioMfcLineKind = isMauricioMfcTenant
    ? options?.mauricioMfcLineKind || resolveMauricioMfcRequestedLineKind(options?.contextText)
    : null;
  if (mauricioMfcLineKind) {
    const matchingImages = orderedImages.filter((image) =>
      mauricioMfcCatalogEntryMatchesLineKind({
        userId: options?.userId,
        userEmail: options?.userEmail,
        productName: product.name,
  ...
  const canSendDescription =
    !isMauricioMfcTenant ||
    options?.mauricioMfcIncludeReady50x50Promo === true ||
    !containsMauricioMfcReady50x50PromoText(description);
