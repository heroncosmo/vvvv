...
import { conversationScheduledMessageService } from "./conversationScheduledMessageService";
...
import { runPlatformPolicyAuditOnce } from "./platformPolicyAuditCodexService";
...
const STATEFUL_JOB_DEFINITIONS: Record<string, StatefulJobDefinition> = {
  ...
  "user-followup": {
    description: "Delega o ciclo do follow-up dos clientes para o runtime web-only agentic.",
    requiresStatefulRuntime: true,
    run: async () => runVpsCronHttpPath("/api/cron/stateful-jobs/user-followup", "stateful-job:user-followup"),
  },
  "pending-ai-timers": {
    description: "Roda um ciclo de recuperacao das respostas de IA pendentes do WhatsApp.",
    requiresStatefulRuntime: true,
    run: async () => {
      const { runAutoRecoveryCycle, runPendingTimersCronCycle } = await import("./whatsapp");
  ...
  "policy-audit": {
    description: "Roda auditoria diaria Codex CLI de politica da plataforma em modo context-only.",
    requiresStatefulRuntime: true,
    run: async () => {
      const result = await runPlatformPolicyAuditOnce();
      return {
        accepted: result.accepted,
        skipped: result.skipped,
        details: result,
...
export function startAutoStartedStatefulIntervalJobs() {
  ...
  console.log("[STATEFUL JOBS] User follow-up interval is disabled; use /api/cron/stateful-jobs/user-followup.");
  conversationScheduledMessageService.start();
...
export function stopAutoStartedStatefulIntervalJobs() {
  ...
  conversationScheduledMessageService.stop();
