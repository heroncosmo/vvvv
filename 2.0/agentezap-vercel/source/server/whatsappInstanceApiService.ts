...
export async function syncInstanceConversationHistory(
  connectionId: string,
  conversationId: string,
) {
  const conversation = await storage.getConversation(conversationId);
  ...
  return syncConversationHistoryOnDemand({
    connectionId,
    conversationId,
  });
}
...
function isUniqueInstanceMessagePersistenceError(error: unknown): boolean {
...
async function persistAcceptedInstanceOutgoingMessage(params: {
  conversation: Conversation;
  messageId?: string | null;
  text: string;
  isFromAgent?: boolean;
}): Promise<void> {
  const messageId = String(params.messageId || `instance_${Date.now()}`).trim();
  const sentAt = new Date();
  const previewText = params.text.substring(0, 255);
  ...
  try {
    try {
      ...
      if (!isUniqueInstanceMessagePersistenceError(messageError)) {
        throw messageError;
      }
    }
    ...
    console.error(
      `[InstanceAPI] WhatsApp aceitou envio, mas a persistencia local falhou. ` +
        `conversationId=${params.conversation.id} messageId=${messageId}`,
      error,
    );
  }
}
...
export async function sendMediaViaInstance(params: SendMediaViaInstanceParams) {
  ...
  const result = await sendUserMediaMessage(params.connection.userId, conversation.id, {
    ...
    skipAutoPause: !isOwnerManualSend,
  });
...
export async function sendContactViaInstance(params: SendContactViaInstanceParams) {
  ...
  await persistAcceptedInstanceOutgoingMessage({
    conversation,
    messageId: sentMessage?.key?.id || null,
    text: params.displayName
      ? `Contato enviado: ${params.displayName}`
      : "Contato enviado",
    isFromAgent: true,
  });
...
export async function sendLocationViaInstance(params: SendLocationViaInstanceParams) {
  ...
  await persistAcceptedInstanceOutgoingMessage({
    ...
    messageId: sentMessage?.key?.id || null,
    text: params.name
      ? `Localizacao enviada: ${params.name}`
      : "Localizacao enviada",
    isFromAgent: true,
  });
