...
const RESTORE_SERVICE_PAUSE_MAX_MS = Math.max(
  Number(process.env.WA_RESTORE_SERVICE_PAUSE_MAX_MS || process.env.WA_RESTORE_GUARD_MAX_BLOCK_MS || 120_000),
  60_000,
);
const restorePauseStartedAtByReason = new Map<string, number>();
let lastStaleRestorePauseLogKey: string | null = null;

function minuteLogKey(reason: string, now: number) {
  return `${reason}:${Math.floor(now / 60_000)}`;
}

function getRawWhatsAppRestorePauseReason(): string | null {
...
function getWhatsAppRestorePauseReason(): string | null {
  const pauseReason = getRawWhatsAppRestorePauseReason();
  if (!pauseReason) {
    restorePauseStartedAtByReason.clear();
    lastStaleRestorePauseLogKey = null;
    return null;
  }

  const now = Date.now();
  const startedAt = restorePauseStartedAtByReason.get(pauseReason) || now;
  restorePauseStartedAtByReason.set(pauseReason, startedAt);
  const ageMs = now - startedAt;
  if (ageMs >= RESTORE_SERVICE_PAUSE_MAX_MS) {
    const logKey = minuteLogKey(pauseReason, now);
    if (lastStaleRestorePauseLogKey !== logKey) {
      lastStaleRestorePauseLogKey = logKey;
      console.warn(
        `[FULL APP] ${pauseReason} is stale after ${Math.round(ageMs / 1000)}s; allowing scheduled services to continue.`,
      );
    }
    return null;
  }
...
function startPendingAIRecoveryServices() {
  ...
  setTimeout(() => {
    console.log(
      `[FULL APP] Starting pending AI timers after ${Math.round(startupDelayMs / 1000)}s startup delay.`,
    );
...
function startPendingIncomingRecoveryService() {
  const startupDelayMs = Math.max(
    Number(process.env.WA_PENDING_INCOMING_RECOVERY_STARTUP_DELAY_MS || 20_000),
    5_000,
  );

  setTimeout(() => {
    console.log(
      `[FULL APP] Starting pending incoming recovery after ${Math.round(startupDelayMs / 1000)}s startup delay.`,
    );
    startPendingIncomingRecoveryCron();
  }, startupDelayMs);
}

function schedulePendingAIRecoveryServices() {
  if (process.env.WA_PENDING_TIMERS_START_DURING_RESTORE === "true") {
    console.log("[FULL APP] Pending AI timers are configured to start during WhatsApp restore.");
    startPendingAIRecoveryServices();
    return;
  }

  runAfterWhatsAppRestore("pending AI recovery services", startPendingAIRecoveryServices);
}
...
function startStatefulAppServices() {
  startWhatsAppLeaderElection({
    onLeader: async () => {
      ...
      startConnectionHealthCheck();
      startPendingIncomingRecoveryService();
      schedulePendingAIRecoveryServices();
