...
${JSON.stringify(candidateList, null, 2)}`;
  ...
  const completeVariationMatch = deps.completeVariationMatch;
  if (!completeVariationMatch) {
    return null;
  }

  try {
    const response = await completeVariationMatch({
      messages: [
