...
async function requestAgendamento2InsightJson(params: {
  conversation: Agendamento2ConversationContextRow;
  messages: Agendamento2MessageContextRow[];
  latestAgentReply: string;
  previousInsight: ExistingAgendamento2InsightRow | null;
...
async function analyzeConversation(params: {
  ...
  const rawText = await requestAgendamento2InsightJson({
  ...
  if (!rawText) {
    console.warn("[AGENDAMENTO 2.0] Analise LLM legada desativada; sem contrato Codex, insight nao sera gravado.");
    return null;
  }
  ...
  try {
    ...
    if (!strictRawText) {
      console.warn("[AGENDAMENTO 2.0] Reparo JSON legado desativado; insight nao sera gravado.");
      return null;
    }
