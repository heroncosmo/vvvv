...
import { db, withRetry, ensureAudioResponseModeConstraint } from "./db";
import { isAppVisibleWhatsappConnection, isUserRemovedConnection } from "./internalSimulatorConnection";
...
import { transcribeAudioCostOptimized } from "./audioTranscriptionOrchestrator";
import { normalizePhoneForComparison as buildPhoneComparisonKeys, normalizePhoneToDigits, phoneNumbersMatch } from "./phoneMatch";
import { buildBrazilWhatsAppPhoneVariants, buildWhatsAppJidFromPhone } from "./whatsappPhoneNumber";
import { supabase } from "./supabaseAuth";
import {
  buildAdminBusinessDashboardReport,
  type AdminBusinessDashboardReport,
} from "./adminBusinessDashboard";
import { getAdminFollowupGlobalConfig, isLegacyAdminFollowupConfig, normalizeAdminFollowupConfig } from "./adminFollowupMigrationService";
import {
  isAdminConversationFollowupManuallyPaused,
  isAdminConversationManuallyPaused,
} from "./adminConversationAutomationState";
import { shouldRecoverCompletedTimer } from "./pendingAiDeliveryState";
import { getPublicCatalogPlans as filterPublicCatalogPlans } from "@shared/planCatalog";
import { repairMojibakeDeep } from "@shared/mojibake";
import { attachMediaToProducts, fetchProductMediaRows } from "./productCatalogAssets";
import { hasCatalogVariationMetadata } from "./productCatalogMessageActions";
...
import { sendMetaPaidWhatsappConversion, type MetaWhatsappAdsAttribution } from "./metaConversionsApi";
...
const RODRIGO_PAID_LEAD_OWNER_EMAIL = "rodrigo4@gmail.com";
const DEFAULT_AUDIO_DAILY_LIMIT = 30;

function normalizeAudioDailyLimit(value: unknown): number {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed <= 0) return DEFAULT_AUDIO_DAILY_LIMIT;
...
export interface AttentionQueueFilters {
  mode?: "critical" | "high" | "needsHumanAttention" | "all";
...
async function resolveAudioTranscriptionForMessage(params: {
  ...
  if (!audioBuffer || audioBuffer.length === 0) {
    console.log("[Storage] Audio transcription skipped: could not load audio buffer.");
    return null;
  }

  let transcriptionModel: string | undefined;
  let llmUserId: string | undefined;
  ...
  console.log("[Storage] Starting cost-optimized audio transcription.");
  try {
    const transcription = await transcribeAudioCostOptimized(audioBuffer, {
      fileName: "whatsapp-audio.ogg",
      mimeType: params.mediaMimeType || "audio/ogg",
      language: "pt",
      mistralModel: transcriptionModel,
      userId: llmUserId,
      maxAttemptsPerModel: params.throwOnFailure ? 2 : 1,
      throwOnFailure: true,
    });

    if (transcription && transcription.length > 0) {
      console.log(`[Storage] Audio transcribed: "${transcription.substring(0, 100)}..."`);
      return transcription;
    }
  } catch (error) {
    transcriptionError = error;
    console.warn("[Storage] Audio transcription failed with cost-optimized path.", {
      message: error instanceof Error ? error.message : String(error),
    });
  }

  console.log("[Storage] Audio transcription returned empty/null on all configured STT providers.");
  if (params.throwOnFailure) {
    if (transcriptionError && isRetryableAudioTranscriptionError(transcriptionError)) {
      throw transcriptionError;
    }
    throw new AudioTranscriptionError("Transcricao de audio retornou vazia.", {
      retryable: false,
...
type PendingAudioTranscriptionRetryResult = {
  checkedCount: number;
  updatedCount: number;
...
export interface IStorage {
  ...
  getRecentConversationRecoveryState(connectionId: string, since: Date): Promise<ConversationRecoveryState[]>;
  ...
  findAdminMirrorConversationForConnection(
    connectionId: string,
    contactNumber: string,
  ): Promise<{ adminConversationId: string; adminId: string } | null>;
  getConversation(id: string): Promise<Conversation | undefined>;
  createConversation(conversation: InsertConversation): Promise<Conversation>;
  updateConversation(id: string, data: Partial<InsertConversation>): Promise<Conversation>;
  clearConversationOperationalHistory(conversationId: string): Promise<void>;
  recordConversationWhatsappAdsAttribution(data: {
    conversationId: string;
    connectionId: string;
    userId: string;
    contactNumber: string;
  ...
  recordAdminConversationWhatsappAdsAttribution(
    conversationId: string,
    attribution: Record<string, any>,
  ): Promise<void>;
  ...
  getLastMessageByConversationId(
    conversationId: string,
  ): Promise<Pick<Message, "id" | "fromMe" | "text" | "timestamp"> | undefined>;
  ...
  getMessageByConversationAndMessageId(conversationId: string, messageId: string): Promise<Message | undefined>;
  getAdminMessageByMessageId(messageId: string): Promise<any | undefined>;
  getLastAdminMessageByConversationId(
    conversationId: string,
  ): Promise<{ id: string; fromMe: boolean; text: string | null; timestamp: Date } | undefined>;
  deleteMessagesByConversationId(conversationId: string): Promise<void>;
  createMessage(message: InsertMessage): Promise<Message>;
  retryPendingAudioTranscriptionsForConversation(
    conversationId: string,
    options?: { limit?: number },
  ): Promise<PendingAudioTranscriptionRetryResult>;
  updateAdminMessage(id: string, data: Partial<{
    text: string;
    status: string;
    mediaType: string;
    mediaUrl: string;
  ...
  getDisabledConversationIds(conversationIds: string[]): Promise<string[]>;
  ...
  getConversationsToAutoReactivate(): Promise<Array<{ conversationId: string; clientLastMessageAt: Date | null; clientHasPendingMessage: boolean; ownerLastReplyAt: Date | null; autoReactivateAfterMinutes: number | null }>>;
  ...
  getPublicCatalogPlans(options?: { extraVisiblePlanIds?: readonly string[] }): Promise<Plan[]>;
  ...
  // Admin operations
  getAdminById(id: string): Promise<any | undefined>;
  ...
  getAdminBusinessDashboardReport(): Promise<AdminBusinessDashboardReport>;
  ...
  getRunningAdminBroadcasts?(): Promise<any[]>;
  ...
  updatePendingAIResponseMessages(
    conversationId: string,
    messages: string[],
    executeAt: Date,
    options?: { resetRetry?: boolean },
  ): Promise<void>;
  replacePendingAIResponseMessages(conversationId: string, messages: string[]): Promise<void>;
  ...
  markPendingAIResponseCompleted(conversationId: string, guard?: PendingAIResponseMutationGuard): Promise<void>;
  markPendingAIResponseFailed(conversationId: string, reason: string, lastError?: string, guard?: PendingAIResponseMutationGuard): Promise<void>;
  ...
  resetPendingAIResponseForRetry(
    conversationId: string,
    delaySec?: number,
    guard?: PendingAIResponseMutationGuard,
    options?: { resetRetryCount?: boolean; lastError?: string | null },
  ): Promise<void>;
...
export type PendingAIResponseMutationGuard = {
  expectedMessages?: string[];
  notUpdatedAfter?: Date;
};

function buildPendingAIResponseMutationGuard(guard?: PendingAIResponseMutationGuard) {
  return sql`
...
function confirmedOutgoingMessageStatusSql(statusColumn: any) {
  return sql`
...
export class DatabaseStorage implements IStorage {
  ...
  private pendingAiSkippedSupportChecked = false;

  private async ensurePendingAiSkippedSupportChecked(): Promise<void> {
    if (this.pendingAiSkippedSupportChecked) {
      return;
    }
    ...
    try {
      const result = await db.execute(sql`
        ...
        WHERE n.nspname = 'public'
          AND t.relname = 'pending_ai_responses'
          AND c.conname = 'pending_ai_responses_status_check'
        LIMIT 1
      `);
      const definition = String((result.rows?.[0] as any)?.definition || "");
      if (definition && !definition.includes("'skipped'")) {
        this.pendingAiSkippedUnsupported = true;
      }
    } catch (error) {
      console.warn("[DB] Nao foi possivel confirmar suporte a status skipped em pending_ai_responses:", error);
    } finally {
      this.pendingAiSkippedSupportChecked = true;
    }
  }
  ...
  private buildQualifiedTableName(tableName: string, schemaName: string = "public"): string {
    return `${this.quotePgIdentifier(schemaName)}.${this.quotePgIdentifier(tableName)}`;
  }

  private async getForeignKeyReferencesToTable(
    tx: any,
    tableName: string,
    schemaName: string = "public",
  ): Promise<Array<{ schemaName: string; tableName: string; columnName: string }>> {
    ...
    return (((result as any)?.rows || []) as Array<any>).map((row) => ({
  ...
  private async getSinglePrimaryKeyColumn(tx: any, tableName: string, schemaName: string = "public"): Promise<string | null> {
    const result = await tx.execute(sql`
  ...
  private async deleteRowsByForeignKey(
    tx: any,
    reference: { schemaName: string; tableName: string; columnName: string },
    parentIds: Array<string | number>,
  ): Promise<void> {
    if (parentIds.length === 0) return;

    const uniqueParentIds = Array.from(new Set(parentIds.filter((value) => value !== null && value !== undefined)));
    ...
    if (primaryKeyColumn) {
      ...
      const childIds = Array.from(
        new Set(
          (((childIdRows as any)?.rows || []) as Array<any>)
            .map((row) => row?.id)
  ...
  private async deleteCascadeByIds(
    tx: any,
    tableName: string,
    ids: Array<string | number>,
    schemaName: string = "public",
  ): Promise<void> {
    const uniqueIds = Array.from(new Set(ids.filter((value) => value !== null && value !== undefined)));
  ...
  private async cleanupSupabaseAuthUsers(userId: string, authEmails: Set<string>): Promise<void> {
    if (authEmails.size === 0) return;

    try {
      const attemptedAuthIds = new Set<string>();
  ...
  async deleteUser(id: string): Promise<void> {
    ...
    await db.transaction(async (tx) => {
      ...
      if (cleanPhone) {
        const adminConversationRows = await tx
  ...
  // Conversation operations
  private async attachAssignedMemberMetadata<T extends { assignedToMemberId?: string | null }>(
    items: T[],
  ): Promise<Array<T & { assignedMemberName: string | null; assignedMemberEmail: string | null }>> {
    const memberIds = Array.from(
      new Set(
        items
          .map((item) => String(item.assignedToMemberId || "").trim())
          .filter(Boolean),
      ),
    );
    ...
    return items.map((item) => {
      const member = item.assignedToMemberId ? membersById.get(String(item.assignedToMemberId)) : null;
      return {
        ...item,
        assignedMemberName: member?.name || null,
        assignedMemberEmail: member?.email || null,
      };
    });
  }
  ...
  async getConversationsByConnectionId(connectionId: string): Promise<Conversation[]> {
    ...
    return await this.attachAssignedMemberMetadata(rows) as Conversation[];
  }

  async getRecentConversationRecoveryState(
    connectionId: string,
    since: Date,
  ): Promise<ConversationRecoveryState[]> {
    const rows = await db
      .select({
        id: conversations.id,
  ...
  async findConversationByIdentity(
    connectionId: string,
    params: {
      contactNumber?: string | null;
      remoteJid?: string | null;
  ...
  async findAdminMirrorConversationForConnection(
    connectionId: string,
    contactNumber: string,
  ): Promise<{ adminConversationId: string; adminId: string } | null> {
    const [result] = await db
  ...
  async createConversation(conversationData: InsertConversation): Promise<Conversation> {
    ...
    if (conversationData.connectionId && (conversationData.contactNumber || conversationData.remoteJid)) {
      ...
      if (existing) {
        console.log(`⚠️ [STORAGE] Conversa ativa já existe para ${conversationData.contactNumber || conversationData.remoteJid} (${existing.id}), retornando existente em vez de duplicar`);
  ...
  private normalizeWhatsappAdsAttribution(input: unknown): MetaWhatsappAdsAttribution | null {
    if (!input || typeof input !== "object") return null;
    const source = input as Record<string, any>;
    const clean = (value: unknown, maxLength = 512): string | null => {
  ...
  private pickBestWhatsappAdsAttribution(
    ...candidates: Array<MetaWhatsappAdsAttribution | null | undefined>
  ): MetaWhatsappAdsAttribution | null {
    const normalized = candidates
      .map((candidate) => this.normalizeWhatsappAdsAttribution(candidate))
      .filter(Boolean) as MetaWhatsappAdsAttribution[];
    if (!normalized.length) return null;

    return (
      normalized.find((candidate) => Boolean(candidate.ctwaClid)) ||
      normalized.find((candidate) => Boolean(candidate.sourceId)) ||
      normalized[0]
    );
  }
  ...
  async recordConversationWhatsappAdsAttribution(data: {
    ...
    const [existing] = await db
      .select({ rawAnalysis: conversationLeadIntelligence.rawAnalysis })
      .from(conversationLeadIntelligence)
      .where(eq(conversationLeadIntelligence.conversationId, data.conversationId))
      .limit(1);

    const currentRawAnalysis = ((existing?.rawAnalysis || {}) as Record<string, any>) || {};
  ...
  async recordAdminConversationWhatsappAdsAttribution(
    conversationId: string,
    attribution: Record<string, any>,
  ): Promise<void> {
    ...
    const currentContextState = ((conversation.contextState || {}) as Record<string, any>) || {};
  ...
  private async getConversationWhatsappAdsAttribution(
    conversationId?: string | null,
  ): Promise<MetaWhatsappAdsAttribution | null> {
    if (!conversationId) return null;
    const [row] = await db
      .select({ rawAnalysis: conversationLeadIntelligence.rawAnalysis })
      .from(conversationLeadIntelligence)
      .where(eq(conversationLeadIntelligence.conversationId, conversationId))
      .limit(1);

    return this.normalizeWhatsappAdsAttribution((row?.rawAnalysis as Record<string, any> | undefined)?.whatsappAdsAttribution);
  }

  private async getAdminConversationWhatsappAdsAttribution(
    conversationId?: string | null,
  ): Promise<MetaWhatsappAdsAttribution | null> {
    if (!conversationId) return null;
    const [row] = await db
      .select({ contextState: adminConversations.contextState })
      .from(adminConversations)
      .where(eq(adminConversations.id, conversationId))
      .limit(1);

    return this.normalizeWhatsappAdsAttribution((row?.contextState as Record<string, any> | undefined)?.whatsappAdsAttribution);
  }

  async clearConversationOperationalHistory(conversationId: string): Promise<void> {
  ...
  async getMessagesByConversationId(conversationId: string): Promise<Message[]> {
    ...
    const result = await db
      ...
      .from(messages)
      .where(eq(messages.conversationId, conversationId))
      .orderBy(asc(messages.timestamp), asc(messages.createdAt), asc(messages.id));
  ...
  async updateMessageMedia(messageId: string, newMediaUrl: string): Promise<void> {
    await db
      ...
      .where(or(eq(messages.messageId, messageId), eq(messages.id, messageId)));
  ...
  async updateMessage(id: string, data: Partial<InsertMessage>): Promise<Message> {
    ...
    const nextMediaMimeType = data.mediaMimeType ?? currentMessage?.mediaMimeType ?? null;
    const nextMediaDuration = data.mediaDuration ?? currentMessage?.mediaDuration ?? null;
    const nextFromMe = data.fromMe ?? currentMessage?.fromMe ?? null;
    const nextTimestamp = data.timestamp ?? currentMessage?.timestamp ?? null;
    const nextText =
    ...
    if (
      ...
      nextMediaType === "audio" &&
      nextMediaUrl &&
      isPendingAudioTranscriptionText(nextText)
    ) {
      try {
        ...
        if (transcription) {
          data.text = formatAudioMessageTextWithTranscription(nextText, transcription);
        }
      } catch (error) {
  ...
  async retryPendingAudioTranscriptionsForConversation(
    conversationId: string,
    options?: { limit?: number },
  ): Promise<PendingAudioTranscriptionRetryResult> {
    ...
    const pendingAudioMessages = recentMessages
      .filter((message) =>
    ...
    const result: PendingAudioTranscriptionRetryResult = {
      checkedCount: pendingAudioMessages.length,
      updatedCount: 0,
      updatedMessageIds: [],
      remainingPendingCount: pendingAudioMessages.length,
      retryableFailure: false,
      permanentFailure: false,
    };

    for (const pendingMessage of pendingAudioMessages) {
      try {
        const transcription = await resolveAudioTranscriptionForMessage({
          conversationId,
          mediaType: pendingMessage.mediaType,
          mediaUrl: pendingMessage.mediaUrl,
          mediaMimeType: pendingMessage.mediaMimeType,
          mediaDuration: pendingMessage.mediaDuration,
          fromMe: pendingMessage.fromMe,
          messageTimestamp: pendingMessage.timestamp,
          throwOnFailure: true,
        });
        ...
        const nextText = formatAudioMessageTextWithTranscription(
          pendingMessage.text,
          transcription,
        );
        ...
        await db
          .update(conversations)
          .set({ lastMessageText: nextText })
          .where(and(
            eq(conversations.id, conversationId),
            lte(conversations.lastMessageTime, pendingMessage.timestamp),
          ));

        result.updatedCount += 1;
        result.updatedMessageIds.push(pendingMessage.id);
        result.remainingPendingCount = Math.max(0, result.remainingPendingCount - 1);
      } catch (error) {
        const retryable = isRetryableAudioTranscriptionError(error);
        const retryAfterMs = getAudioTranscriptionRetryAfterMs(error);
        const formattedError = formatAudioTranscriptionErrorMessage(error);

        result.lastError = formattedError;
        if (retryable) {
          result.retryableFailure = true;
          if (retryAfterMs && (!result.retryAfterMs || retryAfterMs > result.retryAfterMs)) {
            result.retryAfterMs = retryAfterMs;
          }
          console.warn(
            `[AUDIO TRANSCRIPTION] Falha temporaria ao reprocessar audio pendente ${pendingMessage.id}: ${formattedError}`,
          );
        } else {
          result.permanentFailure = true;
          console.warn(
            `[AUDIO TRANSCRIPTION] Falha nao retryable ao reprocessar audio pendente ${pendingMessage.id}: ${formattedError}`,
          );
        }
      }
    }
  ...
  async createMessage(messageData: InsertMessage): Promise<Message> {
    ...
    if (data.mediaType === "audio" && data.mediaUrl) {
      try {
        ...
        if (transcription) {
          data.text = formatAudioMessageTextWithTranscription(data.text, transcription);
        }
      } catch (error) {
    ...
    // Analise automatica de imagens usando a cadeia de visao configurada.
    ...
    if (data.mediaType === "image" && data.mediaUrl && !data.fromMe) {
      try {
        ...
        if (imageUrl) {
          console.log(`🖼️ [Storage] Iniciando analise de imagem com Vision...`);
...
- Se for print/foto com DADOS DO CLIENTE, ENDERECO, DOCUMENTO, CONTATO ou pedido para entrega: extraia nome completo, CPF/CNPJ/documento, telefone, rua, numero, complemento, bairro, cidade, UF/estado e CEP quando visiveis.
- Nao confunda CPF/CNPJ/documento com valor de pagamento. Valor monetario precisa ter R$ ou formato claro de dinheiro.
...
Responda de forma concisa, preservando texto/OCR util. Se houver duvida, diga o campo incerto em vez de inventar. Para catalogo/pedido com varios itens, use uma lista curta com ate 10 itens para preservar todos os codigos visiveis.`;

          const conversationRecord = await db.query.conversations.findFirst({
            where: eq(conversations.id, data.conversationId),
            columns: { connectionId: true },
          });
          const connectionRecord = conversationRecord
            ? await db.query.whatsappConnections.findFirst({
            ...
            if (connectionRecord?.userId) {
              try {
                const { data: productsConfig } = await supabase
                  ...
                  .select("is_active, send_to_ai, image_variations_enabled")
                  .eq("user_id", connectionRecord.userId)
                  .maybeSingle();

                if (
                  productsConfig?.is_active === true &&
                  productsConfig?.send_to_ai !== false
                ) {
                  ...
                  const hydratedProducts = attachMediaToProducts(
                    ...
                    await fetchProductMediaRows({
                      ...
                      productIds: (productsData || []).map((product: any) => String(product.id)),
                    }),
                  );

                  const hasVariationAwareCatalog =
                    productsConfig?.image_variations_enabled === true ||
                    hydratedProducts.some((product: any) =>
                      (Array.isArray(product.media_items) ? product.media_items : []).some((media: any) =>
                  ...
                  if (hasVariationAwareCatalog) {
                    const matchResult = await matchCatalogVariationFromCustomerImage({
                      ...
                      candidates: hydratedProducts.flatMap((product: any) =>
                        (Array.isArray(product.media_items) ? product.media_items : [])
                          .filter((media: any) => media?.variation_is_active !== false)
                          .map((media: any) => ({
                    ...
                    if (matchResult?.matched) {
                      const matches = Array.isArray(matchResult.matches) && matchResult.matches.length > 0
                        ? matchResult.matches.slice(0, 10)
                        : matchResult.mediaId && matchResult.productId
                          ? [{ productId: matchResult.productId, mediaId: matchResult.mediaId }]
                          : [];
                      const matchedDetails: string[] = [];
                      const matchedCodes: number[] = [];

                      for (const [index, match] of matches.entries()) {
                        const matchedProduct = hydratedProducts.find((product: any) => String(product.id) === match.productId);
                        const matchedMedia = matchedProduct?.media_items?.find((media: any) => String(media.id) === match.mediaId);
                        if (!matchedProduct || !matchedMedia) {
                          continue;
                        }

                        const details: string[] = [`item ${index + 1}`, `produto ${matchedProduct.name}`];
                      ...
                      if (matchedDetails.length > 0) {
                        const codesSummary = matchedCodes.length > 0
                          ? `codigos selecionados ${matchedCodes.join(", ")} | `
                          : "";
                ...
                console.warn("[Storage] Falha ao enriquecer imagem com variação do catálogo:", catalogMatchError);
              }
            }
  ...
  async getAgentConfig(userId: string): Promise<AiAgentConfig | undefined> {
    ...
    return config ? repairMojibakeDeep(config) : undefined;
  }
  ...
  async upsertAgentConfig(userId: string, data: Partial<InsertAiAgentConfig>): Promise<AiAgentConfig> {
    ...
    const [config] = await db
      ...
      .values({ userId, ...normalizedData } as InsertAiAgentConfig)
      .onConflictDoUpdate({
  ...
  async upsertBusinessAgentConfig(userId: string, data: Partial<InsertBusinessAgentConfig>): Promise<BusinessAgentConfig> {
    ...
    const [config] = await db
      ...
      .values({ userId, ...normalizedData } as InsertBusinessAgentConfig)
      .onConflictDoUpdate({
  ...
  async getDisabledConversationIds(conversationIds: string[]): Promise<string[]> {
    ...
    const rows = await db
      .select({ conversationId: agentDisabledConversations.conversationId })
      .from(agentDisabledConversations)
      .where(inArray(agentDisabledConversations.conversationId, ids));
  ...
  async getConversationsToAutoReactivate(): Promise<Array<{ conversationId: string; clientLastMessageAt: Date | null; clientHasPendingMessage: boolean; ownerLastReplyAt: Date | null; autoReactivateAfterMinutes: number | null }>> {
  ...
  async getPublicCatalogPlans(options?: { extraVisiblePlanIds?: readonly string[] }): Promise<Plan[]> {
    const activePlans = await this.getActivePlans();
    return filterPublicCatalogPlans(activePlans, options);
  }
  ...
  async getUserSubscription(userId: string): Promise<(Subscription & { plan: Plan }) | undefined> {
    ...
    const approvedReceiptRows = subscriptionIds.length > 0
      ? await db
          .select({
            subscriptionId: paymentReceipts.subscriptionId,
            receiptCreatedAt: sql<Date>`max(${paymentReceipts.createdAt})`,
            approvedReceiptAt: sql<Date>`max(coalesce(${paymentReceipts.reviewedAt}, ${paymentReceipts.updatedAt}, ${paymentReceipts.createdAt}))`,
          })
    ...
    const receiptCreatedAtBySubscriptionId = new Map(
      approvedReceiptRows.map((row) => [row.subscriptionId, row.receiptCreatedAt ?? null]),
    );

    const candidates = result.map((row) => ({
      ...row.subscriptions,
      plan: row.plans,
      receiptCreatedAt: receiptCreatedAtBySubscriptionId.get(row.subscriptions.id) ?? null,
  ...
  private buildPaidLeadPhoneCandidates(user: Pick<User, "phone" | "whatsappNumber">): string[] {
  ...
  private async getRodrigoPaidTag(ownerUserId: string): Promise<Tag | undefined> {
    ...
    const [fallback] = await db
      .select()
      .from(tags)
      .where(and(eq(tags.userId, ownerUserId), eq(tags.name, "Pago")))
      .limit(1);
    return fallback;
  }

  private async markRodrigoPaidLeadAttribution(subscription: Subscription): Promise<void> {
    ...
    const [ownerUser] = await db
      .select()
      .from(users)
      .where(eq(users.email, RODRIGO_PAID_LEAD_OWNER_EMAIL))
      .limit(1);
    const ownerAdmin = await this.getAdminByEmail(RODRIGO_PAID_LEAD_OWNER_EMAIL);
    if (!ownerUser && !ownerAdmin) return;

    let crmConversation: Conversation | undefined;
    let nativeWhatsappLabelResult: unknown = null;
    if (ownerUser) {
    ...
    if (ownerAdmin?.id) {
      const [adminConversation] = await db
        .select()
        .from(adminConversations)
        .where(
          and(
    ...
    const previousAttribution = (metadata.rodrigoPaidLeadAttribution || {}) as Record<string, any>;
    const plan = await this.getPlan(subscription.planId).catch(() => undefined);
    ...
    const whatsappAdsAttribution = this.pickBestWhatsappAdsAttribution(
      ...
      previousAttribution.whatsappAdsAttribution as MetaWhatsappAdsAttribution | null,
    );
  ...
  // Admin operations
  async getAdminById(id: string): Promise<any | undefined> {
  ...
  async getAdminWhatsappConnection(adminId: string): Promise<AdminWhatsappConnection | undefined> {
    ...
    return sanitizeAdminWhatsappConnection(connection as any) as AdminWhatsappConnection | undefined;
  }
  ...
  async createAdminWhatsappConnection(connection: InsertAdminWhatsappConnection): Promise<AdminWhatsappConnection> {
    const [created] = await db
      ...
      .values(sanitizeAdminWhatsappConnection(connection as any) as InsertAdminWhatsappConnection)
      .returning();
    return sanitizeAdminWhatsappConnection(created as any) as AdminWhatsappConnection;
  }
  ...
  async updateAdminWhatsappConnection(adminId: string, data: Partial<InsertAdminWhatsappConnection>): Promise<AdminWhatsappConnection> {
    const [updated] = await db
      ...
      .set({
        ...(sanitizeAdminWhatsappConnection(data as any) as Partial<InsertAdminWhatsappConnection>),
        updatedAt: new Date(),
      })
      .where(eq(adminWhatsappConnection.adminId, adminId))
      .returning();
    return sanitizeAdminWhatsappConnection(updated as any) as AdminWhatsappConnection;
  }
  ...
  async getTotalRevenue(): Promise<number> {
    const historyResult = await db
      .select({ total: sql<number>`COALESCE(SUM(CAST(${paymentHistory.amount} AS NUMERIC)), 0)` })
      .from(paymentHistory)
      .where(eq(paymentHistory.status, "approved"));
  ...
  async getAdminBusinessDashboardReport(): Promise<AdminBusinessDashboardReport> {
    const [allUsers, allConnections, allSubscriptions, allPaymentHistory, pendingReceiptCount, activePlansCount] =
      await withRetry(() =>
        Promise.all([
          this.getAllUsers(),
  ...
  async createAdminConversation(data: {
    ...
    const [result] = await db
      ...
      .values({
        ...
        followupStage: 0,
        nextFollowupAt: null,
        followupConfig: globalFollowupConfig as any,
      })
      .returning();
    return result;
  }
  ...
  async getOrCreateAdminConversation(adminId: string, contactNumber: string, remoteJid?: string, contactName?: string, contactAvatar?: string): Promise<any> {
    ...
    if (!conversation) {
    ...
    } else if (contactName || contactAvatar || isLegacyAdminFollowupConfig(conversation.followupConfig as any)) {
      ...
      if (isLegacyAdminFollowupConfig(conversation.followupConfig as any)) {
        updates.followupConfig = normalizeAdminFollowupConfig(await getAdminFollowupGlobalConfig()) as any;
      }
  ...
  async getAdminMessageByMessageId(messageId: string): Promise<any | undefined> {
    const [result] = await db
      .select()
      .from(adminMessages)
      .where(eq(adminMessages.messageId, messageId))
      .limit(1);
    return result;
  }

  async getLastAdminMessageByConversationId(
    conversationId: string,
  ): Promise<{ id: string; fromMe: boolean; text: string | null; timestamp: Date } | undefined> {
    const [result] = await db
      .select({
      ...
      .from(adminMessages)
      .where(eq(adminMessages.conversationId, conversationId))
      .orderBy(desc(adminMessages.timestamp), desc(adminMessages.id))
      .limit(1);
  ...
  async createAdminMessage(data: {
      ...
      try {
        ...
        if (audioBuffer && audioBuffer.length > 0) {
          ...
          const transcription = await transcribeAudioCostOptimized(audioBuffer, {
            ...
            mimeType: messageData.mediaMimeType || "audio/ogg",
            language: "pt",
          });
    ...
    if (messageData.mediaType === "image" && messageData.mediaUrl) {
      try {
        ...
        if (imageUrl) {
          console.log(`🖼️ [Storage Admin] Iniciando analise de imagem com Vision...`);
...
Responda de forma concisa (máximo 3 frases) descrevendo o que você vê.`;
          ...
          const imageDescription = await analyzeImageWithMistral(imageUrl, [
            analysisPrompt,
            [
              "Complemento obrigatorio de OCR/contexto neutro:",
              "- Se for imagem/lista/print de CATALOGO ou PEDIDO: extraia ate 10 itens visiveis em ordem, incluindo codigo, nome, preco, quantidade, tamanho e acabamento quando aparecerem. Nao pare em 3 itens.",
              "- Se for print/foto com DADOS DO CLIENTE, ENDERECO, DOCUMENTO, CONTATO ou pedido para entrega: extraia nome completo, CPF/CNPJ/documento, telefone, rua, numero, complemento, bairro, cidade, UF/estado e CEP quando visiveis.",
              "- Nao confunda CPF/CNPJ/documento com valor de pagamento. Valor monetario precisa ter R$ ou formato claro de dinheiro.",
              "- Preserve texto/OCR util e diga campo incerto em vez de inventar.",
            ].join("\n"),
          ].join("\n\n"));
  ...
  async updateAdminMessage(id: string, data: Partial<{
    text: string;
    status: string;
    mediaType: string;
    mediaUrl: string;
  ...
  async toggleAdminConversationAgent(
    conversationId: string,
    enabled: boolean,
  ...
  async toggleAdminConversationFollowup(
    conversationId: string,
    active: boolean,
    options?: { manual?: boolean; resetToStageZero?: boolean },
  ): Promise<any> {
    const current = await this.getAdminConversation(conversationId);
  ...
  async resetClientByPhone(phoneNumber: string): Promise<{
    ...
    try {
      ...
      if (user) {
        ...
        const businessAgentResult = await db
          .delete(businessAgentConfigs)
          .where(eq(businessAgentConfigs.userId, user.id));
        ...
        if (result.mediaDeleted > 0) {
          console.log(`🗑️ [RESET CLIENT] ${result.mediaDeleted} mídias do agente excluídas`);
        }
        ...
        try {
          ...
          if (user?.id) {
            ...
            if (deleteByIdError) {
              ...
              console.log(`🗑️ [RESET CLIENT] Usuário Auth excluído por user.id: ${user.id}`);
            }
          }
  ...
  private normalizePhoneForComparison(phoneNumber: string): string[] {
    const variations = buildPhoneComparisonKeys(phoneNumber);
    console.log(`📞 [EXCLUSION] Normalizando número ${phoneNumber} -> chaves: [${variations.join(', ')}]`);
    return variations;
  }
  ...
  async getConversationsByTag(tagId: string, connectionId: string): Promise<Conversation[]> {
    ...
    return await this.attachAssignedMemberMetadata(result.map(r => r.conversation)) as Conversation[];
  }
  ...
  async addTagsToConversations(conversationIds: string[], tagIds: string[]): Promise<void> {
    ...
    const affectedConnections = await db
      .selectDistinct({ connectionId: conversations.connectionId })
      .from(conversations)
  ...
  async getConversationsWithTags(connectionId: string, limit?: number, offset?: number): Promise<{ data: (Conversation & { tags: Tag[] })[]; total: number }> {
    ...
    // ⚡ OTIMIZAÇÃO: Cache do COUNT por 2min, com invalidação automática em mudanças de conversa/tag.
    ...
    if (total === null) {
      ...
      memoryCache.set(countCacheKey, total, 120000);
    }
    ...
    const allConversations = await this.attachAssignedMemberMetadata(await query);
    ...
    if (limit == null) {
      ...
      const cacheKey = `convWithTags:${connectionId}:page0:${limit}`;
      memoryCache.set(cacheKey, result, 45000);
    }
    return result;
  }

  async getAttentionQueue(filters: AttentionQueueFilters): Promise<AttentionQueueResult> {
  ...
  async suspendUser(
    ...
    if (agentConfig) {
      await db.update(aiAgentConfig).set({ isActive: false, updatedAt: now }).where(eq(aiAgentConfig.userId, userId));
    }

    // 4. Desativar IA nas conexoes e parar follow-ups pendentes do tenant suspenso
    await db.update(whatsappConnections)
      .set({ aiEnabled: false, updatedAt: now })
      .where(eq(whatsappConnections.userId, userId));
    ...
    await db.update(conversations)
      .set({
        followupActive: false,
        nextFollowupAt: null,
        followupDisabledReason: "Conta suspensa por violacao de politicas",
        updatedAt: now,
      })
      .where(sql`${conversations.connectionId} IN (
        SELECT id FROM ${whatsappConnections} WHERE ${whatsappConnections.userId} = ${userId}
      )`);
  ...
  async updateAudioConfig(
    userId: string,
    data: Partial<{ isEnabled: boolean; voiceType: string; responseMode: AudioResponseMode; speed: string }>
  ): Promise<AudioConfig> {
  ...
  async incrementAudioMessageCounter(userId: string): Promise<{ count: number; limit: number; canSend: boolean }> {
    ...
    if (!counter) {
      ...
      const [newCounter] = await db.insert(audioMessageCounter).values({
        ...
        date: today,
        count: 1,
        dailyLimit: DEFAULT_AUDIO_DAILY_LIMIT,
      }).returning();
      return { count: 1, limit: DEFAULT_AUDIO_DAILY_LIMIT, canSend: true };
    }
    ...
    if (counter.count >= limit) {
      return { count: counter.count, limit, canSend: false };
    }
    
    // Incrementar contador
    const [updated] = await db.update(audioMessageCounter)
      .set({ count: counter.count + 1, dailyLimit: limit, updatedAt: new Date() })
      .where(eq(audioMessageCounter.id, counter.id))
      .returning();
    
    return { count: updated.count, limit, canSend: true };
  }

  /**
   * Verificar se usuário pode enviar mais áudios hoje
   */
  async canSendAudio(userId: string): Promise<{ canSend: boolean; remaining: number; limit: number; used: number; isUnlimited: boolean }> {
    ...
    if (config && !config.isEnabled) {
      return { canSend: false, remaining: 0, limit: DEFAULT_AUDIO_DAILY_LIMIT, used: 0, isUnlimited: false };
    }
    ...
    if (!counter) {
      return {
        canSend: true,
        remaining: DEFAULT_AUDIO_DAILY_LIMIT,
        limit: DEFAULT_AUDIO_DAILY_LIMIT,
        used: 0,
        isUnlimited: false,
      };
    }
    ...
    return { canSend: remaining > 0, remaining, limit, used: counter.count, isUnlimited: false };
  }
  ...
  async getConversationAgentAudioCountToday(userId: string, conversationId: string): Promise<number> {
    ...
    const result = await db.execute(sql`
      ...
      INNER JOIN ${whatsappConnections}
        ON ${whatsappConnections.id} = ${conversations.connectionId}
      WHERE ${messages.conversationId} = ${conversationId}
        AND ${whatsappConnections.userId} = ${userId}
        AND ${messages.fromMe} IS TRUE
        AND ${messages.isFromAgent} IS TRUE
        AND COALESCE(${messages.mediaType}, '') ILIKE 'audio%'
        AND ${messages.createdAt} >= CURRENT_DATE
        AND ${messages.createdAt} < (CURRENT_DATE + INTERVAL '1 day')
    `);
  ...
  async getAdminNotificationConfig(adminId: string): Promise<any | undefined> {
    try {
      ...
      `);
      return sanitizeAdminNotificationConfig(result.rows[0] as any);
    } catch (error) {
  ...
  async updateAdminNotificationConfig(adminId: string, data: any): Promise<void> {
    try {
      ...
      if (Array.isArray(sanitizedData.welcomeMessageVariations)) {
        welcomeVariationsArray = sanitizedData.welcomeMessageVariations.map((v: any) => 
  ...
  async getRunningAdminBroadcasts(): Promise<any[]> {
    try {
      const result = await db.execute(sql`
        SELECT * FROM admin_broadcasts
  ...
  async updateAdminBroadcast(adminId: string, id: string, data: any): Promise<void> {
    try {
      ...
      if (data.targetFilter !== undefined || data.totalRecipients !== undefined) {
  ...
  async updatePendingAIResponseMessages(
    conversationId: string,
    messages: string[],
    executeAt: Date,
    options?: { resetRetry?: boolean },
  ...
  async replacePendingAIResponseMessages(conversationId: string, messages: string[]): Promise<void> {
    try {
      await db.execute(sql`
        UPDATE pending_ai_responses
        SET messages = ${JSON.stringify(messages)},
        ...
        WHERE conversation_id = ${conversationId} AND status = 'pending'
      `);
      console.log(`📝 [PERSISTENT TIMER] Payload normalizado para conversation ${conversationId}`);
    } catch (error) {
  ...
  async markPendingAIResponseCompleted(conversationId: string, guard?: PendingAIResponseMutationGuard): Promise<void> {
    try {
      await db.execute(sql`
        UPDATE pending_ai_responses
  ...
  async markPendingAIResponseFailed(
    conversationId: string,
    reason: string,
    lastError?: string,
    guard?: PendingAIResponseMutationGuard,
  ...
  async markPendingAIResponseSkipped(
    conversationId: string,
    reason: string,
    guard?: PendingAIResponseMutationGuard,
  ): Promise<void> {
    ...
    try {
      await this.ensurePendingAiSkippedSupportChecked();
  ...
  async resetPendingAIResponseForRetry(
    conversationId: string,
    delaySec: number = 30,
    guard?: PendingAIResponseMutationGuard,
    options?: { resetRetryCount?: boolean; lastError?: string | null },
    ...
    try {
      ...
      await db.execute(sql`
        ...
        SET status = 'pending',
            ...
            retry_count = ${options?.resetRetryCount ? 0 : sql`COALESCE(retry_count, 0) + 1`},
            failure_reason = NULL,
...
async getCompletedTimersWithoutResponse(): Promise<Array<{
    ...
    try {
      // Buscar timers "completed" cujo cliente ainda segue sem resposta confirmada.
      // A janela considera a última mensagem do cliente, não o updated_at do timer,
      // para não perder conversas onde a fila falhou antes e o cliente escreveu horas depois.
      const result = await db.execute(sql`
        SELECT
          p.conversation_id,
          p.user_id,
          ...
          p.jid_suffix,
          p.messages,
          customer.last_customer_at,
          agent.last_agent_at,
          owner_reply.last_owner_at
        FROM pending_ai_responses p
        JOIN conversations c
          ON c.id = p.conversation_id
        JOIN LATERAL (
          SELECT customer_msg.timestamp AS last_customer_at
          FROM messages customer_msg
          WHERE customer_msg.conversation_id = p.conversation_id
            AND customer_msg.from_me = false
            AND customer_msg.timestamp >= NOW() - INTERVAL '24 hours'
          ORDER BY customer_msg.timestamp DESC
          LIMIT 1
        ) customer ON true
        LEFT JOIN LATERAL (
          SELECT agent_msg.timestamp AS last_agent_at
          FROM messages agent_msg
          WHERE agent_msg.conversation_id = p.conversation_id
            AND agent_msg.from_me = true
            AND agent_msg.is_from_agent = true
            AND ${confirmedOutgoingMessageStatusSql(sql`agent_msg.status`)}
          ORDER BY agent_msg.timestamp DESC
          LIMIT 1
        ) agent ON true
        LEFT JOIN LATERAL (
          SELECT owner_msg.timestamp AS last_owner_at
          FROM messages owner_msg
          WHERE owner_msg.conversation_id = p.conversation_id
            AND owner_msg.from_me = true
            AND COALESCE(owner_msg.is_from_agent, false) = false
          ORDER BY owner_msg.timestamp DESC
          LIMIT 1
        ) owner_reply ON true
        WHERE p.status = 'completed'
          AND (owner_reply.last_owner_at IS NULL OR owner_reply.last_owner_at < customer.last_customer_at)
          AND (agent.last_agent_at IS NULL OR customer.last_customer_at > agent.last_agent_at)
        ORDER BY p.updated_at DESC
        LIMIT 80
      `);
      
      if (result.rows && result.rows.length > 0) {
        const recoverableRows = (result.rows as any[]).filter((row) =>
        ...
        if (recoverableRows.length > 0) {
          console.log(`🚨 [AUTO-RECOVERY] Encontrados ${recoverableRows.length} timers "completed" sem resposta confirmada`);
        }
