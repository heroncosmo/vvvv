...
export class AIInterpreter {
  // Legacy AI interpreter disabled; Codex owns semantic flow decisions.
...
- Extraia dados relevantes (números, nomes, etc)`;
    ...
    try {
      // Legacy provider call disabled.
      void systemPrompt;
      void message;
      console.warn("[UnifiedFlowEngine] Legacy LLM intent detection disabled.");
      const response = { choices: [] as Array<{ message?: { content?: string } }> };
...
export class AIHumanizer {
  // Legacy humanizer disabled; Codex owns public wording.
...
export class UnifiedFlowEngine {
  ...
  constructor(config: FlowConfig) {
    this.config = config;
    // Legacy provider config is disabled in this engine.
  ...
  async processMessage(
    ...
    void state;
    void options;
    console.warn("[UnifiedFlowEngine] Legacy flow processing disabled; returning null before local intent routing.");
    return null;
