/**
 * PROVIDER ROUTES - ROTAS PARA O SISTEMA DE AGENDAMENTOS DO PRESTADOR
 */
...
import { createProviderAppointment } from "./providerAIService";
import {
  getProviderGoogleCalendarState,
...
export function registerProviderRoutes(app: Express): void {
  console.log("[Provider] Registrando rotas do prestador...");

  // CONFIGURACAO DO PRESTADOR
  // GET - Obter configuracao do prestador
  app.get("/api/provider/config", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      const googleCalendarState = await getProviderGoogleCalendarState(userId);
      ...
      if (!data) {
        ...
        return res.json({
          ...
          welcome_message: "Ola {cliente_nome}! Bem-vindo(a) ao nosso atendimento! Como posso ajudar voce hoje?",
          ...
          google_calendar_source: "provider_config",
        });
      }
      ...
      console.error("❌ [Provider] Error fetching provider config:", error);
      res.status(500).json({ message: "Failed to fetch provider config" });
    }
  });

  // PUT - Atualizar configuracao do prestador
  app.put("/api/provider/config", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      const body = req.body;
      ...
      console.log(`✅ [Provider] Config atualizada para user: ${userId}`);
      res.json(result);
    } catch (error) {
      console.error("❌ [Provider] Error updating provider config:", error);
      res.status(500).json({ message: "Failed to update provider config" });
    }
  });
  ...
  app.get("/api/provider/services", isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.error("❌ [Provider] Error fetching services:", error);
  ...
  app.post("/api/provider/services", isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.log(`✅ [Provider] Serviço criado: ${name}`);
      res.json(data);
    } catch (error) {
      console.error("❌ [Provider] Error creating service:", error);
  ...
  app.put("/api/provider/services/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.error("❌ [Provider] Error updating service:", error);
  ...
  app.delete("/api/provider/services/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.error("❌ [Provider] Error deleting service:", error);
  ...
  app.get("/api/provider/professionals", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      ...
      console.error("❌ [Provider] Error fetching professionals:", error);
      res.status(500).json({ message: "Failed to fetch professionals" });
    }
  });

  // POST - Criar profissional
  app.post("/api/provider/professionals", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      ...
      console.log(`✅ [Provider] Profissional criado: ${name}`);
      res.json(data);
    } catch (error) {
      console.error("❌ [Provider] Error creating professional:", error);
      res.status(500).json({ message: "Failed to create professional" });
    }
  });

  // PUT - Atualizar profissional
  app.put("/api/provider/professionals/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      const { id } = req.params;
      const { name, bio, avatar_url, is_active } = req.body;
      ...
      console.error("❌ [Provider] Error updating professional:", error);
      res.status(500).json({ message: "Failed to update professional" });
    }
  });

  // DELETE - Remover profissional
  app.delete("/api/provider/professionals/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      const { id } = req.params;
      ...
      console.error("❌ [Provider] Error deleting professional:", error);
  ...
  app.post("/api/provider/appointments", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      const body = req.body || {};
      ...
      const result = await createProviderAppointment(
        userId,
        "",
        {
          clientName,
      ...
      console.error("❌ [Provider] Error creating manual appointment:", error);
      res.status(500).json({ message: "Failed to create appointment" });
    }
  });

  // GET - Listar agendamentos
  app.get("/api/provider/appointments", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      ...
      console.error("❌ [Provider] Error fetching appointments:", error);
      res.status(500).json({ message: "Failed to fetch appointments" });
    }
  });

  // GET - Buscar agendamento por ID
  app.get("/api/provider/appointments/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      const { id } = req.params;
      ...
      console.error("❌ [Provider] Error fetching appointment:", error);
      res.status(500).json({ message: "Failed to fetch appointment" });
    }
  });

  // PUT - Atualizar status do agendamento
  app.put("/api/provider/appointments/:id/status", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      const { id } = req.params;
        ...
        if (["cancelled", "completed", "no_show"].includes(status)) {
          if (googleEventId) {
            ...
            if (!removeResult.success) {
              console.error(`[Provider] Falha ao remover evento ${googleEventId} do Google Calendar:`, removeResult.error);
            }
          }
      ...
      console.log(`[Provider] Agendamento ${id} atualizado para: ${status}`);
      res.json(responseData);
    } catch (error) {
      console.error("[Provider] Error updating appointment:", error);
      res.status(500).json({ message: "Failed to update appointment" });
    }
  });
  // DELETE - Cancelar agendamento
  app.delete("/api/provider/appointments/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      const { id } = req.params;
      ...
      console.error("❌ [Provider] Error cancelling appointment:", error);
      res.status(500).json({ message: "Failed to cancel appointment" });
    }
  });

  // GET - Horários disponíveis para uma data
  app.get("/api/provider/available-slots", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
      ...
      console.error("❌ [Provider] Error fetching available slots:", error);
      res.status(500).json({ message: "Failed to fetch available slots" });
    }
  });

  // GET - Estatisticas do prestador
  ...
  app.post("/api/provider/google-calendar/connect", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
  ...
  app.post("/api/provider/google-calendar/disconnect", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
  ...
  app.put("/api/provider/google-calendar/calendar", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
  ...
  app.get("/api/provider/google-calendar/status", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
  ...
  app.get("/api/provider/google-calendar/events", isAuthenticated, async (req: any, res) => {
    try {
      const userId = getUserId(req);
  ...
  console.log("[Provider] Rotas do prestador registradas com sucesso!");
}
