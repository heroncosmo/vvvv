...
async function sendOwnerNotificationRow(
  ...
  const skip = async (
    status: "skipped_disabled" | "skipped_active_plan",
    reason: string,
  ) => {
    await db.execute(sql`
  ...
  if (!isOwnerNotificationModuleEnabled(notificationType, config)) {
    return skip("skipped_disabled", "Envio bloqueado: módulo desativado no painel.");
  }

  if (isBillingNotificationType(notificationType) && (await hasActiveSubscriptionForUser(row.user_id))) {
    return skip("skipped_active_plan", "Envio bloqueado: cliente já possui assinatura ativa.");
  }
...
export async function getOwnerWorkspaceStats(ownerUserId: string) {
  ...
  const overduePayments = users.filter((user) => {
    ...
    return user.subscription_status !== "active" && !!dueDate && dueDate.getTime() < Date.now();
  }).length;
...
export async function reorganizeOwnerWorkspaceAgenda(ownerUserId: string) {
  ...
  for (const user of managedUsers) {
    ...
    if (!hasActivePlan && config.paymentReminderEnabled && dueDate) {
    ...
    if (!hasActivePlan && config.overdueReminderEnabled && dueDate) {
