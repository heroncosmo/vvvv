...
function buildRemovedConnectionProviderConfig(
  ...
  return {
    ...providerConfig,
    [USER_REMOVED_CONNECTION_FLAG]: true,
    removedByUser: true,
    removalMode: "preserve_history",
    removedAt,
    previousConnectionType: connection.connectionType || null,
    previousProviderStatus: connection.providerStatus || null,
  };
}

async function removeConnectionFromAppPreservingHistory(connection: WhatsappConnection): Promise<void> {
  ...
  await storage.updateConnection(connection.id, {
    ...
    publicApiTokenHash: null,
    publicApiTokenPreview: null,
    publicApiLastUsedAt: null,
    providerConfig: buildRemovedConnectionProviderConfig(connection, removedAt),
  } as any);
}
...
export async function findSamePhoneSiblingConnection(
  ...
  return siblings.find((candidate) => {
    ...
    if (!isAppVisibleWhatsappConnection(candidate)) {
      return false;
    }
...
export async function findReusableDisconnectedConnectionForCreation(
  ...
  const connections = (await storage.getConnectionsByUserId(userId)).filter(
    (connection) => isAppVisibleWhatsappConnection(connection),
  );
...
export async function deleteConnectionSafely(connectionId: string): Promise<{
  ...
  if (historyCount > 0) {
    await removeConnectionFromAppPreservingHistory(connection as WhatsappConnection);
    memoryCache.invalidate(`connByUser:${connection.userId}`);
    memoryCache.invalidate(`api:wa-conn:${connection.userId}`);
    memoryCache.invalidate(`api:wa-conn:${connection.userId}:default`);
    return { deleted: true, preservedHistory: true };
