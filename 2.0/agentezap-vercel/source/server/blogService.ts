...
import { createSupabaseServiceClient, getSupabaseUrl } from "./supabaseService";
import {
  blogAssetImages,
  blogAuthorProfiles,
  blogContextPacks,
...
const BLOG_DEFAULT_ASSET_PATH = "/blog-assets/default-blog.svg";
...
function getBlogStorageObjectPath(fileName: string): string {
  return `posts/${fileName}`;
}

function getBlogStoragePublicUrl(objectPath: string): string | null {
  const supabaseUrl = normalizeUrlBase(getSupabaseUrl());
  ...
  return `${supabaseUrl}/storage/v1/object/public/${encodeURIComponent(BLOG_STORAGE_BUCKET)}/${encodedObjectPath}`;
}

function getDefaultBlogAssetUrl(config?: Pick<BlogConfig, "baseUrl"> | null): string {
  if (config?.baseUrl) {
    return new URL(BLOG_DEFAULT_ASSET_PATH, config.baseUrl).toString();
  }
  return BLOG_DEFAULT_ASSET_PATH;
}

function normalizeLegacyBlogAssetUrl(value: string, config: Pick<BlogConfig, "baseUrl">): string | null {
  try {
    const parsed = new URL(value, config.baseUrl);
    if (!parsed.pathname.startsWith("/uploads/blog-assets/")) {
      return null;
    }

    const fileName = decodeURIComponent(parsed.pathname.slice("/uploads/blog-assets/".length));
    if (!fileName || fileName === "default-blog.svg") {
      return getDefaultBlogAssetUrl(config);
    }

    return getBlogStoragePublicUrl(getBlogStorageObjectPath(fileName)) || getDefaultBlogAssetUrl(config);
  } catch {
    return null;
  }
}

function resolveCanonicalBlogAssetUrl(value: string, config: BlogConfig): string {
  const previewUrl = resolveLocalPreviewAssetUrl(value, config);
  return normalizeLegacyBlogAssetUrl(previewUrl, config) || previewUrl;
}
...
const OFFICIAL_SOURCE_LIBRARY: Record<string, BlogReferenceItem[]> = {
  ...
  "comparativos": [
    ...
    { label: "Google Images best practices", href: "https://developers.google.com/search/docs/appearance/google-images", sourceType: "external-official", description: "Boas praticas para imagens rastreaveis em conteudo publicado." },
  ],
  "nichos": [
...
async function callBlogJsonTask<T>(input: {
  ...
  void input;
  throw new Error("blog_json_task_requires_codex_contract");
}
...
async function uploadBlogAssetToStorage(fileName: string, content: Buffer, mimeType: string) {
  ...
  const supabase = createSupabaseServiceClient();
  const objectPath = getBlogStorageObjectPath(fileName);
...
async function ensureSeoImageVariants(post: BlogPost, config: BlogConfig): Promise<string[]> {
  ...
  for (const variant of variants) {
    ...
    try {
      ...
      urls.push(getDefaultBlogAssetUrl(config));
    }
  }
...
async function saveImageAsset(post: BlogPost, payload: {
  ...
  let publicUrl = getDefaultBlogAssetUrl();
...
function collectPostImageUrls(post: BlogPost, config: BlogConfig): string[] {
  ...
  const addUrl = (value: unknown) => {
    ...
    urls.add(resolveCanonicalBlogAssetUrl(value, config));
  };
  ...
  if (urls.size === 0) {
    addUrl(BLOG_DEFAULT_ASSET_PATH);
  }
...
function resolveLocalPreviewAssetUrl(value: string, config: BlogConfig): string {
  ...
  try {
    const parsed = new URL(value, config.baseUrl);
    if (
      parsed.pathname.startsWith("/blog-imagens/") ||
      parsed.pathname.startsWith("/uploads/") ||
      parsed.pathname.startsWith(BLOG_DEFAULT_ASSET_PATH)
    ) {
...
function resolvePostImageUrl(post: Pick<BlogPost, "slug" | "heroImageUrl">, config: BlogConfig, variant: "16x9" | "4x3" | "1x1" = "16x9"): string {
  ...
  if (post.heroImageUrl) return resolveCanonicalBlogAssetUrl(post.heroImageUrl, config);
  return BLOG_DEFAULT_ASSET_PATH;
}
...
function buildLayoutHtml(input: { title: string; description: string; canonicalUrl: string; ogImage?: string | null; structuredData?: Record<string, unknown> | Record<string, unknown>[]; body: string; ogType?: "article" | "website" | "profile"; extraMeta?: string; }) {
  ...
  const ogImage = input.ogImage ? new URL(input.ogImage, pageUrl).toString() : new URL(BLOG_DEFAULT_ASSET_PATH, pageUrl).toString();
  ...
  const body = `<div class="page-bg"><div class="shell">
    ...
    <section class="split-grid">
      ...
      <div class="card">
        ...
        <p>Para imagem, o fluxo usa os provedores de imagem configurados e, se eles falharem, cai para um template local rastreavel.</p>
      </div>
    </section>
    ${renderBlogFooter(config, categories, tags)}
  </div></div>`;
