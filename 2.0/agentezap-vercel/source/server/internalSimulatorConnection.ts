...
type LooseConnectionLike = ConnectionLike & Record<string, unknown>;
...
export const USER_REMOVED_CONNECTION_TYPE = "removed";
export const USER_REMOVED_CONNECTION_STATUS = "removed";
export const USER_REMOVED_CONNECTION_FLAG = "removedFromApp";
...
function normalizeProviderConfigSource(
...
function readConnectionString(
  connection: LooseConnectionLike | null | undefined,
  camelKey: keyof ConnectionLike,
  snakeKey: string,
): string {
  if (!connection) {
    return "";
...
function readConnectionProviderConfig(
  connection: LooseConnectionLike | null | undefined,
...
function getProviderConfigFlag(
...
export function hasInternalSimulatorIdentity(connection?: ConnectionLike | null): boolean {
  ...
  return (
    ...
    normalizeLower(connection.connectionName) ===
      INTERNAL_SIMULATOR_CONNECTION_NAME.toLowerCase() ||
    normalizeProviderConfigSource(
      (connection.providerConfig as Record<string, unknown> | null | undefined) || null,
    ) === INTERNAL_SIMULATOR_SOURCE
  );
}

export function isInternalOnlySimulatorConnection(
  ...
  const hasOperationalSignal =
    ...
    normalizeLower(connection?.providerStatus) === "connected";

  return !hasOperationalSignal;
}

export function isUserRemovedConnection(connection?: ConnectionLike | null): boolean {
  if (!connection) {
    return false;
  }

  const looseConnection = connection as LooseConnectionLike;
  const providerConfig = readConnectionProviderConfig(looseConnection);

  return (
    normalizeLower(readConnectionString(looseConnection, "connectionType", "connection_type")) === USER_REMOVED_CONNECTION_TYPE ||
    normalizeLower(readConnectionString(looseConnection, "providerStatus", "provider_status")) === USER_REMOVED_CONNECTION_STATUS ||
    getProviderConfigFlag(providerConfig, USER_REMOVED_CONNECTION_FLAG) ||
    getProviderConfigFlag(providerConfig, "removedByUser")
  );
}

export function isAppVisibleWhatsappConnection(connection?: ConnectionLike | null): boolean {
  return !isInternalOnlySimulatorConnection(connection) && !isUserRemovedConnection(connection);
}

export function shouldNormalizeRealWhatsappConnection(
  connection?: ConnectionLike | null,
): boolean {
  return hasInternalSimulatorIdentity(connection) && !isInternalOnlySimulatorConnection(connection);
}

export function buildInternalSimulatorConnectionInsert(
  userId: string,
...
export function buildRealWhatsappConnectionNormalization(
  ...
  const providerConfig =
    ...
    typeof connection.providerConfig === "object" &&
    !Array.isArray(connection.providerConfig)
