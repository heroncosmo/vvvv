...
export interface AccessEntitlement {
  ...
  isPendingReceiptAccess: boolean; // payment/receipt exists but still needs approval
...
function isPendingPaymentStatus(status: unknown): boolean {
  return ["pending", "pending_pix", "pending_payment"].includes(
...
export function invalidateUserAccessCaches(userId: string): void {
  invalidateEntitlementCache(userId);
  memoryCache.invalidate(`api:access-status:${userId}`);
}
...
async function _computeEntitlement(userId: string): Promise<AccessEntitlement> {
  ...
  const hasPendingReceiptAccess =
    Boolean(subscription?.pendingReceipt) && subscriptionStatus !== "cancelled";
  const hasPendingPaymentAccess = isPendingPaymentStatus(subscriptionStatus);
  ...
  if (saasHasActive) {
    return {
      hasActiveSubscription: true,
      isExpired: false,
      isPendingReceiptAccess: hasPendingReceiptAccess || hasPendingPaymentAccess,
      source: 'saas',
