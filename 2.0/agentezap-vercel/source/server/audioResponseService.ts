...
const MAX_AGENT_AUDIO_PER_CONVERSATION_PER_DAY = 12;
const MIN_AGENT_AUDIO_PER_CONVERSATION_PER_DAY = 5;

export function resolveAgentAudioPerConversationDailyLimit(totalDailyLimit: number): number {
  const total = Number.isFinite(totalDailyLimit) && totalDailyLimit > 0 ? Math.floor(totalDailyLimit) : 30;
  const proportionalLimit = Math.ceil(total * 0.4);
  return Math.max(
    1,
    Math.min(
      total,
      MAX_AGENT_AUDIO_PER_CONVERSATION_PER_DAY,
      Math.max(MIN_AGENT_AUDIO_PER_CONVERSATION_PER_DAY, proportionalLimit),
    ),
  );
}
...
export async function getAudioResponseSettings(
  ...
  try {
    ...
    if (decision.shouldGenerateAudio && conversationId) {
      const conversationLimit = resolveAgentAudioPerConversationDailyLimit(usage.limit);
      const conversationUsed = await storage.getConversationAgentAudioCountToday(userId, conversationId);

      if (conversationUsed >= conversationLimit) {
        console.log(
          `[TTS-RESPONSE] Limite de audio por conversa atingido ` +
            `(${conversationUsed}/${conversationLimit}) para conversa ${conversationId}; fallback para texto`,
        );
        return {
          responseMode: decision.responseMode,
          shouldSendText: true,
