...
function describeGrupoOlxListingForReply(lead: GrupoOlxNormalizedLead): string {
  return lead.listingTitle || (lead.clientListingId ? `codigo ${lead.clientListingId}` : "que voce viu");
}
...
export function renderGrupoOlxAutoReply(template: string, lead: GrupoOlxNormalizedLead): string {
  ...
  const listingLabel = describeGrupoOlxListingForReply(lead);
  const replacements: Record<string, string> = {
    nome: lead.name ?? "cliente",
    portal: lead.portalSource,
    lead_type: lead.leadType,
    imovel_codigo: lead.clientListingId ?? "",
    imovel_titulo: listingLabel,
    imovel: listingLabel,
    cidade: lead.city ?? "",
