...
function responseLooksLikeCommercialDeliveryOrSalesOffer(text: string): boolean {
  const folded = normalizeFold(text);
  return /\b(prazo|entrega|frete|kit|unidade|pagamento|pix|debito|credito|pedido|produto|brinde|confirmacao)\b/.test(folded);
}
...
export function enforceBrazilTemporalConsistency(params: {
  ...
  if (
    ...
    relativeWeekdayConflict &&
    responseLooksLikeCommercialDeliveryOrSalesOffer(responseText)
  ) {
    return responseText;
  }
