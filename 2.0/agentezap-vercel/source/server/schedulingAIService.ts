...
async function extractSchedulingFieldsLLM(
  ...
  runtimeContext: { userId: string; conversationId: string; customerPhone: string },
  ...
  try {
    ...
    console.error("[SchedulingAI] Extracao legada desativada:", error);
    return { intent: "general" };
  }
}
...
export async function generateSchedulingAiResponse(
  ...
  const runtimeContext = { userId, conversationId, customerPhone };
  const previousDate = state.date;
  const previousServiceSummary = buildServiceSummary(state.selectedServices);

  const extracted = await extractSchedulingFieldsLLM(message, history, data, state, runtimeContext);
  ...
  if (shouldConfirm) {
    if (!hasAllBookingData) {
      state.awaitingConfirmation = false;
      return {
        text: await generateSchedulingConversationalReply(message, history, data, state, "Faltam dados para confirmar o agendamento. Pergunte o próximo dado necessário.", runtimeContext),
      };
    }
  ...
  if (extracted.intent === "check_availability") {
    ...
    if (!targetDate) {
      return {
        text: await generateSchedulingConversationalReply(message, history, data, state, "O cliente quer consultar disponibilidade, mas ainda não informou a data. Pergunte qual dia ele prefere.", runtimeContext),
      };
    }
  ...
  if (isBookingIntent) {
    if (!hasService) {
      return {
        text: await generateSchedulingConversationalReply(message, history, data, state, `O cliente quer agendar mas ainda não definiu o serviço. Serviços disponíveis: ${data.services.map((service) => service.name).join(", ")}. Pergunte qual serviço ele quer.`, runtimeContext),
      };
    }

    if (data.config.use_professionals && data.professionals.length > 1 && !state.professional) {
      return {
        text: await generateSchedulingConversationalReply(message, history, data, state, `O cliente já escolheu o serviço ${buildServiceSummary(state.selectedServices)}. Profissionais disponíveis: ${data.professionals.map((professional) => professional.name).join(", ")}. Pergunte com quem ele prefere agendar.`, runtimeContext),
      };
    }
    ...
    if (!hasDate) {
      ...
      return {
        text: await generateSchedulingConversationalReply(message, history, data, state, `O cliente já escolheu o serviço ${buildServiceSummary(state.selectedServices)}. Pergunte qual dia ele prefere.`, runtimeContext),
      };
    }
  ...
  if (extracted.intent === "info_services" || extracted.intent === "info_prices") {
    ...
    return {
      text: await generateSchedulingConversationalReply(message, history, data, state, `Informe os serviços e preços disponíveis: ${catalogSummary}`, runtimeContext),
    };
  }

  if (extracted.intent === "info_hours") {
    return {
      text: await generateSchedulingConversationalReply(message, history, data, state, `Informe os horários de funcionamento: ${buildSchedulingHoursText(data.config)}`, runtimeContext),
    };
  }

  return {
    text: await generateSchedulingConversationalReply(message, history, data, state, "", runtimeContext),
  };
}
