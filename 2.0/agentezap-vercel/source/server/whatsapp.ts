...
async function internalSendMessageRaw(
  ...
  jid: string, 
  text: string, 
  options?: { isFromAgent?: boolean; conversationId?: string; connectionId?: string; source?: "owner" | "agent" | "followup" | "system"; fastLane?: boolean }
): Promise<string | null> {
...
export async function sendMessage(
    ...
    if (!isWhatsAppGatewayRuntime() && await resolveWhatsAppConnectionOwner(connection) === "gateway") {
      ...
      if ((result as any)?.success !== false) {
        ...
        const updatePayload: Partial<Conversation> = {
          lastMessageText: normalizeOutboundTextForCustomer(text),
          lastMessageTime: new Date(),
          lastMessageFromMe: true,
          hasReplied: true,
          unreadCount: 0,
  ...
  if (isOfficialCoexistenceConnection(connection)) {
    ...
    const queueResult = await messageQueueService.enqueue(userId, jid, normalizedText, {
    ...
    try {
      ...
      const pendingConfirmedAt = consumePendingOutgoingMessageConfirmation(messageId);
