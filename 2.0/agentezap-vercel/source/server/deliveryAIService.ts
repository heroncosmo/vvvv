...
import { applyVicosaPizzaResponseGuard } from "./vicosaPizzaResponseGuard";
...
function getHalfHalfItemCategoryName(item: HalfHalfPricedItem): string {
...
function getHalfHalfCategory(
  ...
  for (const item of items) {
    const itemCategoryName = getHalfHalfItemCategoryName(item);
    const category = deliveryData.categories.find(cat => normalizeCategoryText(cat.name) === normalizeCategoryText(itemCategoryName));
    if (category) return category;
  }
...
function halfHalfSizeOptionMatches(optionName: string, sizeCode?: string | null): boolean {
  const sizeKey = normalizeHalfHalfSizeKey(sizeCode);
  if (!sizeKey) return false;

  const normalized = normalizeTextForMatch(optionName);
  if (sizeKey === "P") {
    return normalized === "p" || normalized.includes("pequen");
  }
  if (sizeKey === "M") {
    return normalized === "m" || normalized.includes("medi");
  }
  return normalized === "g" || normalized.includes("grand");
}

function getHalfHalfSizeOptionPrice(item: HalfHalfPricedItem, sizeCode?: string | null): number | null {
  if (!normalizeHalfHalfSizeKey(sizeCode)) return null;

  for (const group of item.options || []) {
    if (!isSizeOptionGroup(group.name)) continue;

    const selectedOption = group.options?.find(option => halfHalfSizeOptionMatches(option.name, sizeCode));
    const selectedPrice = parseOptionalPriceValue(selectedOption?.price);
...
function resolveHalfHalfPrice(params: {
  deliveryData: DeliveryData;
  categoryContext?: string | null;
  item1: HalfHalfPricedItem;
  item2: HalfHalfPricedItem;
  ...
  const item1SizePrice = getHalfHalfSizeOptionPrice(item1, sizeCode);
  const item2SizePrice = getHalfHalfSizeOptionPrice(item2, sizeCode);
  if (item1SizePrice !== null || item2SizePrice !== null) {
    return {
      finalPrice: Math.max(item1SizePrice ?? item1.price, item2SizePrice ?? item2.price),
      source: 'highest_item',
    };
  }
...
export function applyDeliveryResponseTenantGuards(
  userId: string,
  message: string,
  response: DeliveryAIResponse
): DeliveryAIResponse {
  const joinedBubbles = response.bubbles.join('\n\n');
  const vicosaGuard = applyVicosaPizzaResponseGuard({
    userId,
    message,
    text: joinedBubbles,
  });

  if (!vicosaGuard.applied.length) {
    return response;
  }

  return {
    ...response,
    bubbles: [vicosaGuard.text],
    mediaActions: [],
    metadata: {
      ...response.metadata,
      tenantResponseGuard: {
        ...(response.metadata as any)?.tenantResponseGuard,
        vicosaPizza: vicosaGuard.applied,
      },
    },
  };
}
  ...
  if (intent === 'HALF_HALF') {
    ...
    if (halfHalfResult.success && halfHalfResult.items.length === 2) {
      ...
      const { finalPrice, source: halfHalfPriceSource } = resolveHalfHalfPrice({
        deliveryData,
        categoryContext,
        item1: fullItem1 ?? item1,
        item2: fullItem2 ?? item2,
...
export async function processDeliveryMessage(
  ...
  const guardedResponse = applyDeliveryResponseTenantGuards(userId, message, response);
  
  console.log(`ðŸ• [DeliveryAI] Resposta gerada: ${guardedResponse.bubbles.length} bolha(s)`);
  guardedResponse.bubbles.forEach((b, i) => {
    console.log(`   Bolha ${i + 1}: ${b.substring(0, 80)}...`);
  });
  guardedResponse.bubbles = guardedResponse.bubbles.map(bubble => sanitizeDeliveryText(bubble));
  if (guardedResponse.mediaActions?.length) {
    guardedResponse.mediaActions = guardedResponse.mediaActions.map(action => ({
      ...action,
