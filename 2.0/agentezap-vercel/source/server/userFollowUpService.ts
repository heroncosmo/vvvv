...
import {
  ...
  resolveUserFollowUpSocketFromSessions,
} from "./userFollowUpConnectionState";
...
import {
  normalizeUserFollowUpAiDecisionPayload,
  type UserFollowUpAiDecision,
} from "./userFollowUpAiDecisionSchema";
import {
  canBackfillOwnerPhoneFromLinkedConnection,
  FOLLOWUP_OWNER_PHONE_LINKED_CONNECTION_SOURCE,
} from "./userFollowUpOwnerPhoneState";
import { resolveStageAfterSentLogSync } from "./userFollowUpStageSyncPolicy";
...
function normalizeFollowupConfigForRuntime(
  config: typeof followupConfigs.$inferSelect | null | undefined,
): typeof followupConfigs.$inferSelect | null {
  if (!config) {
    return null;
  }

  const parsedImportantInfo = followupImportantInfoSchema.safeParse(config.importantInfo);

  return {
    ...config,
    importantInfo: parsedImportantInfo.success ? parsedImportantInfo.data : [],
  };
}
...
const WAITING_FOR_FIRST_CUSTOMER_REPLY_REASON =
  "Aguardando primeira resposta do cliente";
const OUTGOING_CONFIRMATION_RECHECK_MS = 30 * 60 * 1000;
const OWNER_VERIFICATION_CUSTOMER_REPLY_BRIDGE_MS = Math.max(
  30 * 60 * 1000,
  Number(process.env.USER_FOLLOWUP_OWNER_VERIFICATION_CUSTOMER_BRIDGE_MS || 24 * 60 * 60 * 1000),
);
const OWNER_VERIFICATION_COMPANY_REPLY_TOLERANCE_MS = 2 * 60 * 1000;
const OWNER_VERIFICATION_CUSTOMER_REPLY_BRIDGE_MARKER = "USER_FOLLOWUP_OWNER_VERIFICATION_BRIDGE_V525";
...
function extractUserFollowUpJsonObject(rawContent: string): string | null {
...
const FOLLOWUP_SELF_CONVERSATION_REASON =
  "Contato e o proprio numero da conexao. Follow-up bloqueado para evitar envio para o proprio WhatsApp.";
const FOLLOWUP_CONNECTION_REMOVED_REASON = "Conexão removida - sem userId";
...
function getFollowUpOwnerPhoneVerifiedAt(conversation: any): Date | null {
  return parseFollowUpDate(
    conversation?.conversationOwnerPhoneVerifiedAt ||
      conversation?.ownerPhoneVerifiedAt ||
      null,
  );
}
...
function isSelfConversationForFollowUp(conversation: any, ownerPhone: string): boolean {
  const normalizedOwnerPhone = normalizePhoneDigits(ownerPhone);
  if (!normalizedOwnerPhone) {
    return false;
  }

  const contactNumber = normalizePhoneDigits(conversation?.contactNumber || "");
  return Boolean(contactNumber && contactNumber === normalizedOwnerPhone);
}
...
export class UserFollowUpService {
  ...
  private isCompanyReplyRecoveryRunning = false;
  ...
  private async backfillMissingOwnerPhonesFromLinkedConnections(
    limit: number = 5000,
    onlyUserId?: string,
  ): Promise<number> {
    const userScope = onlyUserId ? sql`AND wc.user_id = ${onlyUserId}` : sql``;
    const normalizedLimit = Math.max(1, Math.floor(Number(limit) || 5000));
    const result = await db.execute(sql`
      WITH candidate_conversations AS (
        SELECT
          c.id,
          regexp_replace(COALESCE(wc.phone_number, ''), '\\D', '', 'g') AS owner_phone,
          COALESCE(c.last_message_time, c.updated_at, c.created_at) AS sort_at
        ...
        WHERE c.followup_active = true
          ...
          AND (
            ...
            OR LOWER(COALESCE(wc.provider_status, '')) IN ('connected', 'open', 'ready', 'authenticated')
          )
      ...
      candidates AS (
        ...
        ORDER BY cc.sort_at DESC
        LIMIT ${normalizedLimit}
      )
      UPDATE conversations c
      SET owner_phone_number = candidates.owner_phone,
          owner_phone_verified_at = NOW(),
          owner_phone_source = ${FOLLOWUP_OWNER_PHONE_LINKED_CONNECTION_SOURCE},
          followup_disabled_reason = CASE
            WHEN c.followup_disabled_reason = ${FOLLOWUP_OWNER_UNVERIFIED_REASON}
              THEN NULL
            ELSE c.followup_disabled_reason
          END,
          updated_at = NOW()
      FROM candidates
    ...
    if (count > 0) {
      console.log(
        `[USER-FOLLOW-UP] ${count} conversas receberam numero dono pela conexao vinculada.`,
      );
    }
    return count;
  }
  ...
  private async pruneUnsafeScheduledFollowUps(onlyUserId?: string): Promise<void> {
    await this.backfillMissingOwnerPhonesFromLinkedConnections(5000, onlyUserId);
          ...
          followup_disabled_reason = CASE
            ...
            WHEN NOT EXISTS (
              ...
              WHERE m.conversation_id = c.id
                AND m.from_me = false
                AND (c.owner_phone_verified_at IS NULL OR COALESCE(m.timestamp, m.created_at) >= c.owner_phone_verified_at)
                AND (
              ...
              THEN ${WAITING_FOR_FIRST_CUSTOMER_REPLY_REASON}
  ...
  private isTechnicalScheduleDecisionReason(reason: unknown): boolean {
    ...
    return normalized.includes("fallback tecnico")
      || normalized.includes("erro ao interpretar json")
      || normalized.includes("json fora do contrato")
      || normalized.includes("resposta vazia da ia")
      || normalized.includes("ia nao retornou json");
  }

  private resolveSafeScheduleDecisionDate(
    conversation: any,
    config: any,
    requestedDate: Date,
    now: Date = new Date(),
  ): Date {
    const minimumDate = new Date(now.getTime() + 2 * 60 * 1000);
    let scheduleDate = Number.isFinite(requestedDate?.getTime())
      ? requestedDate
      : null;

    if (!scheduleDate || scheduleDate.getTime() <= minimumDate.getTime()) {
      scheduleDate = this.buildCurrentStageScheduleDate(conversation, config, now) || minimumDate;
    }

    if (scheduleDate.getTime() <= minimumDate.getTime()) {
      scheduleDate = minimumDate;
    }

    return addRandomSeconds(scheduleDate);
  }
  ...
  private async hasCustomerReplyInConversation(
    conversationId: string,
    customerReplyCutoff: Date | null,
  ): Promise<boolean> {
    const result = await db.execute(sql`
      SELECT 1
      FROM messages
      WHERE conversation_id = ${conversationId}
        AND from_me = false
        AND (${customerReplyCutoff}::timestamp IS NULL OR COALESCE(timestamp, created_at) >= ${customerReplyCutoff}::timestamp)
        AND (
          NULLIF(BTRIM(COALESCE(text, '')), '') IS NOT NULL
          OR media_type IS NOT NULL
          OR media_url IS NOT NULL
          OR media_url_original IS NOT NULL
          OR NULLIF(BTRIM(COALESCE(media_caption, '')), '') IS NOT NULL
        )
      LIMIT 1
    `);
  ...
  private async alignOwnerVerificationWithRecentCustomerReply(conversation: any): Promise<boolean> {
    if (!conversation?.id) {
      return false;
    }

    const ownerVerifiedAt = getFollowUpOwnerPhoneVerifiedAt(conversation);
    if (!ownerVerifiedAt) {
      return false;
    }

    const customerWindowStart = new Date(
      ownerVerifiedAt.getTime() - OWNER_VERIFICATION_CUSTOMER_REPLY_BRIDGE_MS,
    );
    const companyWindowStart = new Date(
      ownerVerifiedAt.getTime() - OWNER_VERIFICATION_COMPANY_REPLY_TOLERANCE_MS,
    );

    const result = await db.execute(sql`
      SELECT
        customer.customer_at AS "customerAt",
        company.company_at AS "companyAt"
      FROM (
        SELECT COALESCE(m.timestamp, m.created_at) AS customer_at
        FROM messages m
        WHERE m.conversation_id = ${conversation.id}
          AND m.from_me = false
          AND COALESCE(m.timestamp, m.created_at) < ${ownerVerifiedAt}
          AND COALESCE(m.timestamp, m.created_at) >= ${customerWindowStart}
          AND (
            NULLIF(BTRIM(COALESCE(m.text, '')), '') IS NOT NULL
            OR m.media_type IS NOT NULL
            OR m.media_url IS NOT NULL
      ...
      JOIN LATERAL (
        SELECT COALESCE(m.timestamp, m.created_at) AS company_at
        FROM messages m
        WHERE m.conversation_id = ${conversation.id}
          AND m.from_me = true
          AND COALESCE(m.timestamp, m.created_at) >= customer.customer_at
          AND COALESCE(m.timestamp, m.created_at) >= ${companyWindowStart}
          AND (
            m.status IS NULL
    ...
    const row = (result.rows || [])[0] as { customerAt?: unknown; companyAt?: unknown } | undefined;
    const customerAt = parseFollowUpDate(row?.customerAt);
    const companyAt = parseFollowUpDate(row?.companyAt);

    if (!customerAt || !companyAt || companyAt.getTime() < customerAt.getTime()) {
      return false;
    }

    await db.update(conversations)
      .set({
        ownerPhoneVerifiedAt: customerAt,
        updatedAt: new Date(),
      })
      .where(eq(conversations.id, conversation.id));

    conversation.ownerPhoneVerifiedAt = customerAt;
    conversation.conversationOwnerPhoneVerifiedAt = customerAt;
    console.log(
      `[USER-FOLLOW-UP] ${OWNER_VERIFICATION_CUSTOMER_REPLY_BRIDGE_MARKER} alinhou confirmacao do dono para ${conversation.contactNumber || conversation.id}.`,
    );
    return true;
  }

  private async ensureConversationHasCustomerReplyForFollowUp(
    conversation: any,
    userId: string,
    phase: string,
  ): Promise<boolean> {
    ...
    const customerReplyCutoff = getFollowUpOwnerPhoneVerifiedAt(conversation);
    if (await this.hasCustomerReplyInConversation(conversation.id, customerReplyCutoff)) {
      if (conversation.followupDisabledReason === WAITING_FOR_FIRST_CUSTOMER_REPLY_REASON) {
        await db.update(conversations)
          .set({
    ...
    if (await this.alignOwnerVerificationWithRecentCustomerReply(conversation)) {
      const alignedCustomerReplyCutoff = getFollowUpOwnerPhoneVerifiedAt(conversation);
      if (await this.hasCustomerReplyInConversation(conversation.id, alignedCustomerReplyCutoff)) {
        if (conversation.followupDisabledReason === WAITING_FOR_FIRST_CUSTOMER_REPLY_REASON) {
          await db.update(conversations)
            .set({
              followupDisabledReason: null,
    ...
    const alreadyHeld =
      ...
      conversation.followupDisabledReason === WAITING_FOR_FIRST_CUSTOMER_REPLY_REASON;

    await this.holdFollowUpOutOfQueue(conversation, WAITING_FOR_FIRST_CUSTOMER_REPLY_REASON);
    ...
    if (!alreadyHeld) {
      await this.logFollowUp(
        ...
        {
          action: "wait",
          reason: WAITING_FOR_FIRST_CUSTOMER_REPLY_REASON,
          context: { phase },
        },
      ...
      console.log(
        `[USER-FOLLOW-UP] Conversa ${conversation.contactNumber || conversation.id} sem resposta do cliente; fora da fila de follow-up.`,
      );
    }
  ...
  private async loadFreshConversationForFollowUpExecution(conversationId: string, userId: string): Promise<any | null> {
    ...
    if (!conversation?.connection || conversation.connection.userId !== userId) {
      return null;
    }
  ...
  private async tryBackfillOwnerPhoneFromCurrentSession(conversation: any, userId: string): Promise<boolean> {
    ...
    const [updated] = await db.update(conversations)
      .set({
        ...
        ownerPhoneVerifiedAt: firstTimestamp,
  ...
  private async tryBackfillOwnerPhoneFromLinkedConnection(
    conversation: any,
    userId: string,
    connectionState: {
      available: boolean;
    ...
    const decision = canBackfillOwnerPhoneFromLinkedConnection({
      ownerPhone: conversation.ownerPhoneNumber || conversation.conversationOwnerPhoneNumber,
      connectionPhone: connectionState.connectionPhone,
      liveGatewayPhone: connectionState.liveGatewayPhone,
      contactNumber: conversation.contactNumber,
      remoteJid: conversation.remoteJid,
      jidSuffix: conversation.jidSuffix,
      isConnectionAvailable: connectionState.available,
    });
    ...
    const connection = await this.loadConversationConnection(conversation, userId);
    if (!connection || normalizePhoneDigits(connection.phoneNumber || "") !== decision.phone) {
      return false;
    }

    const [updated] = await db.update(conversations)
      .set({
        ownerPhoneNumber: decision.phone,
        ownerPhoneVerifiedAt: new Date(),
        ownerPhoneSource: FOLLOWUP_OWNER_PHONE_LINKED_CONNECTION_SOURCE,
        updatedAt: new Date(),
      })
      .where(and(
        eq(conversations.id, conversation.id),
  ...
  private async ensureConversationSafeForFollowUpQueue(conversation: any, userId: string): Promise<boolean> {
    ...
    if (!getFollowUpOwnerPhone(conversation)) {
      await this.tryBackfillOwnerPhoneFromLinkedConnection(conversation, userId, connectionState);
    }
    ...
    if (isSelfConversationForFollowUp(conversation, getFollowUpOwnerPhone(conversation))) {
      await this.holdFollowUpOutOfQueue(conversation, FOLLOWUP_SELF_CONVERSATION_REASON);
      return false;
    }
  ...
  private async syncFollowUpStageFromSentLogs(conversation: any, userId: string): Promise<number> {
    const result = await db.execute(sql`
      SELECT
        MAX(ufl.stage) AS "maxStage",
        MAX(ufl.executed_at) AS "maxSentAt",
        (
          ...
          WHERE m.conversation_id = ${conversation.id}
            ...
            AND m.timestamp > COALESCE(
              (
                SELECT MAX(sent_logs.executed_at)
                FROM user_followup_logs sent_logs
                WHERE sent_logs.user_id = ${userId}
                  AND sent_logs.conversation_id = ${conversation.id}
                  AND sent_logs.status = 'sent'
              ),
              TIMESTAMP '1900-01-01 00:00:00'
            )
        ) AS "latestClientAfterSentAt",
        (
          SELECT MAX(company_msg.timestamp)
          FROM messages company_msg
          WHERE company_msg.conversation_id = ${conversation.id}
            AND company_msg.from_me = true
            AND company_msg.timestamp > COALESCE(
              (
                SELECT MAX(client_msg.timestamp)
                FROM messages client_msg
                WHERE client_msg.conversation_id = ${conversation.id}
                  AND client_msg.from_me = false
                  AND client_msg.timestamp > COALESCE(
                    (
                      SELECT MAX(sent_logs.executed_at)
                      FROM user_followup_logs sent_logs
                      WHERE sent_logs.user_id = ${userId}
        ...
        ) AS "latestCompanyAfterClientAt"
      FROM user_followup_logs ufl
      WHERE ufl.user_id = ${userId}
        AND ufl.conversation_id = ${conversation.id}
        AND ufl.status = 'sent'
    `);
    const row = (result.rows || [])[0] || {};
    const nextStage = resolveStageAfterSentLogSync({
      currentStage: conversation.followupStage || 0,
      maxSentStage: row.maxStage,
      maxSentAt: row.maxSentAt as any,
      latestClientAfterSentAt: row.latestClientAfterSentAt as any,
      latestCompanyAfterClientAt: row.latestCompanyAfterClientAt as any,
    });
  ...
  private async sendFollowUpAsAudio(
    ...
    const conversationAudioLimit = resolveAgentAudioPerConversationDailyLimit(usage.limit);
    const conversationAudioUsed = await storage.getConversationAgentAudioCountToday(userId, conversationId);
    if (conversationAudioUsed >= conversationAudioLimit) {
      return {
        success: false,
        error: `Limite diario de audio por conversa atingido (${conversationAudioUsed}/${conversationAudioLimit})`,
      };
    }
  ...
  private async deliverFollowUpMessage(
    ...
    const audioResult = await this.sendFollowUpAsAudio(userId, conversation.id, conversation.remoteJid, message, socket);
  ...
  private async recoverStaleWaitingConnectionReasons(onlyUserId?: string): Promise<void> {
    ...
    for (const conversation of waitingConversations) {
      ...
      const memoryActive = await isUserConnectionActive(userId, connectionId);
      const ownerState = resolveFollowUpOwnerPhoneState(
        conversation,
        conversation.connection?.phoneNumber,
      );
      const connectionRecordActive =
        connectionRecordLooksConnectedForFollowUp(conversation.connection) && ownerState.ok;
      const isActive = memoryActive || connectionRecordActive;
  ...
  async runCycleOnce(options?: {
    ...
    const parsedCompanyReplyRecoveryLimit = Number(
      process.env.USER_FOLLOWUP_COMPANY_REPLY_RECOVERY_LIMIT || 400,
    );
    const companyReplyRecoveryLimit = Number.isFinite(parsedCompanyReplyRecoveryLimit)
      ? Math.max(0, Math.floor(parsedCompanyReplyRecoveryLimit))
      : 400;
    ...
    if (companyReplyRecoveryLimit > 0) {
      if (this.isCompanyReplyRecoveryRunning) {
        repairSummary = {
          ...(repairSummary || {}),
          companyReplyWaiting: { skipped: "company_reply_recovery_busy" },
        };
      } else {
        this.isCompanyReplyRecoveryRunning = true;
        try {
          repairSummary = {
            ...(repairSummary || {}),
            companyReplyWaiting: await runRepair(
              "companyReplyWaiting",
              () => this.repairWaitingCompanyReplyStuckSchedules(
                companyReplyRecoveryLimit,
                options?.onlyUserId,
              ),
            ),
          };
        } finally {
          this.isCompanyReplyRecoveryRunning = false;
        }
      }
    }
  ...
  private async processFollowUps(onlyUserId?: string) {
    ...
    try {
      ...
      await this.recoverStaleWaitingConnectionReasons(onlyUserId);
      ...
      // Em backlog grande, clientes que conversaram mais recentemente devem sair primeiro.
      // Conversas antigas continuam em ondas seguintes, sem furar o escopo por conexão.
      const sorted = sortUserFollowUpBacklogEntries(
      ...
      let skippedForMissingCustomerReply = 0;
      ...
      for (const conversation of cycleConversations) {
        ...
        if (!await this.ensureConversationHasCustomerReplyForFollowUp(conversation, userId, "cycle_candidate")) {
          skippedForMissingCustomerReply += 1;
          continue;
        }
      ...
      if (skippedForMissingCustomerReply > 0) {
        console.log(
          `[USER-FOLLOW-UP] ${skippedForMissingCustomerReply} conversation(s) removed from queue because the customer has not replied yet`,
        );
      }
  ...
  private async executeFollowUp(conversation: any) {
    ...
    // Recarrega a conversa com a conexão atual antes de qualquer decisão.
    // Isso evita usar snapshot antigo quando a agenda foi reorganizada ou a conversa mudou de estado.
    const freshConversation = await this.loadFreshConversationForFollowUpExecution(conversation.id, userId);
    if (!freshConversation || !freshConversation.followupActive) {
    ...
    if (!await this.ensureConversationHasCustomerReplyForFollowUp(conversation, userId, "execucao")) {
      return;
    }
    ...
    try {
      ...
      if (decision.action === 'schedule' && decision.scheduleDate) {
        const requestedScheduleDate = new Date(decision.scheduleDate);
        const scheduleDate = this.resolveSafeScheduleDecisionDate(conversation, config, requestedScheduleDate);
        const scheduleDateLabel = scheduleDate.toLocaleDateString("pt-BR", { timeZone: "America/Sao_Paulo" });
        const technicalScheduleReason = this.isTechnicalScheduleDecisionReason(decision.reason);
        const loggedDecision = {
          ...decision,
          scheduleDate: scheduleDate.toISOString(),
        };

        console.log(`[USER-FOLLOW-UP] Follow-up reagendado para ${scheduleDateLabel}: ${decision.reason}`);
        await db.update(conversations)
          .set({
            nextFollowupAt: scheduleDate,
            followupDisabledReason: technicalScheduleReason
              ? null
              : `Retornar em ${scheduleDateLabel}`,
            updatedAt: new Date(),
          })
          .where(eq(conversations.id, conversation.id));
        await this.logFollowUp(
          ...
          null,
          loggedDecision,
          technicalScheduleReason
            ? `Rechecagem agendada para ${scheduleDateLabel}`
            : `Reagendado para ${scheduleDateLabel} conforme combinado`,
        );
        return;
      }
      ...
      if (decision.action === 'send' && decision.message) {
        // Revalidar estado e conexão logo antes de enviar.
        const freshBeforeSend = await this.loadFreshConversationForFollowUpExecution(conversation.id, userId);
        if (!freshBeforeSend || !freshBeforeSend.followupActive) {
        ...
        conversation = freshBeforeSend;

        if (!await this.ensureConversationSafeForFollowUpQueue(conversation, userId)) {
          return;
        }

        if (!await this.ensureConversationHasCustomerReplyForFollowUp(conversation, userId, "before_send")) {
          return;
        }
  ...
  async getFollowupConfig(userId: string) {
    ...
    if (cached && (Date.now() - cached.timestamp < CACHE_TTL_MS)) {
      return normalizeFollowupConfigForRuntime(cached.data);
    }
    ...
    // 🚀 Salvar no cache
    const normalizedConfig = normalizeFollowupConfigForRuntime(config);

    followupConfigCache.set(userId, { data: normalizedConfig, timestamp: Date.now() });
  ...
  async updateFollowupConfig(userId: string, data: Partial<typeof followupConfigs.$inferInsert>) {
    ...
    if (Object.prototype.hasOwnProperty.call(cleanData, "importantInfo")) {
      const parsedImportantInfo = followupImportantInfoSchema.safeParse(cleanData.importantInfo);
      cleanData.importantInfo = parsedImportantInfo.success ? parsedImportantInfo.data : [];
    }
    ...
    if (existing) {
      ...
      // 🚀 Atualizar cache
      const normalizedUpdated = normalizeFollowupConfigForRuntime(updated);
      followupConfigCache.set(userId, { data: normalizedUpdated, timestamp: Date.now() });
      result = normalizedUpdated;
    } else {
      ...
      // 🚀 Salvar no cache
      const normalizedCreated = normalizeFollowupConfigForRuntime(created);
      followupConfigCache.set(userId, { data: normalizedCreated, timestamp: Date.now() });
      result = normalizedCreated;
    }
    ...
    if (cleanData.isEnabled === false) {
      console.log(`🛑 [USER-FOLLOW-UP] Follow-up GLOBAL desativado pelo usuário ${userId}. Pausando conversas ativas com snapshot...`);
      try {
        const paused = await this.pauseActiveConversationsForGlobalDisable(userId);
        console.log(`✅ [USER-FOLLOW-UP] ${paused} conversas do usuário ${userId} foram pausadas com snapshot da agenda.`);
      } catch (err) {
        console.error(`❌ [USER-FOLLOW-UP] Erro ao pausar conversas ativas:`, err);
      }
    }

    if (!result) {
      throw new Error("Nao foi possivel salvar a configuracao de follow-up.");
    }

    if (!wasEnabledBefore && willBeEnabled) {
      try {
        const restored = await this.restoreGlobalDisabledConversations(userId, result);
        console.log(`✅ [USER-FOLLOW-UP] Follow-up GLOBAL reativado para ${userId}. ${restored.restored} conversas restauradas, ${restored.skipped} ignoradas.`);
      } catch (err) {
        console.error(`❌ [USER-FOLLOW-UP] Erro ao restaurar conversas pausadas globalmente:`, err);
      }
    }
...
- Ações válidas neste momento: ${allowedActionsLabel}.`;
    ...
    const decisionActionExample = decisionWindow.allowedActions.includes("send")
      ? "send"
      : decisionWindow.allowedActions[0] || "wait";
...
## SAIDA PELO CODEX VIVO:
A decisao final deve sair apenas pelo contrato Codex vivo agentezap_live_cli_plan_v1 descrito abaixo.`;

    const buildTechnicalDecisionFallback = (reason: string) => {
      const safeReason = `Fallback tecnico sem autoria publica: ${reason}`.slice(0, 600);

      if (!decisionWindow.allowedActions.includes("schedule")) {
        return {
      ...
      return {
        ...
        reason: safeReason,
        scheduleDate: (fallbackDate || new Date(now.getTime() + Math.max(30, silenceThresholdMinutes) * 60 * 1000)).toISOString(),
      };
    ...
    const parseDecision = (rawContent: unknown): {
      ...
      if (!content) {
        return { ok: false, reason: "Fallback técnico: resposta vazia da IA" };
      }

      const jsonBlock = extractUserFollowUpJsonObject(content);
      if (!jsonBlock) {
        return { ok: false, reason: "Fallback técnico: IA não retornou JSON válido" };
      }

      try {
        const candidatePayload = JSON.parse(jsonBlock);
        if (
          candidatePayload &&
          typeof candidatePayload === "object" &&
          typeof candidatePayload.action === "string" &&
          candidatePayload.action.includes("|")
        ) {
          const actionText = candidatePayload.action;
          const candidateActions = decisionWindow.allowedActions.filter((action) => actionText.includes(action));
          const preferredAction =
            candidateActions.includes("send") && String(candidatePayload.message || "").trim()
              ? "send"
              : candidateActions.includes("schedule") && candidatePayload.scheduleDate
                ? "schedule"
                : candidateActions[0] || decisionActionExample;
          candidatePayload.action = preferredAction;
        }
        const normalized = normalizeUserFollowUpAiDecisionPayload(candidatePayload);
        if (!normalized.ok) {
          return normalized;
        }
        return { ok: true, decision: normalized.decision };
      } catch (error) {
        return { ok: false, reason: "Fallback técnico: erro ao interpretar JSON da IA" };
      }
    };
    ...
    try {
      ...
      console.warn("[USER-FOLLOW-UP] Nao foi possivel carregar email do owner para scope Codex:", error);
    }
    const codexScope: AgenteZapLiveCliScope =
      ownerEmail === RODRIGO_AGENT_CREATOR_EMAIL ? "rodrigo_existing_account_support" : "tenant_customer_support";

    const requestDecisionContent = async (retryReason?: string) => {
      const retryBlock = retryReason
        ? [
            "",
            "## RETENTATIVA ESTRUTURADA",
            `A resposta anterior falhou no contrato: ${retryReason}`,
            "Retorne apenas um objeto JSON valido, sem markdown e sem texto antes/depois.",
            `A action deve ser uma destas: ${decisionWindow.allowedActions.join(", ")}.`,
            "Use o contrato Codex vivo: customerFacingMessages para SEND, no_send para WAIT/ABORT, schedule_followup para SCHEDULE.",
          ].join("\n")
        : "";

      const codexContract = [
        "## CONTRATO CODEX VIVO",
        "Use o schema agentezap_live_cli_plan_v1.",
        `A action de follow-up deve ser uma destas: ${decisionWindow.allowedActions.join(", ")}.`,
        "Para SEND: decision respond, customerFacingMessages com a mensagem publica final e actions vazio.",
        "Para WAIT: decision no_send e action no_send com arguments.reason iniciando ACTION=wait;.",
        "Para ABORT: decision no_send e action no_send com arguments.reason iniciando ACTION=abort;.",
        "Para SCHEDULE: action schedule_followup com arguments.reason iniciando ACTION=schedule; e arguments.scheduleDate em ISO.",
        "Nao retorne JSON textual solto ao cliente. A fala publica, quando houver, precisa estar em customerFacingMessages.",
      ].join("\n");
      ...
      const result = await runAgenteZapLiveCliRuntime({
        ...
        ownerEmail,
        userId,
        conversationId: `user-followup:${conversation.id}`,
        contactPhone: conversation.contactNumber,
        currentMessage: [prompt, retryBlock, codexContract].filter(Boolean).join("\n\n"),
        messages: [...recentMessages].reverse().map((message) => ({
          role: message.fromMe ? "assistant" : "user",
          content: String(message.text || message.mediaType || "").trim() || "[midia]",
        })),
        contextArtifacts: {
          source: "userFollowUpService",
          decisionWindow,
          followupStage: conversation.followupStage || 0,
          config,
          businessContext,
          agentPromptContext,
          realEstatePromptBlock,
        ...
        timeoutMs: 90_000,
        maxTokens: retryReason ? 700 : 900,
      });

      const plan = result.plan;
      const noSendAction = (plan.actions || []).find((action) => action.type === "no_send");
      const scheduleAction = (plan.actions || []).find((action) => action.type === "schedule_followup");
      const publicMessage = (plan.customerFacingMessages || []).map((message) => String(message || "").trim()).filter(Boolean).join("\n\n");
      const actionReason = String(
        scheduleAction?.arguments?.reason ||
        scheduleAction?.reason ||
        noSendAction?.arguments?.reason ||
        noSendAction?.reason ||
        plan.evidence?.uncertainty?.join("; ") ||
        "",
      ).trim();
      const normalizedReason = actionReason.toLowerCase();

      if (publicMessage && decisionWindow.allowedActions.includes("send")) {
        return JSON.stringify({
          action: "send",
          reason: actionReason || "Codex gerou mensagem de follow-up",
          message: publicMessage,
        });
      }

      if (scheduleAction && decisionWindow.allowedActions.includes("schedule")) {
        return JSON.stringify({
          action: "schedule",
          reason: actionReason || "Codex agendou nova tentativa de follow-up",
          scheduleDate: scheduleAction.arguments?.scheduleDate || scheduleAction.arguments?.scheduledAt || scheduleAction.arguments?.date,
        });
      }

      if (normalizedReason.startsWith("action=abort") && decisionWindow.allowedActions.includes("abort")) {
        return JSON.stringify({
          action: "abort",
          reason: actionReason || "Codex decidiu abortar follow-up",
        });
      }

      return JSON.stringify({
        action: "wait",
        reason: actionReason || "Codex decidiu aguardar follow-up",
      });
    };

    const requestParsedDecision = async () => {
      let firstFailureReason = "Fallback técnico: erro na análise da IA";

      try {
        const content = await requestDecisionContent();
        const parsed = parseDecision(content);
        if (parsed.ok) {
          return parsed.decision;
        }

        firstFailureReason = parsed.reason;
        console.warn(`[USER-FOLLOW-UP] IA retornou decisão inválida para ${conversation.contactNumber}. Tentando retentativa estruturada. Motivo: ${parsed.reason}`);
      } catch (error) {
        console.warn(`[USER-FOLLOW-UP] Erro na primeira análise IA para ${conversation.contactNumber}. Tentando retentativa estruturada.`, error);
      }

      try {
        const retryContent = await requestDecisionContent(firstFailureReason);
        const retryParsed = parseDecision(retryContent);
        if (retryParsed.ok) {
          return retryParsed.decision;
        }

        console.warn(`[USER-FOLLOW-UP] Retentativa estruturada ainda inválida para ${conversation.contactNumber}. Usando fallback seguro. Motivo: ${retryParsed.reason}`);
        return buildTechnicalDecisionFallback(retryParsed.reason);
      } catch (retryError) {
        console.warn(`[USER-FOLLOW-UP] Retentativa estruturada falhou para ${conversation.contactNumber}. Usando fallback seguro.`, retryError);
        return buildTechnicalDecisionFallback(firstFailureReason);
      }
    };

    try {
      const parsed = await requestParsedDecision();
      ...
      return buildTechnicalDecisionFallback("Fallback técnico: erro na análise da IA");
    }
  }
  ...
  private async restoreGlobalDisabledConversations(
    ...
    for (const conversation of candidates) {
      ...
      if (!await this.ensureConversationHasCustomerReplyForFollowUp(conversation, userId, "restore_global")) {
        skipped += 1;
        continue;
      }
  ...
  async repairWaitingCompanyReplyStuckSchedules(
    limit: number = 400,
    onlyUserId?: string,
  ): Promise<{ scanned: number; repaired: number; skipped: number }> {
    const normalizedLimit = Math.max(1, Math.floor(Number(limit) || 400));
    const perUserLimit = Math.max(
      1,
      Math.floor(Number(process.env.USER_FOLLOWUP_COMPANY_REPLY_RECOVERY_PER_USER_LIMIT || 30) || 30),
    );
    const candidateScanLimit = Math.max(normalizedLimit * 10, normalizedLimit);
    const userFilter = onlyUserId ? sql`AND wc.user_id = ${onlyUserId}` : sql``;
    const safeOwnerFilter = sql`
      AND (
        NULLIF(regexp_replace(COALESCE(c.owner_phone_number, ''), '\\D', '', 'g'), '') IS NULL
        OR regexp_replace(COALESCE(c.owner_phone_number, ''), '\\D', '', 'g') = regexp_replace(COALESCE(wc.phone_number, ''), '\\D', '', 'g')
      )
      AND EXISTS (
        SELECT 1
        FROM messages m
      ...
      AND COALESCE(c.remote_jid, '') NOT LIKE '%@g.us'
      AND COALESCE(c.contact_number, '') NOT LIKE '%@g.us'
      AND COALESCE(c.contact_number, '') NOT LIKE '120363%'
    `;
    ...
    const result = await db.execute(sql`
      WITH candidate_conversations AS (
        SELECT
          ...
          c.owner_phone_number AS "ownerPhoneNumber",
          c.owner_phone_verified_at AS "ownerPhoneVerifiedAt",
          c.remote_jid AS "remoteJid",
          c.jid_suffix AS "jidSuffix",
          c.followup_active AS "followupActive",
          COALESCE(c.followup_stage, 0) AS "followupStage",
          c.next_followup_at AS "nextFollowupAt",
          c.followup_disabled_reason AS "followupDisabledReason",
          c.last_message_time AS "lastMessageTime",
          c.last_message_from_me AS "lastMessageFromMe",
          wc.user_id AS "userId",
          wc.connection_name AS "connectionName",
          wc.phone_number AS "connectionPhoneNumber",
          wc.is_connected AS "connectionIsConnected",
          wc.provider AS "connectionProvider",
          wc.provider_status AS "connectionProviderStatus",
          wc.connection_method AS "connectionMethod",
          wc.session_data AS "connectionSessionData"
        FROM conversations c
        JOIN whatsapp_connections wc
          ON wc.id = c.connection_id
        JOIN followup_configs fc
          ON fc.user_id = wc.user_id
      ...
      candidates AS (
        SELECT
          cc.*,
          lm.latest_from_me AS "latestFromMe",
          lm.latest_status AS "latestStatus",
          lm.latest_message_at AS "latestMessageAt",
          ROW_NUMBER() OVER (
        ...
        JOIN LATERAL (
          SELECT
            m.from_me AS latest_from_me,
            m.status AS latest_status,
            COALESCE(m.timestamp, m.created_at) AS latest_message_at
          FROM messages m
          WHERE m.conversation_id = cc."conversationId"
            AND (
              NULLIF(BTRIM(COALESCE(m.text, '')), '') IS NOT NULL
              OR m.media_type IS NOT NULL
              OR m.media_url IS NOT NULL
    ...
    const rows = (result.rows || []) as Array<{
      ...
      connectionProviderStatus: string | null;
      connectionMethod: string | null;
      connectionSessionData: unknown;
      latestFromMe: boolean | null;
    ...
    for (const row of rows) {
      const config = await this.getFollowupConfig(row.userId);
      ...
      if (!isWaitingForCompanyReplyReason(row.followupDisabledReason)) {
        skipped += 1;
        index += 1;
        continue;
      }

      if (shouldHoldFollowUpForOutgoingConfirmation({
        fromMe: row.latestFromMe,
      ...
      const conversation = {
        ...
        ownerPhoneVerifiedAt: row.ownerPhoneVerifiedAt,
        conversationOwnerPhoneVerifiedAt: row.ownerPhoneVerifiedAt,
        remoteJid: row.remoteJid,
        jidSuffix: row.jidSuffix,
      ...
      if (!await this.ensureConversationSafeForFollowUpQueue(conversation, row.userId)) {
        skipped += 1;
        index += 1;
        continue;
      }

      if (!await this.ensureConversationHasCustomerReplyForFollowUp(conversation, row.userId, "company_reply_recovery")) {
        skipped += 1;
        index += 1;
        continue;
      }

      const isExcludedFromFollowup = await storage.isNumberExcludedFromFollowup(
        row.userId,
        row.contactNumber || "",
      );
      if (isExcludedFromFollowup) {
        await this.holdFollowUpOutOfQueue(conversation, "Numero na lista de exclusao");
        skipped += 1;
        index += 1;
        continue;
      }

      const offsetMinutes = 5 + index * 2;
      const referenceTime = new Date(Date.now() + offsetMinutes * 60 * 1000);
      ...
      await this.enforcePriorityOverAdminConflicts(
        conversation,
        row.userId,
        "company_reply_recovery",
      );

      repaired += 1;
      index += 1;
      console.log(
        `[USER-FOLLOW-UP] Recuperado follow-up aguardando empresa para ${row.contactNumber || row.conversationId}: stage 0 em ${restartSchedule.nextFollowupAt.toISOString()}.`,
      );
    }
  ...
  async repairMissingSchedules(limit: number = 50000, onlyUserId?: string): Promise<{ scanned: number; repaired: number; disabled: number; skipped: number }> {
    await this.backfillMissingOwnerPhonesFromLinkedConnections(limit, onlyUserId);
        ...
        and(
          ...
          sql`EXISTS (
            ...
            WHERE m.conversation_id = ${conversations.id}
              AND m.from_me = false
              AND (${conversations.ownerPhoneVerifiedAt} IS NULL OR COALESCE(m.timestamp, m.created_at) >= ${conversations.ownerPhoneVerifiedAt})
              AND (
                NULLIF(BTRIM(COALESCE(m.text, '')), '') IS NOT NULL
                OR NULLIF(BTRIM(COALESCE(m.media_caption, '')), '') IS NOT NULL
                OR m.media_type IS NOT NULL
    ...
    for (const conversation of missingSchedules) {
      ...
      if (!userId) {
        await this.disableFollowUp(conversation.id, "Conexão removida - sem userId");
        disabled += 1;
        continue;
      }
      ...
      if (!await this.ensureConversationHasCustomerReplyForFollowUp(conversation, userId, "repair_missing_schedule")) {
        skipped += 1;
        continue;
      }
  ...
  private async scheduleNextFollowUp(conversationId: string, date: Date) {
    if (!Number.isFinite(date?.getTime())) {
      throw new Error("Data invalida para agendar follow-up.");
    }
  ...
  async disableFollowUp(conversationId: string, reason: string = "Desativado") {
    ...
    if (!conversation?.connection?.userId) {
      throw new Error("Conversa sem usuario dono confirmado para follow-up.");
    }
  ...
  async enableFollowUp(conversationId: string) {
    ...
    if (!await this.ensureConversationHasCustomerReplyForFollowUp(conversation, userId, "ativacao")) {
      return;
    }
  ...
  async resetFollowUpCycle(conversationId: string, reason?: string, referenceTime?: Date) {
    ...
    if (!await this.ensureConversationHasCustomerReplyForFollowUp(conversation, userId, "reset")) {
      return;
    }
  ...
  async scheduleManualFollowUp(conversationId: string, scheduledFor: Date, note?: string) {
    ...
    const config = await this.getFollowupConfig(conversation.connection.userId);
    if (!config?.isEnabled) {
      await db.update(conversations)
        .set({
          followupActive: false,
          nextFollowupAt: null,
          followupDisabledReason: GLOBAL_FOLLOWUP_DISABLED_REASON_ASCII,
          updatedAt: new Date(),
        })
        .where(eq(conversations.id, conversationId));
      throw new Error("O Follow-up Inteligente esta desligado nesta conta.");
    }
    ...
    return memoryCache.getOrCompute(cacheKey, async () => {
      ...
      const rows = await db
        ...
        .orderBy(
          ...
          sql`${conversations.nextFollowupAt} asc`,
        );
  ...
  async repairTooEarlySchedulesAfterTimezoneRegression(
    ...
    const userFilter = onlyUserId ? sql`AND wc.user_id = ${onlyUserId}` : sql``;
    const normalizedLimit = Math.max(1, Math.floor(Number(limit) || 50000));
    const safeOwnerFilter = sql`
      AND NULLIF(regexp_replace(COALESCE(c.owner_phone_number, ''), '\\D', '', 'g'), '') IS NOT NULL
    ...
    const result = await db.execute(sql`
      ...
      SELECT
        ...
        cc.last_message_from_me,
        cc.followup_disabled_reason,
      ...
      FROM candidate_conversations cc
      LEFT JOIN LATERAL (
        SELECT l.executed_at AS last_followup_sent_at
        FROM user_followup_logs l
        WHERE l.conversation_id = cc.conversation_id
          AND l.status = 'sent'
      ...
      ORDER BY cc.next_followup_at ASC
      LIMIT ${normalizedLimit}
    `);
  ...
  async repairMissedCycleResetsAfterCompanyReply(
    ...
    const result = await db.execute(sql`
      WITH candidate_conversations AS (
        ...
        JOIN whatsapp_connections wc
          ON wc.id = c.connection_id
        WHERE c.last_message_from_me = true
      ...
      SELECT
        ...
        cc.followup_disabled_reason,
        cc.last_message_time,
        company.last_company_after_followup
      FROM candidate_conversations cc
      JOIN LATERAL (
        SELECT l.executed_at AS last_followup_sent_at
        FROM user_followup_logs l
        WHERE l.conversation_id = cc.conversation_id
          AND l.user_id = cc.user_id
          AND l.status = 'sent'
        ORDER BY l.executed_at DESC
        LIMIT 1
      ) lfs ON true
      LEFT JOIN LATERAL (
        SELECT MAX(m.timestamp) AS last_customer_after_followup
        FROM messages m
        WHERE m.conversation_id = cc.conversation_id
          AND m.from_me = false
      ...
      LEFT JOIN LATERAL (
        SELECT MAX(m.timestamp) AS last_company_after_followup
        FROM messages m
        WHERE m.conversation_id = cc.conversation_id
          AND m.from_me = true
          AND m.timestamp > lfs.last_followup_sent_at
      ...
      WHERE customer.last_customer_after_followup IS NOT NULL
        AND company.last_company_after_followup IS NOT NULL
        AND company.last_company_after_followup > customer.last_customer_after_followup
      ORDER BY company.last_company_after_followup DESC
      LIMIT ${normalizedLimit}
    `);
  ...
  async repairMissingActivationAfterCompanyOutbound(
    ...
    const result = await db.execute(sql`
      ...
      LEFT JOIN LATERAL (
        SELECT MAX(l.executed_at) AS last_cancelled_at
        FROM user_followup_logs l
        WHERE l.conversation_id = cc.conversation_id
  ...
  async reorganizeAllFollowups(userId: string): Promise<{ reorganized: number; skipped: number }> {
    ...
    for (const conversation of userConversations) {
      try {
        ...
        if (!await this.ensureConversationHasCustomerReplyForFollowUp(conversation, userId, "reorganizacao")) {
          skipped++;
          continue;
        }
  ...
  async clearConnectionWaitingStatus(connectionId: string): Promise<number> {
    try {
      ...
      await db.execute(sql`
        UPDATE conversations c
        SET next_followup_at = NULL,
            followup_disabled_reason = ${WAITING_FOR_FIRST_CUSTOMER_REPLY_REASON},
            updated_at = NOW()
        WHERE c.connection_id = ${connectionId}
          AND c.followup_active = true
          AND c.followup_disabled_reason = ${WAITING_FOR_WHATSAPP_CONNECTION_REASON}
          AND regexp_replace(COALESCE(c.owner_phone_number, ''), '\\D', '', 'g') = ${connectionPhone}
          AND NOT EXISTS (
            SELECT 1
            FROM messages m
            WHERE m.conversation_id = c.id
      ...
      const result = await db.execute(sql`
        ...
        WHERE connection_id = ${connectionId}
          ...
          AND EXISTS (
            ...
            WHERE m.conversation_id = conversations.id
              AND m.from_me = false
              AND (conversations.owner_phone_verified_at IS NULL OR COALESCE(m.timestamp, m.created_at) >= conversations.owner_phone_verified_at)
              AND (
                NULLIF(BTRIM(COALESCE(m.text, '')), '') IS NOT NULL
                OR m.media_type IS NOT NULL
                OR m.media_url IS NOT NULL
